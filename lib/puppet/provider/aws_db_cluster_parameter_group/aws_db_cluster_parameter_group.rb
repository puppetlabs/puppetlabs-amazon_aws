...
Puppet::Type.type(:aws_db_cluster_parameter_group).provide(:arm) do
  mk_resource_methods
  ...
  def db_cluster_parameter_group_arn=(value)
    Puppet.info("db_cluster_parameter_group_arn setter called to change to #{value}")
    @property_flush[:db_cluster_parameter_group_arn] = value
  end
  ...
  def build_hash
    ...
    db_cluster_parameter_group[self.namevar] = resource[:name]
    return symbolize(db_cluster_parameter_group)
  end
  ...
  def create
    ...
    response = client.create_db_cluster_parameter_group(build_hash)
  ...
  def destroy
    ...
    @is_delete = true
    Puppet.info("Calling operation delete_db_cluster_parameter_group")
    client = Aws::RDS::Client.new(region: self.class.get_region)
    client.delete_db_cluster_parameter_group(build_hash)
