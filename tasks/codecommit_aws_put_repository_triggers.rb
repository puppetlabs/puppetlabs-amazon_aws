...
def operation_with_hash(arg_hash)
  ...
  client.put_repository_triggers(call_hash)
end

def put_repository_triggers(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeCommit::Client.new(region: region)
  response = client.put_repository_triggers if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_repository_triggers(params)
