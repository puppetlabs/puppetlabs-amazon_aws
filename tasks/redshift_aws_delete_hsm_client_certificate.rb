...
def operation_with_hash(arg_hash)
  ...
  client.delete_hsm_client_certificate(call_hash)
end

def delete_hsm_client_certificate(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.delete_hsm_client_certificate if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_hsm_client_certificate(params)
