...
def operation_with_hash(arg_hash)
  ...
  client.describe_nfs_file_shares(call_hash)
end

def describe_nfs_file_shares(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.describe_nfs_file_shares if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_nfs_file_shares(params)
