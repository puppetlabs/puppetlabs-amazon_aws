...
def operation_with_hash(arg_hash)
  ...
  client.put_room_skill_parameter(call_hash)
end

def put_room_skill_parameter(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.put_room_skill_parameter if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_room_skill_parameter(params)
