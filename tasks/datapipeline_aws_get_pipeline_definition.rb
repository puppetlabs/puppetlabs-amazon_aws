...
def operation_with_hash(arg_hash)
  ...
  client = Aws::DataPipeline::Client.new(region: region)
  client.get_pipeline_definition(call_hash)
end

def get_pipeline_definition(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DataPipeline::Client.new(region: region)
  response = client.get_pipeline_definition if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_pipeline_definition(params)
