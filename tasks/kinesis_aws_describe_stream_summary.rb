...
def operation_with_hash(arg_hash)
  ...
  client.describe_stream_summary(call_hash)
end

def describe_stream_summary(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Kinesis::Client.new(region: region)
  response = client.describe_stream_summary if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_stream_summary(params)
