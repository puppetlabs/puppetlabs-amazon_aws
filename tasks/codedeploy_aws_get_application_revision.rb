...
def operation_with_hash(arg_hash)
  ...
  client.get_application_revision(call_hash)
end

def get_application_revision(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeDeploy::Client.new(region: region)
  response = client.get_application_revision if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_application_revision(params)
