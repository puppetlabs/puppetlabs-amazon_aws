...
def operation_with_hash(arg_hash)
  ...
  client.get_public_access_block(call_hash)
end

def get_public_access_block(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::S3::Client.new(region: region)
  response = client.get_public_access_block if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_public_access_block(params)
