...
def operation_with_hash(arg_hash)
  ...
  client.list_volume_initiators(call_hash)
end

def list_volume_initiators(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.list_volume_initiators if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_volume_initiators(params)
