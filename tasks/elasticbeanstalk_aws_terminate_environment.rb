...
def operation_with_hash(arg_hash)
  ...
  client.terminate_environment(call_hash)
end

def terminate_environment(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.terminate_environment if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = terminate_environment(params)
