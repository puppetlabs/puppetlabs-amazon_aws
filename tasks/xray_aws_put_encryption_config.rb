...
def operation_with_hash(arg_hash)
  ...
  client.put_encryption_config(call_hash)
end

def put_encryption_config(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::XRay::Client.new(region: region)
  response = client.put_encryption_config if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_encryption_config(params)
