...
def operation_with_hash(arg_hash)
  ...
  client.delete_domain_entry(call_hash)
end

def delete_domain_entry(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.delete_domain_entry if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_domain_entry(params)
