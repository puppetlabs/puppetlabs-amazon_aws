...
def operation_with_hash(arg_hash)
  ...
  client.list_reusable_delegation_sets(call_hash)
end

def list_reusable_delegation_sets(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.list_reusable_delegation_sets if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_reusable_delegation_sets(params)
