...
def operation_with_hash(arg_hash)
  ...
  client.generate_change_set(call_hash)
end

def generate_change_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SMS::Client.new(region: region)
  response = client.generate_change_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = generate_change_set(params)
