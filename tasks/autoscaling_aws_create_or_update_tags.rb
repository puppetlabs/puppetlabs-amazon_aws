...
def operation_with_hash(arg_hash)
  ...
  client.create_or_update_tags(call_hash)
end

def create_or_update_tags(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.create_or_update_tags if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_or_update_tags(params)
