...
def operation_with_hash(arg_hash)
  ...
  client.describe_effective_instance_associations(call_hash)
end

def describe_effective_instance_associations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.describe_effective_instance_associations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_effective_instance_associations(params)
