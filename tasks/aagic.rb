...
def operation_with_hash(arg_hash)
  ...
  client.get_invitation_configuration(call_hash)
end

def get_invitation_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.get_invitation_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_invitation_configuration(params)
