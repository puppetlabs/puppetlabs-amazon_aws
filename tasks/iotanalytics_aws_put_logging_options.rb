...
def operation_with_hash(arg_hash)
  ...
  client.put_logging_options(call_hash)
end

def put_logging_options(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoTAnalytics::Client.new(region: region)
  response = client.put_logging_options if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_logging_options(params)
