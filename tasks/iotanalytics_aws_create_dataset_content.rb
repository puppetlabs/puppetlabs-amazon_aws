...
def operation_with_hash(arg_hash)
  ...
  client.create_dataset_content(call_hash)
end

def create_dataset_content(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoTAnalytics::Client.new(region: region)
  response = client.create_dataset_content if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_dataset_content(params)
