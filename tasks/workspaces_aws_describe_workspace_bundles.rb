...
def operation_with_hash(arg_hash)
  ...
  client.describe_workspace_bundles(call_hash)
end

def describe_workspace_bundles(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkSpaces::Client.new(region: region)
  response = client.describe_workspace_bundles if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_workspace_bundles(params)
