...
def operation_with_hash(arg_hash)
  ...
  client.stop_stack_set_operation(call_hash)
end

def stop_stack_set_operation(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudFormation::Client.new(region: region)
  response = client.stop_stack_set_operation if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = stop_stack_set_operation(params)
