...
def operation_with_hash(arg_hash)
  ...
  client.create_cluster_parameter_group(call_hash)
end

def create_cluster_parameter_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.create_cluster_parameter_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_cluster_parameter_group(params)
