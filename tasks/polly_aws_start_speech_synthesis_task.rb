...
def operation_with_hash(arg_hash)
  ...
  client.start_speech_synthesis_task(call_hash)
end

def start_speech_synthesis_task(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Polly::Client.new(region: region)
  response = client.start_speech_synthesis_task if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_speech_synthesis_task(params)
