...
def operation_with_hash(arg_hash)
  ...
  client.list_entities_for_policy(call_hash)
end

def list_entities_for_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.list_entities_for_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_entities_for_policy(params)
