...
def operation_with_hash(arg_hash)
  ...
  client.list_outgoing_certificates(call_hash)
end

def list_outgoing_certificates(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.list_outgoing_certificates if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_outgoing_certificates(params)
