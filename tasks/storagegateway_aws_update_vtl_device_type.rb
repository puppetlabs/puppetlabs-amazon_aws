...
def operation_with_hash(arg_hash)
  ...
  client.update_vtl_device_type(call_hash)
end

def update_vtl_device_type(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.update_vtl_device_type if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_vtl_device_type(params)
