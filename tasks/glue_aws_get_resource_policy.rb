...
def operation_with_hash(arg_hash)
  ...
  client.get_resource_policy(call_hash)
end

def get_resource_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glue::Client.new(region: region)
  response = client.get_resource_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_resource_policy(params)
