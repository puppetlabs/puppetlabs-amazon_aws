...
def operation_with_hash(arg_hash)
  ...
  client.request_environment_info(call_hash)
end

def request_environment_info(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.request_environment_info if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = request_environment_info(params)
