...
def operation_with_hash(arg_hash)
  ...
  client.describe_db_cluster_snapshots(call_hash)
end
...
def describe_db_cluster_snapshots(*args)
  ...
  client = Aws::Neptune::Client.new(region: region)
  response = client.describe_db_cluster_snapshots if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_db_cluster_snapshots(params)
