...
def operation_with_hash(arg_hash)
  ...
  client.list_server_certificates(call_hash)
end

def list_server_certificates(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.list_server_certificates if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_server_certificates(params)
