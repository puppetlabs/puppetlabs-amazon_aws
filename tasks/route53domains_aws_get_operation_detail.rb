...
def operation_with_hash(arg_hash)
  ...
  client.get_operation_detail(call_hash)
end

def get_operation_detail(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53Domains::Client.new(region: region)
  response = client.get_operation_detail if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_operation_detail(params)
