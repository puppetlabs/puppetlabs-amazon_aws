...
def operation_with_hash(arg_hash)
  ...
  client.generate_data_key_without_plaintext(call_hash)
end

def generate_data_key_without_plaintext(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KMS::Client.new(region: region)
  response = client.generate_data_key_without_plaintext if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = generate_data_key_without_plaintext(params)
