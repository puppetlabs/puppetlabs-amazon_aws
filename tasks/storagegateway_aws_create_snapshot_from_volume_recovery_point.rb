...
def operation_with_hash(arg_hash)
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  client.create_snapshot_from_volume_recovery_point(call_hash)
end

def create_snapshot_from_volume_recovery_point(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.create_snapshot_from_volume_recovery_point if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_snapshot_from_volume_recovery_point(params)
