...
def operation_with_hash(arg_hash)
  ...
  client.add_tags_to_resource(call_hash)
end

def add_tags_to_resource(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.add_tags_to_resource if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_tags_to_resource(params)
