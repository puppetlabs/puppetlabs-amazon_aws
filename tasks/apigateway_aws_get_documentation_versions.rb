...
def operation_with_hash(arg_hash)
  ...
  client.get_documentation_versions(call_hash)
end

def get_documentation_versions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.get_documentation_versions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_documentation_versions(params)
