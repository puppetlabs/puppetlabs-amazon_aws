...
def operation_with_hash(arg_hash)
  ...
  client.attach_load_balancers(call_hash)
end

def attach_load_balancers(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.attach_load_balancers if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = attach_load_balancers(params)
