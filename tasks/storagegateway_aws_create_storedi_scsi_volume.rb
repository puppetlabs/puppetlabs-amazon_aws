...
def operation_with_hash(arg_hash)
  ...
  client.create_storedi_scsi_volume(call_hash)
end

def create_storedi_scsi_volume(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.create_storedi_scsi_volume if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_storedi_scsi_volume(params)
