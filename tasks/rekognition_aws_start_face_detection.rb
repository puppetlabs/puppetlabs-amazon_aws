...
def operation_with_hash(arg_hash)
  ...
  client.start_face_detection(call_hash)
end

def start_face_detection(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Rekognition::Client.new(region: region)
  response = client.start_face_detection if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_face_detection(params)
