...
def operation_with_hash(arg_hash)
  ...
  client.delete_job_template(call_hash)
end

def delete_job_template(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaConvert::Client.new(region: region)
  response = client.delete_job_template if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_job_template(params)
