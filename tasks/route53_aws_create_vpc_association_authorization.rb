...
def operation_with_hash(arg_hash)
  ...
  client.create_vpc_association_authorization(call_hash)
end

def create_vpc_association_authorization(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.create_vpc_association_authorization if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_vpc_association_authorization(params)
