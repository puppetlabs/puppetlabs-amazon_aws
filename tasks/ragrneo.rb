...
def operation_with_hash(arg_hash)
  ...
  client.get_reserved_node_exchange_offerings(call_hash)
end

def get_reserved_node_exchange_offerings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.get_reserved_node_exchange_offerings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_reserved_node_exchange_offerings(params)
