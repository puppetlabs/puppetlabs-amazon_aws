...
def operation_with_hash(arg_hash)
  ...
  client.update_conference_provider(call_hash)
end

def update_conference_provider(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.update_conference_provider if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_conference_provider(params)
