...
def operation_with_hash(arg_hash)
  ...
  client.deregister_task_definition(call_hash)
end

def deregister_task_definition(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.deregister_task_definition if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = deregister_task_definition(params)
