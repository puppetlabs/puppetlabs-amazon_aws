...
def operation_with_hash(arg_hash)
  ...
  client.delete_spot_datafeed_subscription(call_hash)
end

def delete_spot_datafeed_subscription(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.delete_spot_datafeed_subscription if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_spot_datafeed_subscription(params)
