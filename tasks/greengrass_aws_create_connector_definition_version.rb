...
def operation_with_hash(arg_hash)
  ...
  client.create_connector_definition_version(call_hash)
end

def create_connector_definition_version(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.create_connector_definition_version if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_connector_definition_version(params)
