...
def operation_with_hash(arg_hash)
  ...
  client.start_content_moderation(call_hash)
end

def start_content_moderation(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Rekognition::Client.new(region: region)
  response = client.start_content_moderation if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_content_moderation(params)
