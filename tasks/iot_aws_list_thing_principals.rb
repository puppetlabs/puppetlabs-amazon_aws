...
def operation_with_hash(arg_hash)
  ...
  client.list_thing_principals(call_hash)
end

def list_thing_principals(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.list_thing_principals if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_thing_principals(params)
