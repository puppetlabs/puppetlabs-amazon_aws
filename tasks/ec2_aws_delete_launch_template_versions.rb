...
def operation_with_hash(arg_hash)
  ...
  client.delete_launch_template_versions(call_hash)
end

def delete_launch_template_versions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.delete_launch_template_versions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_launch_template_versions(params)
