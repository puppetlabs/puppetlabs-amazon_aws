...
def operation_with_hash(arg_hash)
  ...
  client.delete_entity_recognizer(call_hash)
end

def delete_entity_recognizer(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Comprehend::Client.new(region: region)
  response = client.delete_entity_recognizer if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_entity_recognizer(params)
