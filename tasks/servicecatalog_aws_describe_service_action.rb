...
def operation_with_hash(arg_hash)
  ...
  client.describe_service_action(call_hash)
end

def describe_service_action(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.describe_service_action if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_service_action(params)
