...
def operation_with_hash(arg_hash)
  ...
  client.describe_working_storage(call_hash)
end

def describe_working_storage(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.describe_working_storage if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_working_storage(params)
