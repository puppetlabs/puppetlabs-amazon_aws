...
def operation_with_hash(arg_hash)
  ...
  client.describe_game_sessions(call_hash)
end

def describe_game_sessions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.describe_game_sessions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_game_sessions(params)
