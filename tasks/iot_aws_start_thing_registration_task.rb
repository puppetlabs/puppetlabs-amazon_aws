...
def operation_with_hash(arg_hash)
  ...
  client.start_thing_registration_task(call_hash)
end

def start_thing_registration_task(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.start_thing_registration_task if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_thing_registration_task(params)
