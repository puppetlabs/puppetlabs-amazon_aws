...
def operation_with_hash(arg_hash)
  ...
  client.describe_reserved_instances_listings(call_hash)
end

def describe_reserved_instances_listings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_reserved_instances_listings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_reserved_instances_listings(params)
