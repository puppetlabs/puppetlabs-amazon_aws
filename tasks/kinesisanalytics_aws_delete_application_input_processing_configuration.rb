...
def DeleteApplicationInputProcessingConfiguration(*args)
  puts "Calling task operation DeleteApplicationInputProcessingConfiguration" 
  ...
  client = Aws::KinesisAnalytics::Client.new(region: get_region)
  ...
  if arg_hash.empty?
    ...
    response = client.delete_application_input_processing_configuration(call_hash)
  end
...
begin
  result = DeleteApplicationInputProcessingConfiguration(params)
  puts JSON.pretty_generate(result)
  exit 0
rescue Aws::KinesisAnalytics::Errors::ServiceError => e
