...
def operation_with_hash(arg_hash)
  ...
  client.describe_role_alias(call_hash)
end

def describe_role_alias(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.describe_role_alias if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_role_alias(params)
