...
def operation_with_hash(arg_hash)
  ...
  client.add_role_to_db_cluster(call_hash)
end

def add_role_to_db_cluster(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Neptune::Client.new(region: region)
  response = client.add_role_to_db_cluster if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_role_to_db_cluster(params)
