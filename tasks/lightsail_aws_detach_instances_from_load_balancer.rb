...
def operation_with_hash(arg_hash)
  ...
  client.detach_instances_from_load_balancer(call_hash)
end

def detach_instances_from_load_balancer(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.detach_instances_from_load_balancer if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = detach_instances_from_load_balancer(params)
