...
def operation_with_hash(arg_hash)
  ...
  client.get_aggregate_discovered_resource_counts(call_hash)
end

def get_aggregate_discovered_resource_counts(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.get_aggregate_discovered_resource_counts if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_aggregate_discovered_resource_counts(params)
