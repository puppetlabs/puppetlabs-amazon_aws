...
def operation_with_hash(arg_hash)
  ...
  client.update_container_agent(call_hash)
end

def update_container_agent(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.update_container_agent if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_container_agent(params)
