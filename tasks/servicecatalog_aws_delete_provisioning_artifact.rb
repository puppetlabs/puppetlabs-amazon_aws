...
def operation_with_hash(arg_hash)
  ...
  client.delete_provisioning_artifact(call_hash)
end

def delete_provisioning_artifact(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.delete_provisioning_artifact if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_provisioning_artifact(params)
