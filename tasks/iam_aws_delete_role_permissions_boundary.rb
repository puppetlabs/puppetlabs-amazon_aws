...
def operation_with_hash(arg_hash)
  ...
  client.delete_role_permissions_boundary(call_hash)
end

def delete_role_permissions_boundary(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.delete_role_permissions_boundary if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_role_permissions_boundary(params)
