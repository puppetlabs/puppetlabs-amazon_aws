...
def operation_with_hash(arg_hash)
  ...
  client.list_origin_endpoints(call_hash)
end

def list_origin_endpoints(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaPackage::Client.new(region: region)
  response = client.list_origin_endpoints if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_origin_endpoints(params)
