...
def operation_with_hash(arg_hash)
  ...
  client.create_transit_gateway_vpc_attachment(call_hash)
end

def create_transit_gateway_vpc_attachment(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_transit_gateway_vpc_attachment if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_transit_gateway_vpc_attachment(params)
