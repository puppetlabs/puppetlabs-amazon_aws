...
def operation_with_hash(arg_hash)
  ...
  client.delete_function_concurrency(call_hash)
end

def delete_function_concurrency(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lambda::Client.new(region: region)
  response = client.delete_function_concurrency if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_function_concurrency(params)
