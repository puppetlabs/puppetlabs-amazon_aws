...
def operation_with_hash(arg_hash)
  ...
  client.get_resource_config_history(call_hash)
end

def get_resource_config_history(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.get_resource_config_history if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_resource_config_history(params)
