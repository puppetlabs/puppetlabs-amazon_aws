...
def operation_with_hash(arg_hash)
  ...
  client.get_aws_organizations_access_status(call_hash)
end

def get_aws_organizations_access_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.get_aws_organizations_access_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_aws_organizations_access_status(params)
