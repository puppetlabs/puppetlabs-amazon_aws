...
def operation_with_hash(arg_hash)
  ...
  client.describe_task_definition(call_hash)
end

def describe_task_definition(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.describe_task_definition if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_task_definition(params)
