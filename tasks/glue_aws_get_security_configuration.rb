...
def operation_with_hash(arg_hash)
  ...
  client.get_security_configuration(call_hash)
end

def get_security_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glue::Client.new(region: region)
  response = client.get_security_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_security_configuration(params)
