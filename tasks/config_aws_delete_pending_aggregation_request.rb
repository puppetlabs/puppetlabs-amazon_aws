...
def operation_with_hash(arg_hash)
  ...
  client.delete_pending_aggregation_request(call_hash)
end

def delete_pending_aggregation_request(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.delete_pending_aggregation_request if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_pending_aggregation_request(params)
