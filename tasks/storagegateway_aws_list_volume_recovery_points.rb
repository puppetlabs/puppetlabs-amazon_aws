...
def operation_with_hash(arg_hash)
  ...
  client.list_volume_recovery_points(call_hash)
end

def list_volume_recovery_points(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.list_volume_recovery_points if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_volume_recovery_points(params)
