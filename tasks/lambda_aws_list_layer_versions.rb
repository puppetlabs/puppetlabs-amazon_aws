...
def operation_with_hash(arg_hash)
  ...
  client.list_layer_versions(call_hash)
end

def list_layer_versions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lambda::Client.new(region: region)
  response = client.list_layer_versions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_layer_versions(params)
