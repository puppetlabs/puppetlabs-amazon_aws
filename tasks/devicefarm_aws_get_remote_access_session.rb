...
def operation_with_hash(arg_hash)
  ...
  client.get_remote_access_session(call_hash)
end

def get_remote_access_session(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.get_remote_access_session if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_remote_access_session(params)
