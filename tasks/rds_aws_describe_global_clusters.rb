...
def operation_with_hash(arg_hash)
  ...
  client.describe_global_clusters(call_hash)
end

def describe_global_clusters(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.describe_global_clusters if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_global_clusters(params)
