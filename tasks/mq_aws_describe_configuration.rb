...
def operation_with_hash(arg_hash)
  ...
  client.describe_configuration(call_hash)
end

def describe_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MQ::Client.new(region: region)
  response = client.describe_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_configuration(params)
