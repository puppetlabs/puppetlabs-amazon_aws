...
def operation_with_hash(arg_hash)
  ...
  client.create_virtual_mfa_device(call_hash)
end

def create_virtual_mfa_device(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.create_virtual_mfa_device if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_virtual_mfa_device(params)
