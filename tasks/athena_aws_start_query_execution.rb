...
def operation_with_hash(arg_hash)
  ...
  client.start_query_execution(call_hash)
end

def start_query_execution(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Athena::Client.new(region: region)
  response = client.start_query_execution if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_query_execution(params)
