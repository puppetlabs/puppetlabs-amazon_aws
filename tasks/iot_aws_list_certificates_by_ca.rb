...
def operation_with_hash(arg_hash)
  ...
  client.list_certificates_by_ca(call_hash)
end

def list_certificates_by_ca(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.list_certificates_by_ca if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_certificates_by_ca(params)
