...
def operation_with_hash(arg_hash)
  ...
  client.describe_virtual_interfaces(call_hash)
end

def describe_virtual_interfaces(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.describe_virtual_interfaces if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_virtual_interfaces(params)
