...
def operation_with_hash(arg_hash)
  ...
  client.delete_user_endpoints(call_hash)
end

def delete_user_endpoints(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Pinpoint::Client.new(region: region)
  response = client.delete_user_endpoints if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_user_endpoints(params)
