...
def operation_with_hash(arg_hash)
  ...
  client.get_account_authorization_details(call_hash)
end

def get_account_authorization_details(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.get_account_authorization_details if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_account_authorization_details(params)
