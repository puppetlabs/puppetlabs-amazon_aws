...
def operation_with_hash(arg_hash)
  ...
  client.change_message_visibility(call_hash)
end

def change_message_visibility(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SQS::Client.new(region: region)
  response = client.change_message_visibility if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = change_message_visibility(params)
