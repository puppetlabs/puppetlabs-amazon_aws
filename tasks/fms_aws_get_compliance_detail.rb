...
def operation_with_hash(arg_hash)
  ...
  client.get_compliance_detail(call_hash)
end

def get_compliance_detail(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::FMS::Client.new(region: region)
  response = client.get_compliance_detail if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_compliance_detail(params)
