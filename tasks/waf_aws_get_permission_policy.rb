...
def operation_with_hash(arg_hash)
  ...
  client.get_permission_policy(call_hash)
end

def get_permission_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.get_permission_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_permission_policy(params)
