...
def operation_with_hash(arg_hash)
  ...
  client.describe_organizational_unit(call_hash)
end

def describe_organizational_unit(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Organizations::Client.new(region: region)
  response = client.describe_organizational_unit if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_organizational_unit(params)
