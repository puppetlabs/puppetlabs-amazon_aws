...
def operation_with_hash(arg_hash)
  ...
  client.describe_application(call_hash)
end

def describe_application(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KinesisAnalytics::Client.new(region: region)
  response = client.describe_application if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_application(params)
