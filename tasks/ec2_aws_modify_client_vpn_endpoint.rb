...
def operation_with_hash(arg_hash)
  ...
  client.modify_client_vpn_endpoint(call_hash)
end

def modify_client_vpn_endpoint(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.modify_client_vpn_endpoint if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_client_vpn_endpoint(params)
