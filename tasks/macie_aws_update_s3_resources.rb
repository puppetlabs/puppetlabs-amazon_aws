...
def operation_with_hash(arg_hash)
  ...
  client.update_s3_resources(call_hash)
end

def update_s3_resources(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Macie::Client.new(region: region)
  response = client.update_s3_resources if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_s3_resources(params)
