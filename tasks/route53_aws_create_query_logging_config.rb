...
def operation_with_hash(arg_hash)
  ...
  client.create_query_logging_config(call_hash)
end

def create_query_logging_config(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.create_query_logging_config if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_query_logging_config(params)
