...
def operation_with_hash(arg_hash)
  ...
  client.update_scheduled_audit(call_hash)
end

def update_scheduled_audit(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.update_scheduled_audit if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_scheduled_audit(params)
