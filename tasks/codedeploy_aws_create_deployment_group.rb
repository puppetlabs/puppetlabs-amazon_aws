...
def operation_with_hash(arg_hash)
  ...
  client.create_deployment_group(call_hash)
end

def create_deployment_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeDeploy::Client.new(region: region)
  response = client.create_deployment_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_deployment_group(params)
