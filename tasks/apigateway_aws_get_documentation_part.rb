...
def operation_with_hash(arg_hash)
  ...
  client.get_documentation_part(call_hash)
end

def get_documentation_part(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.get_documentation_part if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_documentation_part(params)
