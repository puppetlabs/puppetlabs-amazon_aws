...
def operation_with_hash(arg_hash)
  ...
  client.describe_custom_key_stores(call_hash)
end

def describe_custom_key_stores(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KMS::Client.new(region: region)
  response = client.describe_custom_key_stores if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_custom_key_stores(params)
