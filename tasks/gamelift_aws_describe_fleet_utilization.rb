...
def operation_with_hash(arg_hash)
  ...
  client.describe_fleet_utilization(call_hash)
end

def describe_fleet_utilization(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.describe_fleet_utilization if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_fleet_utilization(params)
