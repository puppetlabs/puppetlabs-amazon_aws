...
def operation_with_hash(arg_hash)
  ...
  client.describe_replication_groups(call_hash)
end

def describe_replication_groups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElastiCache::Client.new(region: region)
  response = client.describe_replication_groups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_replication_groups(params)
