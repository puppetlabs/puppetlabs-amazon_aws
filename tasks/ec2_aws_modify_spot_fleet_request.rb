...
def operation_with_hash(arg_hash)
  ...
  client.modify_spot_fleet_request(call_hash)
end

def modify_spot_fleet_request(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.modify_spot_fleet_request if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_spot_fleet_request(params)
