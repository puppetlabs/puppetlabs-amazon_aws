...
def operation_with_hash(arg_hash)
  ...
  client = Aws::XRay::Client.new(region: region)
  client.put_telemetry_records(call_hash)
end

def put_telemetry_records(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::XRay::Client.new(region: region)
  response = client.put_telemetry_records if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_telemetry_records(params)
