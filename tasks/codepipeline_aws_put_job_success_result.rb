...
def operation_with_hash(arg_hash)
  ...
  client.put_job_success_result(call_hash)
end

def put_job_success_result(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodePipeline::Client.new(region: region)
  response = client.put_job_success_result if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_job_success_result(params)
