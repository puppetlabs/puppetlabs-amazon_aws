...
def operation_with_hash(arg_hash)
  ...
  client.add_resource_permissions(call_hash)
end

def add_resource_permissions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.add_resource_permissions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_resource_permissions(params)
