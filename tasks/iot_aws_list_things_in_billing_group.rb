...
def operation_with_hash(arg_hash)
  ...
  client.list_things_in_billing_group(call_hash)
end

def list_things_in_billing_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.list_things_in_billing_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_things_in_billing_group(params)
