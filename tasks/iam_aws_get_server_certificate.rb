...
def operation_with_hash(arg_hash)
  ...
  client.get_server_certificate(call_hash)
end

def get_server_certificate(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.get_server_certificate if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_server_certificate(params)
