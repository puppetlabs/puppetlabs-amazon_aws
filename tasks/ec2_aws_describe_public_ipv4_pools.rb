...
def operation_with_hash(arg_hash)
  ...
  client.describe_public_ipv4_pools(call_hash)
end

def describe_public_ipv4_pools(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_public_ipv4_pools if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_public_ipv4_pools(params)
