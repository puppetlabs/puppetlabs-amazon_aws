...
def operation_with_hash(arg_hash)
  ...
  client.describe_key_phrases_detection_job(call_hash)
end

def describe_key_phrases_detection_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Comprehend::Client.new(region: region)
  response = client.describe_key_phrases_detection_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_key_phrases_detection_job(params)
