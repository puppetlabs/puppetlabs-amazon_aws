...
def operation_with_hash(arg_hash)
  ...
  client.describe_orderable_db_instance_options(call_hash)
end

def describe_orderable_db_instance_options(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Neptune::Client.new(region: region)
  response = client.describe_orderable_db_instance_options if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_orderable_db_instance_options(params)
