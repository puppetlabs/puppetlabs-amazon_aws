...
def operation_with_hash(arg_hash)
  ...
  client.put_notification_channel(call_hash)
end

def put_notification_channel(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::FMS::Client.new(region: region)
  response = client.put_notification_channel if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_notification_channel(params)
