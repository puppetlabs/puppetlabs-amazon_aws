...
def operation_with_hash(arg_hash)
  ...
  client.attach_internet_gateway(call_hash)
end

def attach_internet_gateway(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.attach_internet_gateway if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = attach_internet_gateway(params)
