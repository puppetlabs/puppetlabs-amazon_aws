...
def operation_with_hash(arg_hash)
  ...
  client.start_configuration_recorder(call_hash)
end

def start_configuration_recorder(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.start_configuration_recorder if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_configuration_recorder(params)
