...
def operation_with_hash(arg_hash)
  ...
  client.get_telemetry_metadata(call_hash)
end

def get_telemetry_metadata(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.get_telemetry_metadata if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_telemetry_metadata(params)
