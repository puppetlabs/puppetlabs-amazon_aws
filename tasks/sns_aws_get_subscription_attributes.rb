...
def operation_with_hash(arg_hash)
  ...
  client.get_subscription_attributes(call_hash)
end

def get_subscription_attributes(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SNS::Client.new(region: region)
  response = client.get_subscription_attributes if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_subscription_attributes(params)
