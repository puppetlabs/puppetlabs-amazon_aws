...
def operation_with_hash(arg_hash)
  ...
  client.create_job_template(call_hash)
end

def create_job_template(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaConvert::Client.new(region: region)
  response = client.create_job_template if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_job_template(params)
