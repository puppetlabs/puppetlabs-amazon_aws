...
def operation_with_hash(arg_hash)
  ...
  client.list_tags_for_vault(call_hash)
end

def list_tags_for_vault(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glacier::Client.new(region: region)
  response = client.list_tags_for_vault if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_tags_for_vault(params)
