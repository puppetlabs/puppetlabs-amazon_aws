...
def operation_with_hash(arg_hash)
  ...
  client.modify_vpc_peering_connection_options(call_hash)
end

def modify_vpc_peering_connection_options(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.modify_vpc_peering_connection_options if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_vpc_peering_connection_options(params)
