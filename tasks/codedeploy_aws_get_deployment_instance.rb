...
def operation_with_hash(arg_hash)
  ...
  client.get_deployment_instance(call_hash)
end

def get_deployment_instance(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeDeploy::Client.new(region: region)
  response = client.get_deployment_instance if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_deployment_instance(params)
