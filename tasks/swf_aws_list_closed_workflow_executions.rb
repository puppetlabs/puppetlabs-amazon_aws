...
def operation_with_hash(arg_hash)
  ...
  client.list_closed_workflow_executions(call_hash)
end

def list_closed_workflow_executions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SWF::Client.new(region: region)
  response = client.list_closed_workflow_executions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_closed_workflow_executions(params)
