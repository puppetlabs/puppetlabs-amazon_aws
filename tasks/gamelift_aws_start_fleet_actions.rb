...
def operation_with_hash(arg_hash)
  ...
  client.start_fleet_actions(call_hash)
end

def start_fleet_actions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.start_fleet_actions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_fleet_actions(params)
