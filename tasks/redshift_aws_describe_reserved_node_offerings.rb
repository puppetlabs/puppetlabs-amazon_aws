...
def operation_with_hash(arg_hash)
  ...
  client.describe_reserved_node_offerings(call_hash)
end

def describe_reserved_node_offerings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.describe_reserved_node_offerings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_reserved_node_offerings(params)
