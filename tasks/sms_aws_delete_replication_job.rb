...
def operation_with_hash(arg_hash)
  ...
  client.delete_replication_job(call_hash)
end

def delete_replication_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SMS::Client.new(region: region)
  response = client.delete_replication_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_replication_job(params)
