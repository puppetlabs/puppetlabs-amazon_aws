...
def operation_with_hash(arg_hash)
  ...
  client.get_event_source_mapping(call_hash)
end

def get_event_source_mapping(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lambda::Client.new(region: region)
  response = client.get_event_source_mapping if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_event_source_mapping(params)
