...
def operation_with_hash(arg_hash)
  ...
  client.update_ssh_public_key(call_hash)
end

def update_ssh_public_key(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.update_ssh_public_key if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_ssh_public_key(params)
