...
def operation_with_hash(arg_hash)
  ...
  client.delete_skill_authorization(call_hash)
end

def delete_skill_authorization(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.delete_skill_authorization if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_skill_authorization(params)
