...
def operation_with_hash(arg_hash)
  ...
  client.start_bulk_deployment(call_hash)
end

def start_bulk_deployment(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.start_bulk_deployment if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_bulk_deployment(params)
