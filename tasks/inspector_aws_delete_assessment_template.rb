...
def operation_with_hash(arg_hash)
  ...
  client.delete_assessment_template(call_hash)
end

def delete_assessment_template(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.delete_assessment_template if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_assessment_template(params)
