...
def operation_with_hash(arg_hash)
  ...
  client.update_account_password_policy(call_hash)
end
...
def update_account_password_policy(*args)
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.update_account_password_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_account_password_policy(params)
