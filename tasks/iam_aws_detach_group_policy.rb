...
def operation_with_hash(arg_hash)
  ...
  client.detach_group_policy(call_hash)
end

def detach_group_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.detach_group_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = detach_group_policy(params)
