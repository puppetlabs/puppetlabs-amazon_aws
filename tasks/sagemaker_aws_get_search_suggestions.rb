...
def operation_with_hash(arg_hash)
  ...
  client.get_search_suggestions(call_hash)
end

def get_search_suggestions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.get_search_suggestions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_search_suggestions(params)
