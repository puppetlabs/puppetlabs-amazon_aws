...
def operation_with_hash(arg_hash)
  ...
  client.list_dataset_contents(call_hash)
end

def list_dataset_contents(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoTAnalytics::Client.new(region: region)
  response = client.list_dataset_contents if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_dataset_contents(params)
