...
def operation_with_hash(arg_hash)
  ...
  client.get_dataset_content(call_hash)
end

def get_dataset_content(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoTAnalytics::Client.new(region: region)
  response = client.get_dataset_content if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_dataset_content(params)
