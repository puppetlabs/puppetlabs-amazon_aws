...
def operation_with_hash(arg_hash)
  ...
  client.create_reserved_instances_listing(call_hash)
end

def create_reserved_instances_listing(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_reserved_instances_listing if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_reserved_instances_listing(params)
