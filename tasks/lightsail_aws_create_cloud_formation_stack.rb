...
def operation_with_hash(arg_hash)
  ...
  client.create_cloud_formation_stack(call_hash)
end

def create_cloud_formation_stack(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.create_cloud_formation_stack if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_cloud_formation_stack(params)
