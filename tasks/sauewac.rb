...
def operation_with_hash(arg_hash)
  ...
  client.update_endpoint_weights_and_capacities(call_hash)
end

def update_endpoint_weights_and_capacities(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.update_endpoint_weights_and_capacities if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_endpoint_weights_and_capacities(params)
