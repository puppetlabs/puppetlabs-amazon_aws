...
def operation_with_hash(arg_hash)
  ...
  client.delete_scaling_policy(call_hash)
end

def delete_scaling_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.delete_scaling_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_scaling_policy(params)
