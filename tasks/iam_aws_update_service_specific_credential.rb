...
def operation_with_hash(arg_hash)
  ...
  client.update_service_specific_credential(call_hash)
end

def update_service_specific_credential(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.update_service_specific_credential if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_service_specific_credential(params)
