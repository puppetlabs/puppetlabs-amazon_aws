...
def operation_with_hash(arg_hash)
  ...
  client.get_findings_statistics(call_hash)
end

def get_findings_statistics(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GuardDuty::Client.new(region: region)
  response = client.get_findings_statistics if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_findings_statistics(params)
