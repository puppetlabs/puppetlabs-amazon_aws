...
def operation_with_hash(arg_hash)
  ...
  client.create_portfolio_share(call_hash)
end

def create_portfolio_share(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.create_portfolio_share if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_portfolio_share(params)
