...
def operation_with_hash(arg_hash)
  ...
  client.describe_volumes_modifications(call_hash)
end

def describe_volumes_modifications(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_volumes_modifications if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_volumes_modifications(params)
