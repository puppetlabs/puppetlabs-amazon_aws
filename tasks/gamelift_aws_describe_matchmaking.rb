...
def operation_with_hash(arg_hash)
  ...
  client.describe_matchmaking(call_hash)
end

def describe_matchmaking(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.describe_matchmaking if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_matchmaking(params)
