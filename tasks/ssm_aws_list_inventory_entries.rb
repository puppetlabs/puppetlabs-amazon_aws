...
def operation_with_hash(arg_hash)
  ...
  client.list_inventory_entries(call_hash)
end

def list_inventory_entries(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.list_inventory_entries if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_inventory_entries(params)
