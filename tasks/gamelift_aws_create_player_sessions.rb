...
def operation_with_hash(arg_hash)
  ...
  client.create_player_sessions(call_hash)
end

def create_player_sessions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.create_player_sessions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_player_sessions(params)
