...
def operation_with_hash(arg_hash)
  ...
  client.describe_environment_resources(call_hash)
end

def describe_environment_resources(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.describe_environment_resources if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_environment_resources(params)
