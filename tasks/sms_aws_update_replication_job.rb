...
def operation_with_hash(arg_hash)
  ...
  client.update_replication_job(call_hash)
end

def update_replication_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SMS::Client.new(region: region)
  response = client.update_replication_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_replication_job(params)
