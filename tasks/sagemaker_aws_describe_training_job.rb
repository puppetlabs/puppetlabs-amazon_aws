...
def operation_with_hash(arg_hash)
  ...
  client.describe_training_job(call_hash)
end

def describe_training_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.describe_training_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_training_job(params)
