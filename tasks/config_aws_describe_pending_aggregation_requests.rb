...
def operation_with_hash(arg_hash)
  ...
  client.describe_pending_aggregation_requests(call_hash)
end

def describe_pending_aggregation_requests(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.describe_pending_aggregation_requests if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_pending_aggregation_requests(params)
