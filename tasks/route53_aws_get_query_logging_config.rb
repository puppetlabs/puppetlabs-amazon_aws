...
def operation_with_hash(arg_hash)
  ...
  client.get_query_logging_config(call_hash)
end

def get_query_logging_config(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.get_query_logging_config if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_query_logging_config(params)
