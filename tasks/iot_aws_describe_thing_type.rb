...
def operation_with_hash(arg_hash)
  ...
  client.describe_thing_type(call_hash)
end

def describe_thing_type(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.describe_thing_type if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_thing_type(params)
