...
def operation_with_hash(arg_hash)
  ...
  client.create_exclusions_preview(call_hash)
end

def create_exclusions_preview(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.create_exclusions_preview if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_exclusions_preview(params)
