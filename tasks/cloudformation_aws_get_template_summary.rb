...
def operation_with_hash(arg_hash)
  ...
  client.get_template_summary(call_hash)
end

def get_template_summary(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudFormation::Client.new(region: region)
  response = client.get_template_summary if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_template_summary(params)
