...
def operation_with_hash(arg_hash)
  ...
  client.delete_dev_endpoint(call_hash)
end

def delete_dev_endpoint(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glue::Client.new(region: region)
  response = client.delete_dev_endpoint if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_dev_endpoint(params)
