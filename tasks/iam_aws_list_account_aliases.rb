...
def operation_with_hash(arg_hash)
  ...
  client.list_account_aliases(call_hash)
end

def list_account_aliases(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.list_account_aliases if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_account_aliases(params)
