...
def operation_with_hash(arg_hash)
  ...
  client.delete_vpc_association_authorization(call_hash)
end

def delete_vpc_association_authorization(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.delete_vpc_association_authorization if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_vpc_association_authorization(params)
