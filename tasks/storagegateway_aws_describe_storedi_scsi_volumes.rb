...
def operation_with_hash(arg_hash)
  ...
  client.describe_storedi_scsi_volumes(call_hash)
end

def describe_storedi_scsi_volumes(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.describe_storedi_scsi_volumes if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_storedi_scsi_volumes(params)
