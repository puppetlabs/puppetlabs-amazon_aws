...
def operation_with_hash(arg_hash)
  ...
  client.update_time_to_live(call_hash)
end

def update_time_to_live(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DynamoDB::Client.new(region: region)
  response = client.update_time_to_live if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_time_to_live(params)
