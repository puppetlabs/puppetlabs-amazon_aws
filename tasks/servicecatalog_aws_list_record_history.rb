...
def operation_with_hash(arg_hash)
  ...
  client.list_record_history(call_hash)
end

def list_record_history(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.list_record_history if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_record_history(params)
