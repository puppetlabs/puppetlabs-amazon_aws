...
def operation_with_hash(arg_hash)
  ...
  client.detach_classic_link_vpc(call_hash)
end

def detach_classic_link_vpc(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.detach_classic_link_vpc if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = detach_classic_link_vpc(params)
