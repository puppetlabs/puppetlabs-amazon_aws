...
def operation_with_hash(arg_hash)
  ...
  client = Aws::MediaLive::Client.new(region: region)
  client.describe_input_security_group(call_hash)
end

def describe_input_security_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaLive::Client.new(region: region)
  response = client.describe_input_security_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_input_security_group(params)
