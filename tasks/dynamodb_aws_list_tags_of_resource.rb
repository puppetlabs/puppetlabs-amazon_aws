...
def operation_with_hash(arg_hash)
  ...
  client.list_tags_of_resource(call_hash)
end

def list_tags_of_resource(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DynamoDB::Client.new(region: region)
  response = client.list_tags_of_resource if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_tags_of_resource(params)
