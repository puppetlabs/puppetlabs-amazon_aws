...
def operation_with_hash(arg_hash)
  ...
  client.put_pipeline_definition(call_hash)
end

def put_pipeline_definition(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DataPipeline::Client.new(region: region)
  response = client.put_pipeline_definition if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_pipeline_definition(params)
