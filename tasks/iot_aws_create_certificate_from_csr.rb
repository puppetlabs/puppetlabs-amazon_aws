...
def operation_with_hash(arg_hash)
  ...
  client.create_certificate_from_csr(call_hash)
end

def create_certificate_from_csr(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.create_certificate_from_csr if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_certificate_from_csr(params)
