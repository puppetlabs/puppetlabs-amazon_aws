...
def operation_with_hash(arg_hash)
  ...
  client.describe_classic_link_instances(call_hash)
end

def describe_classic_link_instances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_classic_link_instances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_classic_link_instances(params)
