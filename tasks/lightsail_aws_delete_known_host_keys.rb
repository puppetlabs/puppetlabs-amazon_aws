...
def operation_with_hash(arg_hash)
  ...
  client.delete_known_host_keys(call_hash)
end

def delete_known_host_keys(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.delete_known_host_keys if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_known_host_keys(params)
