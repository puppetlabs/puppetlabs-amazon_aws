...
def operation_with_hash(arg_hash)
  ...
  client.create_hyper_parameter_tuning_job(call_hash)
end

def create_hyper_parameter_tuning_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.create_hyper_parameter_tuning_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_hyper_parameter_tuning_job(params)
