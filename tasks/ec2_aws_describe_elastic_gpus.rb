...
def operation_with_hash(arg_hash)
  ...
  client.describe_elastic_gpus(call_hash)
end

def describe_elastic_gpus(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_elastic_gpus if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_elastic_gpus(params)
