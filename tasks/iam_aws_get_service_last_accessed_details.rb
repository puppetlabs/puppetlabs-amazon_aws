...
def operation_with_hash(arg_hash)
  ...
  client.get_service_last_accessed_details(call_hash)
end

def get_service_last_accessed_details(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.get_service_last_accessed_details if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_service_last_accessed_details(params)
