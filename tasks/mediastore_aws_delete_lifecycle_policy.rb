...
def operation_with_hash(arg_hash)
  ...
  client = Aws::MediaStore::Client.new(region: region)
  client.delete_lifecycle_policy(call_hash)
end

def delete_lifecycle_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaStore::Client.new(region: region)
  response = client.delete_lifecycle_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_lifecycle_policy(params)
