...
def operation_with_hash(arg_hash)
  ...
  client.describe_notification_configurations(call_hash)
end

def describe_notification_configurations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.describe_notification_configurations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_notification_configurations(params)
