...
def operation_with_hash(arg_hash)
  ...
  client.put_logging_configuration(call_hash)
end

def put_logging_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.put_logging_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_logging_configuration(params)
