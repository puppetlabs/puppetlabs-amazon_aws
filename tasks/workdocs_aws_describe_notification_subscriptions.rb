...
def operation_with_hash(arg_hash)
  ...
  client.describe_notification_subscriptions(call_hash)
end

def describe_notification_subscriptions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.describe_notification_subscriptions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_notification_subscriptions(params)
