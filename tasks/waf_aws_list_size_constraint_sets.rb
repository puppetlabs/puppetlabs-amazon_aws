...
def operation_with_hash(arg_hash)
  ...
  client.list_size_constraint_sets(call_hash)
end

def list_size_constraint_sets(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.list_size_constraint_sets if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_size_constraint_sets(params)
