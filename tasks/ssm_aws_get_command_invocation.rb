...
def operation_with_hash(arg_hash)
  ...
  client.get_command_invocation(call_hash)
end

def get_command_invocation(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.get_command_invocation if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_command_invocation(params)
