...
def operation_with_hash(arg_hash)
  ...
  client.describe_transit_gateways(call_hash)
end

def describe_transit_gateways(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_transit_gateways if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_transit_gateways(params)
