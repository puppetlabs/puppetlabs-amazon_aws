...
def operation_with_hash(arg_hash)
  ...
  client.start_transcription_job(call_hash)
end

def start_transcription_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::TranscribeService::Client.new(region: region)
  response = client.start_transcription_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_transcription_job(params)
