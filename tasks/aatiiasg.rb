...
def operation_with_hash(arg_hash)
  ...
  client.terminate_instance_in_auto_scaling_group(call_hash)
end

def terminate_instance_in_auto_scaling_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.terminate_instance_in_auto_scaling_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = terminate_instance_in_auto_scaling_group(params)
