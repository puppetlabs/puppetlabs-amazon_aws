...
def operation_with_hash(arg_hash)
  ...
  client.get_vpce_configuration(call_hash)
end

def get_vpce_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.get_vpce_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_vpce_configuration(params)
