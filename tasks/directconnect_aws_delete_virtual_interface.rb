...
def operation_with_hash(arg_hash)
  ...
  client.delete_virtual_interface(call_hash)
end

def delete_virtual_interface(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.delete_virtual_interface if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_virtual_interface(params)
