...
def operation_with_hash(arg_hash)
  ...
  client.start_associations_once(call_hash)
end

def start_associations_once(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.start_associations_once if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_associations_once(params)
