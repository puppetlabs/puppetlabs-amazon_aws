...
def operation_with_hash(arg_hash)
  ...
  client.put_skill_authorization(call_hash)
end

def put_skill_authorization(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.put_skill_authorization if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_skill_authorization(params)
