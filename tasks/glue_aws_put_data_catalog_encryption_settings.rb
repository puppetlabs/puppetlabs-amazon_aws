...
def operation_with_hash(arg_hash)
  ...
  client.put_data_catalog_encryption_settings(call_hash)
end

def put_data_catalog_encryption_settings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glue::Client.new(region: region)
  response = client.put_data_catalog_encryption_settings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_data_catalog_encryption_settings(params)
