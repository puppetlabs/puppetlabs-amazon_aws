...
def operation_with_hash(arg_hash)
  ...
  client.list_multipart_uploads(call_hash)
end

def list_multipart_uploads(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glacier::Client.new(region: region)
  response = client.list_multipart_uploads if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_multipart_uploads(params)
