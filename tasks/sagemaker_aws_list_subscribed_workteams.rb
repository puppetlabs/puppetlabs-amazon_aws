...
def operation_with_hash(arg_hash)
  ...
  client.list_subscribed_workteams(call_hash)
end

def list_subscribed_workteams(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.list_subscribed_workteams if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_subscribed_workteams(params)
