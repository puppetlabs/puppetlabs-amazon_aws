...
def operation_with_hash(arg_hash)
  ...
  client.update_fleet_port_settings(call_hash)
end

def update_fleet_port_settings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.update_fleet_port_settings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_fleet_port_settings(params)
