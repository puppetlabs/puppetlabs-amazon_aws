...
def operation_with_hash(arg_hash)
  ...
  client.put_compliance_items(call_hash)
end

def put_compliance_items(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.put_compliance_items if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_compliance_items(params)
