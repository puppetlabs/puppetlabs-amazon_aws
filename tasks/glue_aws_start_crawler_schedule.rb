...
def operation_with_hash(arg_hash)
  ...
  client.start_crawler_schedule(call_hash)
end

def start_crawler_schedule(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glue::Client.new(region: region)
  response = client.start_crawler_schedule if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_crawler_schedule(params)
