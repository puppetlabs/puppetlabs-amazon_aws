...
def operation_with_hash(arg_hash)
  ...
  client.delete_permission_policy(call_hash)
end

def delete_permission_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.delete_permission_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_permission_policy(params)
