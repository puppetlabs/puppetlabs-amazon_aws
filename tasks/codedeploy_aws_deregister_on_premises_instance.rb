...
def operation_with_hash(arg_hash)
  ...
  client.deregister_on_premises_instance(call_hash)
end

def deregister_on_premises_instance(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeDeploy::Client.new(region: region)
  response = client.deregister_on_premises_instance if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = deregister_on_premises_instance(params)
