...
def operation_with_hash(arg_hash)
  ...
  client.create_network_interface_permission(call_hash)
end

def create_network_interface_permission(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_network_interface_permission if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_network_interface_permission(params)
