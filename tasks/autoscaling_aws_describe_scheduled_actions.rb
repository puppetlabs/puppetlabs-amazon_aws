...
def operation_with_hash(arg_hash)
  ...
  client.describe_scheduled_actions(call_hash)
end

def describe_scheduled_actions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.describe_scheduled_actions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_scheduled_actions(params)
