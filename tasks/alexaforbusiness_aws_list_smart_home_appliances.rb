...
def operation_with_hash(arg_hash)
  ...
  client.list_smart_home_appliances(call_hash)
end

def list_smart_home_appliances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.list_smart_home_appliances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_smart_home_appliances(params)
