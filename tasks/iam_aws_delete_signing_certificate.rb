...
def operation_with_hash(arg_hash)
  ...
  client.delete_signing_certificate(call_hash)
end

def delete_signing_certificate(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.delete_signing_certificate if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_signing_certificate(params)
