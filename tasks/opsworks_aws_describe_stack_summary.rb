...
def operation_with_hash(arg_hash)
  ...
  client.describe_stack_summary(call_hash)
end

def describe_stack_summary(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::OpsWorks::Client.new(region: region)
  response = client.describe_stack_summary if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_stack_summary(params)
