...
def operation_with_hash(arg_hash)
  ...
  client.get_room_skill_parameter(call_hash)
end

def get_room_skill_parameter(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.get_room_skill_parameter if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_room_skill_parameter(params)
