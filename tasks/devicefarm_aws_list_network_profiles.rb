...
def operation_with_hash(arg_hash)
  ...
  client.list_network_profiles(call_hash)
end

def list_network_profiles(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.list_network_profiles if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_network_profiles(params)
