...
def operation_with_hash(arg_hash)
  ...
  client.describe_hsm_configurations(call_hash)
end

def describe_hsm_configurations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.describe_hsm_configurations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_hsm_configurations(params)
