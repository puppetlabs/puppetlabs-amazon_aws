...
def operation_with_hash(arg_hash)
  ...
  client.generate_credential_report(call_hash)
end

def generate_credential_report(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.generate_credential_report if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = generate_credential_report(params)
