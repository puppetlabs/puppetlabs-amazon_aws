...
def operation_with_hash(arg_hash)
  ...
  client.list_document_classification_jobs(call_hash)
end

def list_document_classification_jobs(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Comprehend::Client.new(region: region)
  response = client.list_document_classification_jobs if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_document_classification_jobs(params)
