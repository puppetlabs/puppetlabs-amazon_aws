...
def operation_with_hash(arg_hash)
  ...
  client.delete_open_id_connect_provider(call_hash)
end

def delete_open_id_connect_provider(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.delete_open_id_connect_provider if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_open_id_connect_provider(params)
