...
def operation_with_hash(arg_hash)
  ...
  client.describe_virtual_gateways(call_hash)
end

def describe_virtual_gateways(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.describe_virtual_gateways if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_virtual_gateways(params)
