...
def operation_with_hash(arg_hash)
  ...
  client.list_container_instances(call_hash)
end

def list_container_instances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.list_container_instances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_container_instances(params)
