...
def operation_with_hash(arg_hash)
  ...
  client.get_pipeline_execution(call_hash)
end

def get_pipeline_execution(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodePipeline::Client.new(region: region)
  response = client.get_pipeline_execution if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_pipeline_execution(params)
