...
def operation_with_hash(arg_hash)
  ...
  client.describe_auto_scaling_groups(call_hash)
end

def describe_auto_scaling_groups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.describe_auto_scaling_groups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_auto_scaling_groups(params)
