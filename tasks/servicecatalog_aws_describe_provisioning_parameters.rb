...
def operation_with_hash(arg_hash)
  ...
  client.describe_provisioning_parameters(call_hash)
end

def describe_provisioning_parameters(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.describe_provisioning_parameters if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_provisioning_parameters(params)
