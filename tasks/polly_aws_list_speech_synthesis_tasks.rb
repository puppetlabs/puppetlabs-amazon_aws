...
def operation_with_hash(arg_hash)
  ...
  client.list_speech_synthesis_tasks(call_hash)
end

def list_speech_synthesis_tasks(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Polly::Client.new(region: region)
  response = client.list_speech_synthesis_tasks if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_speech_synthesis_tasks(params)
