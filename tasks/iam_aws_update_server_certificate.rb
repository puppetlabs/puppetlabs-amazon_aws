...
def operation_with_hash(arg_hash)
  ...
  client.update_server_certificate(call_hash)
end

def update_server_certificate(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.update_server_certificate if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_server_certificate(params)
