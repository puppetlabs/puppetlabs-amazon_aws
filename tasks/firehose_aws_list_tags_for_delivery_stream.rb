...
def operation_with_hash(arg_hash)
  ...
  client.list_tags_for_delivery_stream(call_hash)
end

def list_tags_for_delivery_stream(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Firehose::Client.new(region: region)
  response = client.list_tags_for_delivery_stream if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_tags_for_delivery_stream(params)
