...
def operation_with_hash(arg_hash)
  ...
  client.describe_fleet_capacity(call_hash)
end

def describe_fleet_capacity(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.describe_fleet_capacity if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_fleet_capacity(params)
