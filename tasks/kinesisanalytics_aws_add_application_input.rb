...
def operation_with_hash(arg_hash)
  ...
  client.add_application_input(call_hash)
end

def add_application_input(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KinesisAnalytics::Client.new(region: region)
  response = client.add_application_input if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_application_input(params)
