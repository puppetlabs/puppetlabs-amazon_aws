...
def operation_with_hash(arg_hash)
  ...
  client.describe_collection(call_hash)
end

def describe_collection(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Rekognition::Client.new(region: region)
  response = client.describe_collection if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_collection(params)
