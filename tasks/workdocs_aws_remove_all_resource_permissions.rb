...
def operation_with_hash(arg_hash)
  ...
  client.remove_all_resource_permissions(call_hash)
end

def remove_all_resource_permissions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.remove_all_resource_permissions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = remove_all_resource_permissions(params)
