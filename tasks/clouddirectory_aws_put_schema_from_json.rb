...
def operation_with_hash(arg_hash)
  ...
  client.put_schema_from_json(call_hash)
end

def put_schema_from_json(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudDirectory::Client.new(region: region)
  response = client.put_schema_from_json if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_schema_from_json(params)
