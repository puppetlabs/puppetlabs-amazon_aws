...
def operation_with_hash(arg_hash)
  ...
  client.list_compilation_jobs(call_hash)
end

def list_compilation_jobs(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.list_compilation_jobs if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_compilation_jobs(params)
