...
def operation_with_hash(arg_hash)
  ...
  client.list_object_children(call_hash)
end

def list_object_children(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudDirectory::Client.new(region: region)
  response = client.list_object_children if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_object_children(params)
