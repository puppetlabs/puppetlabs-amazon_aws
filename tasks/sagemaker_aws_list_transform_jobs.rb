...
def operation_with_hash(arg_hash)
  ...
  client.list_transform_jobs(call_hash)
end

def list_transform_jobs(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.list_transform_jobs if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_transform_jobs(params)
