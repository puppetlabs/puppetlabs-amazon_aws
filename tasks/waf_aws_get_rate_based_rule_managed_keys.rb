...
def operation_with_hash(arg_hash)
  ...
  client.get_rate_based_rule_managed_keys(call_hash)
end

def get_rate_based_rule_managed_keys(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.get_rate_based_rule_managed_keys if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_rate_based_rule_managed_keys(params)
