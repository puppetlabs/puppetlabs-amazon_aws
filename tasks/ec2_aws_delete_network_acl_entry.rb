...
def operation_with_hash(arg_hash)
  ...
  client.delete_network_acl_entry(call_hash)
end

def delete_network_acl_entry(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.delete_network_acl_entry if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_network_acl_entry(params)
