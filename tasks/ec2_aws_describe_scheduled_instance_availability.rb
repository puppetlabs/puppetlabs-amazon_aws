...
def operation_with_hash(arg_hash)
  ...
  client.describe_scheduled_instance_availability(call_hash)
end

def describe_scheduled_instance_availability(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_scheduled_instance_availability if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_scheduled_instance_availability(params)
