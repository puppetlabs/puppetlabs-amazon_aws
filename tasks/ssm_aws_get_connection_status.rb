...
def operation_with_hash(arg_hash)
  ...
  client.get_connection_status(call_hash)
end

def get_connection_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.get_connection_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_connection_status(params)
