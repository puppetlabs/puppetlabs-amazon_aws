...
def operation_with_hash(arg_hash)
  ...
  client.delete_repository_policy(call_hash)
end

def delete_repository_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECR::Client.new(region: region)
  response = client.delete_repository_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_repository_policy(params)
