...
def operation_with_hash(arg_hash)
  ...
  client = Aws::CloudDirectory::Client.new(region: region)
  client.list_object_parent_paths(call_hash)
end

def list_object_parent_paths(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudDirectory::Client.new(region: region)
  response = client.list_object_parent_paths if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_object_parent_paths(params)
