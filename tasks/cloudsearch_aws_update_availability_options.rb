...
def operation_with_hash(arg_hash)
  ...
  client.update_availability_options(call_hash)
end

def update_availability_options(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudSearch::Client.new(region: region)
  response = client.update_availability_options if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_availability_options(params)
