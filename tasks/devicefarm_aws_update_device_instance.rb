...
def operation_with_hash(arg_hash)
  ...
  client.update_device_instance(call_hash)
end

def update_device_instance(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.update_device_instance if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_device_instance(params)
