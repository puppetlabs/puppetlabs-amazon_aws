...
def operation_with_hash(arg_hash)
  ...
  client.describe_db_instance_automated_backups(call_hash)
end

def describe_db_instance_automated_backups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.describe_db_instance_automated_backups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_db_instance_automated_backups(params)
