...
def operation_with_hash(arg_hash)
  ...
  client.modify_option_group(call_hash)
end

def modify_option_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.modify_option_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_option_group(params)
