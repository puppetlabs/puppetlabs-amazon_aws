...
def operation_with_hash(arg_hash)
  ...
  client.list_conference_providers(call_hash)
end

def list_conference_providers(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.list_conference_providers if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_conference_providers(params)
