...
def operation_with_hash(arg_hash)
  ...
  client = Aws::Neptune::Client.new(region: region)
  client.remove_source_identifier_from_subscription(call_hash)
end

def remove_source_identifier_from_subscription(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Neptune::Client.new(region: region)
  response = client.remove_source_identifier_from_subscription if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = remove_source_identifier_from_subscription(params)
