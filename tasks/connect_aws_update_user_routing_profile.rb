...
def operation_with_hash(arg_hash)
  ...
  client.update_user_routing_profile(call_hash)
end

def update_user_routing_profile(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Connect::Client.new(region: region)
  response = client.update_user_routing_profile if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_user_routing_profile(params)
