...
def operation_with_hash(arg_hash)
  ...
  client.create_client_vpn_endpoint(call_hash)
end

def create_client_vpn_endpoint(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_client_vpn_endpoint if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_client_vpn_endpoint(params)
