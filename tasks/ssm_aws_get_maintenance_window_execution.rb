...
def operation_with_hash(arg_hash)
  ...
  client.get_maintenance_window_execution(call_hash)
end

def get_maintenance_window_execution(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.get_maintenance_window_execution if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_maintenance_window_execution(params)
