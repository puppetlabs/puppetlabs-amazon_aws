...
def operation_with_hash(arg_hash)
  ...
  client.describe_ecs_clusters(call_hash)
end

def describe_ecs_clusters(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::OpsWorks::Client.new(region: region)
  response = client.describe_ecs_clusters if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_ecs_clusters(params)
