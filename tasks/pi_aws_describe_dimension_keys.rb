...
def operation_with_hash(arg_hash)
  ...
  client = Aws::PI::Client.new(region: region)
  client.describe_dimension_keys(call_hash)
end

def describe_dimension_keys(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::PI::Client.new(region: region)
  response = client.describe_dimension_keys if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_dimension_keys(params)
