...
def operation_with_hash(arg_hash)
  ...
  client.describe_environment_managed_actions(call_hash)
end

def describe_environment_managed_actions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.describe_environment_managed_actions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_environment_managed_actions(params)
