...
def operation_with_hash(arg_hash)
  ...
  client.modify_load_balancer_attributes(call_hash)
end

def modify_load_balancer_attributes(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticLoadBalancingV2::Client.new(region: region)
  response = client.modify_load_balancer_attributes if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_load_balancer_attributes(params)
