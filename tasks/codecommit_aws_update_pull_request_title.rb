...
def operation_with_hash(arg_hash)
  ...
  client.update_pull_request_title(call_hash)
end

def update_pull_request_title(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeCommit::Client.new(region: region)
  response = client.update_pull_request_title if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_pull_request_title(params)
