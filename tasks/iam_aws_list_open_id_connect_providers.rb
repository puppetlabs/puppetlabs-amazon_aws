...
def operation_with_hash(arg_hash)
  ...
  client.list_open_id_connect_providers(call_hash)
end

def list_open_id_connect_providers(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.list_open_id_connect_providers if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_open_id_connect_providers(params)
