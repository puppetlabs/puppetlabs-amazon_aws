...
def operation_with_hash(arg_hash)
  ...
  client.list_device_instances(call_hash)
end

def list_device_instances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.list_device_instances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_device_instances(params)
