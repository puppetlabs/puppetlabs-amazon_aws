...
def operation_with_hash(arg_hash)
  ...
  client.create_rate_based_rule(call_hash)
end

def create_rate_based_rule(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.create_rate_based_rule if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_rate_based_rule(params)
