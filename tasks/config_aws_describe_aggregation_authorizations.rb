...
def operation_with_hash(arg_hash)
  ...
  client.describe_aggregation_authorizations(call_hash)
end

def describe_aggregation_authorizations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.describe_aggregation_authorizations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_aggregation_authorizations(params)
