...
def operation_with_hash(arg_hash)
  ...
  client.delete_custom_metadata(call_hash)
end

def delete_custom_metadata(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.delete_custom_metadata if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_custom_metadata(params)
