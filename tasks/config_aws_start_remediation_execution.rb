...
def operation_with_hash(arg_hash)
  ...
  client.start_remediation_execution(call_hash)
end

def start_remediation_execution(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.start_remediation_execution if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_remediation_execution(params)
