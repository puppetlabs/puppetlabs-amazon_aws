...
def operation_with_hash(arg_hash)
  ...
  client.create_cache_subnet_group(call_hash)
end

def create_cache_subnet_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElastiCache::Client.new(region: region)
  response = client.create_cache_subnet_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_cache_subnet_group(params)
