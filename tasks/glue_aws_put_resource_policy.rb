...
def operation_with_hash(arg_hash)
  ...
  client.put_resource_policy(call_hash)
end

def put_resource_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glue::Client.new(region: region)
  response = client.put_resource_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_resource_policy(params)
