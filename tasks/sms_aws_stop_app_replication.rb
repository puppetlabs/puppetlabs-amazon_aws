...
def operation_with_hash(arg_hash)
  ...
  client.stop_app_replication(call_hash)
end

def stop_app_replication(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SMS::Client.new(region: region)
  response = client.stop_app_replication if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = stop_app_replication(params)
