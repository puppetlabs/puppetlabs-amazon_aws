...
def operation_with_hash(arg_hash)
  ...
  client.get_base_path_mappings(call_hash)
end

def get_base_path_mappings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.get_base_path_mappings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_base_path_mappings(params)
