...
def operation_with_hash(arg_hash)
  ...
  client.describe_budget_performance_history(call_hash)
end

def describe_budget_performance_history(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Budgets::Client.new(region: region)
  response = client.describe_budget_performance_history if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_budget_performance_history(params)
