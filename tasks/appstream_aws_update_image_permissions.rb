...
def operation_with_hash(arg_hash)
  ...
  client.update_image_permissions(call_hash)
end

def update_image_permissions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AppStream::Client.new(region: region)
  response = client.update_image_permissions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_image_permissions(params)
