...
def operation_with_hash(arg_hash)
  ...
  client.update_rules_of_ip_group(call_hash)
end

def update_rules_of_ip_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkSpaces::Client.new(region: region)
  response = client.update_rules_of_ip_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_rules_of_ip_group(params)
