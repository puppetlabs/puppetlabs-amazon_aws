...
def operation_with_hash(arg_hash)
  ...
  client.get_app_launch_configuration(call_hash)
end

def get_app_launch_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SMS::Client.new(region: region)
  response = client.get_app_launch_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_app_launch_configuration(params)
