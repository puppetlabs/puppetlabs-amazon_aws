...
def operation_with_hash(arg_hash)
  ...
  client.delete_saml_provider(call_hash)
end

def delete_saml_provider(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.delete_saml_provider if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_saml_provider(params)
