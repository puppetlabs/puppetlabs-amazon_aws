...
def operation_with_hash(arg_hash)
  ...
  client.describe_instances_health(call_hash)
end

def describe_instances_health(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.describe_instances_health if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_instances_health(params)
