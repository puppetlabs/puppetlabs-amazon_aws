...
def operation_with_hash(arg_hash)
  ...
  client.describe_activations(call_hash)
end

def describe_activations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.describe_activations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_activations(params)
