...
def operation_with_hash(arg_hash)
  ...
  client.describe_byoip_cidrs(call_hash)
end

def describe_byoip_cidrs(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_byoip_cidrs if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_byoip_cidrs(params)
