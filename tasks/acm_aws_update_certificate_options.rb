...
def operation_with_hash(arg_hash)
  ...
  client.update_certificate_options(call_hash)
end

def update_certificate_options(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ACM::Client.new(region: region)
  response = client.update_certificate_options if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_certificate_options(params)
