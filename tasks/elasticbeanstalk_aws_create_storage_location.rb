...
def operation_with_hash(arg_hash)
  ...
  client.create_storage_location(call_hash)
end

def create_storage_location(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.create_storage_location if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_storage_location(params)
