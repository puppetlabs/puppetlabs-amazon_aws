...
def operation_with_hash(arg_hash)
  ...
  client.get_instances_health_status(call_hash)
end

def get_instances_health_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceDiscovery::Client.new(region: region)
  response = client.get_instances_health_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_instances_health_status(params)
