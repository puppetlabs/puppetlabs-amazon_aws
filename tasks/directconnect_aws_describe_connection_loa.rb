...
def operation_with_hash(arg_hash)
  ...
  client.describe_connection_loa(call_hash)
end

def describe_connection_loa(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.describe_connection_loa if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_connection_loa(params)
