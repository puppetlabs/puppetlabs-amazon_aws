...
def operation_with_hash(arg_hash)
  ...
  client.get_assessment_report(call_hash)
end

def get_assessment_report(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.get_assessment_report if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_assessment_report(params)
