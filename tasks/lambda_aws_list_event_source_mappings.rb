...
def operation_with_hash(arg_hash)
  ...
  client.list_event_source_mappings(call_hash)
end

def list_event_source_mappings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lambda::Client.new(region: region)
  response = client.list_event_source_mappings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_event_source_mappings(params)
