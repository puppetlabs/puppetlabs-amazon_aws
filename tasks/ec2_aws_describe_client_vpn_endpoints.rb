...
def operation_with_hash(arg_hash)
  ...
  client.describe_client_vpn_endpoints(call_hash)
end

def describe_client_vpn_endpoints(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_client_vpn_endpoints if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_client_vpn_endpoints(params)
