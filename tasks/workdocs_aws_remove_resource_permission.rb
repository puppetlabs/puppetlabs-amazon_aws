...
def operation_with_hash(arg_hash)
  ...
  client.remove_resource_permission(call_hash)
end

def remove_resource_permission(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.remove_resource_permission if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = remove_resource_permission(params)
