...
def operation_with_hash(arg_hash)
  ...
  client.deregister_from_work_mail(call_hash)
end

def deregister_from_work_mail(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkMail::Client.new(region: region)
  response = client.deregister_from_work_mail if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = deregister_from_work_mail(params)
