...
def operation_with_hash(arg_hash)
  ...
  client.delete_network_profile(call_hash)
end

def delete_network_profile(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.delete_network_profile if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_network_profile(params)
