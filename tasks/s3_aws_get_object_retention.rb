...
def operation_with_hash(arg_hash)
  ...
  client.get_object_retention(call_hash)
end

def get_object_retention(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::S3::Client.new(region: region)
  response = client.get_object_retention if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_object_retention(params)
