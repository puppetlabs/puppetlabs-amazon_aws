...
def operation_with_hash(arg_hash)
  ...
  client.delete_cluster_parameter_group(call_hash)
end

def delete_cluster_parameter_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.delete_cluster_parameter_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_cluster_parameter_group(params)
