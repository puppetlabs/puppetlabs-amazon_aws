...
def operation_with_hash(arg_hash)
  ...
  client.modify_capacity_reservation(call_hash)
end

def modify_capacity_reservation(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.modify_capacity_reservation if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_capacity_reservation(params)
