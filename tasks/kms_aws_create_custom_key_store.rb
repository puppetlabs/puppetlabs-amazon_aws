...
def operation_with_hash(arg_hash)
  ...
  client.create_custom_key_store(call_hash)
end

def create_custom_key_store(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KMS::Client.new(region: region)
  response = client.create_custom_key_store if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_custom_key_store(params)
