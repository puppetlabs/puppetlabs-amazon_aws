...
def operation_with_hash(arg_hash)
  ...
  client.get_core_definition(call_hash)
end

def get_core_definition(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.get_core_definition if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_core_definition(params)
