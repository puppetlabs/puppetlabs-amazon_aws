...
def operation_with_hash(arg_hash)
  ...
  client.delete_git_hub_account_token(call_hash)
end

def delete_git_hub_account_token(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeDeploy::Client.new(region: region)
  response = client.delete_git_hub_account_token if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_git_hub_account_token(params)
