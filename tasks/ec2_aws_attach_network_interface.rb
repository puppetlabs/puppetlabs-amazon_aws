...
def operation_with_hash(arg_hash)
  ...
  client.attach_network_interface(call_hash)
end

def attach_network_interface(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.attach_network_interface if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = attach_network_interface(params)
