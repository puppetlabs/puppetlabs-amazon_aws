...
def operation_with_hash(arg_hash)
  ...
  client.list_command_invocations(call_hash)
end

def list_command_invocations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.list_command_invocations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_command_invocations(params)
