...
def operation_with_hash(arg_hash)
  ...
  client.describe_account_limits(call_hash)
end

def describe_account_limits(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudFormation::Client.new(region: region)
  response = client.describe_account_limits if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_account_limits(params)
