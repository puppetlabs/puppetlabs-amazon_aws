...
def operation_with_hash(arg_hash)
  ...
  client.create_batch_prediction(call_hash)
end

def create_batch_prediction(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MachineLearning::Client.new(region: region)
  response = client.create_batch_prediction if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_batch_prediction(params)
