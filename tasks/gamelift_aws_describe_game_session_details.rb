...
def operation_with_hash(arg_hash)
  ...
  client.describe_game_session_details(call_hash)
end

def describe_game_session_details(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.describe_game_session_details if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_game_session_details(params)
