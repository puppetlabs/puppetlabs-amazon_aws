...
def operation_with_hash(arg_hash)
  ...
  client.detach_load_balancer_target_groups(call_hash)
end

def detach_load_balancer_target_groups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.detach_load_balancer_target_groups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = detach_load_balancer_target_groups(params)
