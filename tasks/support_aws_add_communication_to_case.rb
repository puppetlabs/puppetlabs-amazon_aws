...
def operation_with_hash(arg_hash)
  ...
  client.add_communication_to_case(call_hash)
end

def add_communication_to_case(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Support::Client.new(region: region)
  response = client.add_communication_to_case if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_communication_to_case(params)
