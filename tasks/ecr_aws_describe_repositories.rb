...
def operation_with_hash(arg_hash)
  ...
  client.describe_repositories(call_hash)
end

def describe_repositories(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECR::Client.new(region: region)
  response = client.describe_repositories if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_repositories(params)
