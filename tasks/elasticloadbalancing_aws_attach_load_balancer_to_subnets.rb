...
def operation_with_hash(arg_hash)
  ...
  client.attach_load_balancer_to_subnets(call_hash)
end

def attach_load_balancer_to_subnets(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticLoadBalancingV2::Client.new(region: region)
  response = client.attach_load_balancer_to_subnets if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = attach_load_balancer_to_subnets(params)
