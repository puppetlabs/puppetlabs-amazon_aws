...
def operation_with_hash(arg_hash)
  ...
  client.get_signing_profile(call_hash)
end

def get_signing_profile(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Signer::Client.new(region: region)
  response = client.get_signing_profile if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_signing_profile(params)
