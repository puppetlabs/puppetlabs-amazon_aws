...
def operation_with_hash(arg_hash)
  ...
  client.update_user_phone_config(call_hash)
end

def update_user_phone_config(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Connect::Client.new(region: region)
  response = client.update_user_phone_config if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_user_phone_config(params)
