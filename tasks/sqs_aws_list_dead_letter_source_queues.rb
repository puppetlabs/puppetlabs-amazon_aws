...
def operation_with_hash(arg_hash)
  ...
  client.list_dead_letter_source_queues(call_hash)
end

def list_dead_letter_source_queues(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SQS::Client.new(region: region)
  response = client.list_dead_letter_source_queues if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_dead_letter_source_queues(params)
