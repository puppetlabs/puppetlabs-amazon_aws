...
def operation_with_hash(arg_hash)
  ...
  client.delete_global_cluster(call_hash)
end

def delete_global_cluster(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.delete_global_cluster if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_global_cluster(params)
