...
def operation_with_hash(arg_hash)
  ...
  client.get_lifecycle_policies(call_hash)
end

def get_lifecycle_policies(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DLM::Client.new(region: region)
  response = client.get_lifecycle_policies if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_lifecycle_policies(params)
