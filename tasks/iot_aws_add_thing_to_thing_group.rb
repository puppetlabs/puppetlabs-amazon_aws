...
def operation_with_hash(arg_hash)
  ...
  client.add_thing_to_thing_group(call_hash)
end

def add_thing_to_thing_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.add_thing_to_thing_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_thing_to_thing_group(params)
