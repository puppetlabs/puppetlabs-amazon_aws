...
def operation_with_hash(arg_hash)
  ...
  client.list_event_subscriptions(call_hash)
end

def list_event_subscriptions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.list_event_subscriptions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_event_subscriptions(params)
