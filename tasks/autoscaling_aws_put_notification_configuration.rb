...
def operation_with_hash(arg_hash)
  ...
  client.put_notification_configuration(call_hash)
end

def put_notification_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.put_notification_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_notification_configuration(params)
