...
def operation_with_hash(arg_hash)
  ...
  client.update_nfs_file_share(call_hash)
end

def update_nfs_file_share(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.update_nfs_file_share if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_nfs_file_share(params)
