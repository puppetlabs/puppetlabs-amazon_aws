...
def operation_with_hash(arg_hash)
  ...
  client.describe_option_group_options(call_hash)
end

def describe_option_group_options(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.describe_option_group_options if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_option_group_options(params)
