...
def operation_with_hash(arg_hash)
  ...
  client.update_domain_nameservers(call_hash)
end

def update_domain_nameservers(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53Domains::Client.new(region: region)
  response = client.update_domain_nameservers if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_domain_nameservers(params)
