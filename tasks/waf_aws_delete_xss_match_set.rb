...
def operation_with_hash(arg_hash)
  ...
  client.delete_xss_match_set(call_hash)
end

def delete_xss_match_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.delete_xss_match_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_xss_match_set(params)
