...
def operation_with_hash(arg_hash)
  ...
  client.get_client_certificate(call_hash)
end

def get_client_certificate(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.get_client_certificate if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_client_certificate(params)
