...
def operation_with_hash(arg_hash)
  ...
  client.start_outbound_voice_contact(call_hash)
end

def start_outbound_voice_contact(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Connect::Client.new(region: region)
  response = client.start_outbound_voice_contact if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_outbound_voice_contact(params)
