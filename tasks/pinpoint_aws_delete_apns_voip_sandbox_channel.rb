...
def operation_with_hash(arg_hash)
  ...
  client = Aws::Pinpoint::Client.new(region: region)
  client.delete_apns_voip_sandbox_channel(call_hash)
end

def delete_apns_voip_sandbox_channel(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Pinpoint::Client.new(region: region)
  response = client.delete_apns_voip_sandbox_channel if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_apns_voip_sandbox_channel(params)
