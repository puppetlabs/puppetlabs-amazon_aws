...
def operation_with_hash(arg_hash)
  ...
  client = Aws::Cloud9::Client.new(region: region)
  client.describe_environment_memberships(call_hash)
end

def describe_environment_memberships(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Cloud9::Client.new(region: region)
  response = client.describe_environment_memberships if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_environment_memberships(params)
