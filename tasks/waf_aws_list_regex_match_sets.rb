...
def operation_with_hash(arg_hash)
  ...
  client.list_regex_match_sets(call_hash)
end

def list_regex_match_sets(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.list_regex_match_sets if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_regex_match_sets(params)
