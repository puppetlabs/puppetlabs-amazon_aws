...
def operation_with_hash(arg_hash)
  ...
  client.create_code_repository(call_hash)
end

def create_code_repository(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.create_code_repository if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_code_repository(params)
