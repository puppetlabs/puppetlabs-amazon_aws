...
def operation_with_hash(arg_hash)
  ...
  client = Aws::States::Client.new(region: region)
  client.describe_state_machine_for_execution(call_hash)
end

def describe_state_machine_for_execution(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::States::Client.new(region: region)
  response = client.describe_state_machine_for_execution if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_state_machine_for_execution(params)
