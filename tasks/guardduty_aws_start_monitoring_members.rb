...
def operation_with_hash(arg_hash)
  ...
  client = Aws::GuardDuty::Client.new(region: region)
  client.start_monitoring_members(call_hash)
end

def start_monitoring_members(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GuardDuty::Client.new(region: region)
  response = client.start_monitoring_members if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_monitoring_members(params)
