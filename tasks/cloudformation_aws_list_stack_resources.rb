...
def operation_with_hash(arg_hash)
  ...
  client.list_stack_resources(call_hash)
end

def list_stack_resources(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudFormation::Client.new(region: region)
  response = client.list_stack_resources if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_stack_resources(params)
