...
def operation_with_hash(arg_hash)
  ...
  client.get_compliance_details_by_config_rule(call_hash)
end

def get_compliance_details_by_config_rule(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.get_compliance_details_by_config_rule if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_compliance_details_by_config_rule(params)
