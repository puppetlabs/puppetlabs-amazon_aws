...
def operation_with_hash(arg_hash)
  ...
  client.update_user_security_profiles(call_hash)
end

def update_user_security_profiles(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Connect::Client.new(region: region)
  response = client.update_user_security_profiles if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_user_security_profiles(params)
