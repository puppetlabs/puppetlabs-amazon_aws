...
def operation_with_hash(arg_hash)
  ...
  client.describe_cache_security_groups(call_hash)
end

def describe_cache_security_groups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElastiCache::Client.new(region: region)
  response = client.describe_cache_security_groups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_cache_security_groups(params)
