...
def operation_with_hash(arg_hash)
  ...
  client.modify_fpga_image_attribute(call_hash)
end

def modify_fpga_image_attribute(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.modify_fpga_image_attribute if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_fpga_image_attribute(params)
