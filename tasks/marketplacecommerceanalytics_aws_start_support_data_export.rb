...
def operation_with_hash(arg_hash)
  ...
  client = Aws::MarketplaceCommerceAnalytics::Client.new(region: region)
  client.start_support_data_export(call_hash)
end

def start_support_data_export(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MarketplaceCommerceAnalytics::Client.new(region: region)
  response = client.start_support_data_export if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_support_data_export(params)
