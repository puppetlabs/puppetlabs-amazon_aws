...
def operation_with_hash(arg_hash)
  ...
  client.get_protection_status(call_hash)
end

def get_protection_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::FMS::Client.new(region: region)
  response = client.get_protection_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_protection_status(params)
