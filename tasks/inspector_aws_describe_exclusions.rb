...
def operation_with_hash(arg_hash)
  ...
  client.describe_exclusions(call_hash)
end

def describe_exclusions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.describe_exclusions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_exclusions(params)
