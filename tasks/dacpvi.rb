...
def operation_with_hash(arg_hash)
  ...
  client.create_private_virtual_interface(call_hash)
end

def create_private_virtual_interface(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.create_private_virtual_interface if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_private_virtual_interface(params)
