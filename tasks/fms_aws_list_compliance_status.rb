...
def operation_with_hash(arg_hash)
  ...
  client.list_compliance_status(call_hash)
end

def list_compliance_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::FMS::Client.new(region: region)
  response = client.list_compliance_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_compliance_status(params)
