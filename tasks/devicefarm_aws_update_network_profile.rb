...
def operation_with_hash(arg_hash)
  ...
  client.update_network_profile(call_hash)
end

def update_network_profile(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.update_network_profile if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_network_profile(params)
