...
def operation_with_hash(arg_hash)
  ...
  client = Aws::Shield::Client.new(region: region)
  client.describe_emergency_contact_settings(call_hash)
end

def describe_emergency_contact_settings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Shield::Client.new(region: region)
  response = client.describe_emergency_contact_settings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_emergency_contact_settings(params)
