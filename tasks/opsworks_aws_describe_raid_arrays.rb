...
def operation_with_hash(arg_hash)
  ...
  client.describe_raid_arrays(call_hash)
end

def describe_raid_arrays(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::OpsWorks::Client.new(region: region)
  response = client.describe_raid_arrays if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_raid_arrays(params)
