...
def GetJobTemplate(*args)
  puts "Calling task operation GetJobTemplate" 
  ...
  client = Aws::MediaConvert::Client.new(region: get_region)

  if arg_hash.empty?
    response = client.get_job_template()
  else 
    call_hash={}
    ...
    response = client.get_job_template(call_hash)
  end
