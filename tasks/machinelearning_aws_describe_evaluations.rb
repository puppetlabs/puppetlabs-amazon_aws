...
def operation_with_hash(arg_hash)
  ...
  client.describe_evaluations(call_hash)
end

def describe_evaluations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MachineLearning::Client.new(region: region)
  response = client.describe_evaluations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_evaluations(params)
