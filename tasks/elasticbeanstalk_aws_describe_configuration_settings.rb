...
def operation_with_hash(arg_hash)
  ...
  client.describe_configuration_settings(call_hash)
end

def describe_configuration_settings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.describe_configuration_settings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_configuration_settings(params)
