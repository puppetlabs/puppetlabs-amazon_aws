...
def operation_with_hash(arg_hash)
  ...
  client.describe_rules_packages(call_hash)
end

def describe_rules_packages(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.describe_rules_packages if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_rules_packages(params)
