...
def operation_with_hash(arg_hash)
  ...
  client.create_lifecycle_policy(call_hash)
end

def create_lifecycle_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DLM::Client.new(region: region)
  response = client.create_lifecycle_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_lifecycle_policy(params)
