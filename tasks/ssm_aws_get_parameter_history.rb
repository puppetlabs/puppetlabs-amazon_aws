...
def operation_with_hash(arg_hash)
  ...
  client.get_parameter_history(call_hash)
end

def get_parameter_history(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.get_parameter_history if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_parameter_history(params)
