...
def operation_with_hash(arg_hash)
  ...
  client.start_smart_home_appliance_discovery(call_hash)
end

def start_smart_home_appliance_discovery(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.start_smart_home_appliance_discovery if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_smart_home_appliance_discovery(params)
