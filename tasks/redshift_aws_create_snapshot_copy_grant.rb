...
def operation_with_hash(arg_hash)
  ...
  client.create_snapshot_copy_grant(call_hash)
end

def create_snapshot_copy_grant(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.create_snapshot_copy_grant if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_snapshot_copy_grant(params)
