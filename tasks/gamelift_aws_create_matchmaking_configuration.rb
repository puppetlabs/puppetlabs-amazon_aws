...
def operation_with_hash(arg_hash)
  ...
  client.create_matchmaking_configuration(call_hash)
end

def create_matchmaking_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.create_matchmaking_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_matchmaking_configuration(params)
