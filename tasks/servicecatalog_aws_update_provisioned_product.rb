...
def operation_with_hash(arg_hash)
  ...
  client.update_provisioned_product(call_hash)
end

def update_provisioned_product(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.update_provisioned_product if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_provisioned_product(params)
