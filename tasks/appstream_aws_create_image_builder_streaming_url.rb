...
def operation_with_hash(arg_hash)
  ...
  client.create_image_builder_streaming_url(call_hash)
end

def create_image_builder_streaming_url(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AppStream::Client.new(region: region)
  response = client.create_image_builder_streaming_url if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_image_builder_streaming_url(params)
