...
def operation_with_hash(arg_hash)
  ...
  client.create_tape_with_barcode(call_hash)
end

def create_tape_with_barcode(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.create_tape_with_barcode if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_tape_with_barcode(params)
