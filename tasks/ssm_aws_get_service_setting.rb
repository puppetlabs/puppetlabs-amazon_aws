...
def operation_with_hash(arg_hash)
  ...
  client.get_service_setting(call_hash)
end

def get_service_setting(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.get_service_setting if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_service_setting(params)
