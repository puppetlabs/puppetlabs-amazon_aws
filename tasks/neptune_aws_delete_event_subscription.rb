...
def operation_with_hash(arg_hash)
  ...
  client.delete_event_subscription(call_hash)
end

def delete_event_subscription(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Neptune::Client.new(region: region)
  response = client.delete_event_subscription if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_event_subscription(params)
