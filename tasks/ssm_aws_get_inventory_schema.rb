...
def operation_with_hash(arg_hash)
  ...
  client.get_inventory_schema(call_hash)
end

def get_inventory_schema(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.get_inventory_schema if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_inventory_schema(params)
