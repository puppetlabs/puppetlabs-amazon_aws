...
def operation_with_hash(arg_hash)
  ...
  client.update_runtime_configuration(call_hash)
end

def update_runtime_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.update_runtime_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_runtime_configuration(params)
