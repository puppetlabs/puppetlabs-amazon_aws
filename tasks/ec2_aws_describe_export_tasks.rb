...
def operation_with_hash(arg_hash)
  ...
  client.describe_export_tasks(call_hash)
end

def describe_export_tasks(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_export_tasks if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_export_tasks(params)
