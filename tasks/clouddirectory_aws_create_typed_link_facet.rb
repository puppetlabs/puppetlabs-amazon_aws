...
def operation_with_hash(arg_hash)
  ...
  client.create_typed_link_facet(call_hash)
end

def create_typed_link_facet(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudDirectory::Client.new(region: region)
  response = client.create_typed_link_facet if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_typed_link_facet(params)
