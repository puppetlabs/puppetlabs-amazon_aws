...
def operation_with_hash(arg_hash)
  ...
  client.get_application_settings(call_hash)
end

def get_application_settings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Pinpoint::Client.new(region: region)
  response = client.get_application_settings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_application_settings(params)
