...
def operation_with_hash(arg_hash)
  ...
  client.modify_vpc_endpoint(call_hash)
end

def modify_vpc_endpoint(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.modify_vpc_endpoint if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_vpc_endpoint(params)
