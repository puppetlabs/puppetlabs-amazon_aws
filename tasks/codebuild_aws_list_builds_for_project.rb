...
def operation_with_hash(arg_hash)
  ...
  client.list_builds_for_project(call_hash)
end

def list_builds_for_project(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeBuild::Client.new(region: region)
  response = client.list_builds_for_project if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_builds_for_project(params)
