...
def operation_with_hash(arg_hash)
  ...
  client.update_managed_instance_role(call_hash)
end

def update_managed_instance_role(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.update_managed_instance_role if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_managed_instance_role(params)
