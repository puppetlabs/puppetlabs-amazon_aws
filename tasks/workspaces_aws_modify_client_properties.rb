...
def operation_with_hash(arg_hash)
  ...
  client.modify_client_properties(call_hash)
end

def modify_client_properties(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkSpaces::Client.new(region: region)
  response = client.modify_client_properties if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_client_properties(params)
