...
def operation_with_hash(arg_hash)
  ...
  client.delete_db_instance_automated_backup(call_hash)
end

def delete_db_instance_automated_backup(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.delete_db_instance_automated_backup if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_db_instance_automated_backup(params)
