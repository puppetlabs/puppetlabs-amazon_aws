...
def operation_with_hash(arg_hash)
  ...
  client.describe_stack_resource_drifts(call_hash)
end

def describe_stack_resource_drifts(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudFormation::Client.new(region: region)
  response = client.describe_stack_resource_drifts if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_stack_resource_drifts(params)
