...
def operation_with_hash(arg_hash)
  ...
  client.create_hsm_configuration(call_hash)
end

def create_hsm_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.create_hsm_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_hsm_configuration(params)
