...
def operation_with_hash(arg_hash)
  ...
  client.create_realtime_endpoint(call_hash)
end

def create_realtime_endpoint(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MachineLearning::Client.new(region: region)
  response = client.create_realtime_endpoint if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_realtime_endpoint(params)
