...
def operation_with_hash(arg_hash)
  ...
  client.get_size_constraint_set(call_hash)
end

def get_size_constraint_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.get_size_constraint_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_size_constraint_set(params)
