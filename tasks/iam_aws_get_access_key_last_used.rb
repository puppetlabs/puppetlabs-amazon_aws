...
def operation_with_hash(arg_hash)
  ...
  client.get_access_key_last_used(call_hash)
end

def get_access_key_last_used(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.get_access_key_last_used if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_access_key_last_used(params)
