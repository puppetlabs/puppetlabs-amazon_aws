...
def operation_with_hash(arg_hash)
  ...
  client.create_auto_scaling_group(call_hash)
end

def create_auto_scaling_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.create_auto_scaling_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_auto_scaling_group(params)
