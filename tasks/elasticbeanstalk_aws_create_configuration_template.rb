...
def operation_with_hash(arg_hash)
  ...
  client.create_configuration_template(call_hash)
end

def create_configuration_template(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.create_configuration_template if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_configuration_template(params)
