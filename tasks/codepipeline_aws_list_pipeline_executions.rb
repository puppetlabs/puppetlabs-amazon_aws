...
def operation_with_hash(arg_hash)
  ...
  client.list_pipeline_executions(call_hash)
end

def list_pipeline_executions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodePipeline::Client.new(region: region)
  response = client.list_pipeline_executions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_pipeline_executions(params)
