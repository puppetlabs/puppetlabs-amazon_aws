...
def operation_with_hash(arg_hash)
  ...
  client.create_launch_configuration(call_hash)
end

def create_launch_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.create_launch_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_launch_configuration(params)
