...
def operation_with_hash(arg_hash)
  ...
  client.update_open_id_connect_provider_thumbprint(call_hash)
end

def update_open_id_connect_provider_thumbprint(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.update_open_id_connect_provider_thumbprint if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_open_id_connect_provider_thumbprint(params)
