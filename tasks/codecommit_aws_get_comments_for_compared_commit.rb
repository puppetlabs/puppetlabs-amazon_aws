...
def GetCommentsForComparedCommit(*args)
  puts "Calling task operation GetCommentsForComparedCommit" 
  ...
  if arg_hash.empty?
    ...
    response = client.get_comments_for_compared_commit(call_hash)
  end
...
begin
  result = GetCommentsForComparedCommit(params)
