...
def operation_with_hash(arg_hash)
  ...
  client.create_vpc_endpoint_connection_notification(call_hash)
end

def create_vpc_endpoint_connection_notification(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_vpc_endpoint_connection_notification if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_vpc_endpoint_connection_notification(params)
