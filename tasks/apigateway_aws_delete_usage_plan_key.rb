...
def operation_with_hash(arg_hash)
  ...
  client.delete_usage_plan_key(call_hash)
end

def delete_usage_plan_key(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.delete_usage_plan_key if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_usage_plan_key(params)
