...
def operation_with_hash(arg_hash)
  ...
  client = Aws::Macie::Client.new(region: region)
  client.list_member_accounts(call_hash)
end

def list_member_accounts(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Macie::Client.new(region: region)
  response = client.list_member_accounts if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_member_accounts(params)
