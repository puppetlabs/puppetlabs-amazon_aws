...
def operation_with_hash(arg_hash)
  ...
  client.get_document_version(call_hash)
end

def get_document_version(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.get_document_version if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_document_version(params)
