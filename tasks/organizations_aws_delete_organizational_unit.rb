...
def operation_with_hash(arg_hash)
  ...
  client.delete_organizational_unit(call_hash)
end

def delete_organizational_unit(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Organizations::Client.new(region: region)
  response = client.delete_organizational_unit if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_organizational_unit(params)
