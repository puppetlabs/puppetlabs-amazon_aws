...
def operation_with_hash(arg_hash)
  ...
  client.get_reserved_instances_exchange_quote(call_hash)
end

def get_reserved_instances_exchange_quote(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.get_reserved_instances_exchange_quote if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_reserved_instances_exchange_quote(params)
