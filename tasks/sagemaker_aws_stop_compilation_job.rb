...
def operation_with_hash(arg_hash)
  ...
  client.stop_compilation_job(call_hash)
end

def stop_compilation_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.stop_compilation_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = stop_compilation_job(params)
