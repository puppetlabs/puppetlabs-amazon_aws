...
def operation_with_hash(arg_hash)
  ...
  client.update_security_group_rule_descriptions_egress(call_hash)
end

def update_security_group_rule_descriptions_egress(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.update_security_group_rule_descriptions_egress if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_security_group_rule_descriptions_egress(params)
