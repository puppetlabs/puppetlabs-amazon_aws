...
def operation_with_hash(arg_hash)
  ...
  client.get_download_url_for_layer(call_hash)
end

def get_download_url_for_layer(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECR::Client.new(region: region)
  response = client.get_download_url_for_layer if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_download_url_for_layer(params)
