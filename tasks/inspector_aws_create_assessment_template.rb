...
def operation_with_hash(arg_hash)
  ...
  client.create_assessment_template(call_hash)
end

def create_assessment_template(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.create_assessment_template if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_assessment_template(params)
