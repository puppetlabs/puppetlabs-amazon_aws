...
def operation_with_hash(arg_hash)
  ...
  client.list_available_zones(call_hash)
end

def list_available_zones(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudHSM::Client.new(region: region)
  response = client.list_available_zones if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_available_zones(params)
