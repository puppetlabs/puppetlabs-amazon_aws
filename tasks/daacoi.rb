...
def operation_with_hash(arg_hash)
  ...
  client.allocate_connection_on_interconnect(call_hash)
end

def allocate_connection_on_interconnect(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.allocate_connection_on_interconnect if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = allocate_connection_on_interconnect(params)
