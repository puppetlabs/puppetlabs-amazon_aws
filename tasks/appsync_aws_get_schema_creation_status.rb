...
def operation_with_hash(arg_hash)
  ...
  client.get_schema_creation_status(call_hash)
end

def get_schema_creation_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AppSync::Client.new(region: region)
  response = client.get_schema_creation_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_schema_creation_status(params)
