...
def operation_with_hash(arg_hash)
  ...
  client.stop_remote_access_session(call_hash)
end

def stop_remote_access_session(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.stop_remote_access_session if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = stop_remote_access_session(params)
