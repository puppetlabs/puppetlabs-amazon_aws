...
def operation_with_hash(arg_hash)
  ...
  client.describe_instance_health(call_hash)
end

def describe_instance_health(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticLoadBalancingV2::Client.new(region: region)
  response = client.describe_instance_health if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_instance_health(params)
