...
def operation_with_hash(arg_hash)
  ...
  client.create_transit_gateway_route_table(call_hash)
end

def create_transit_gateway_route_table(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_transit_gateway_route_table if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_transit_gateway_route_table(params)
