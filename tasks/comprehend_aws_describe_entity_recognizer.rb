...
def operation_with_hash(arg_hash)
  ...
  client.describe_entity_recognizer(call_hash)
end

def describe_entity_recognizer(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Comprehend::Client.new(region: region)
  response = client.describe_entity_recognizer if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_entity_recognizer(params)
