...
def operation_with_hash(arg_hash)
  ...
  client.modify_target_group(call_hash)
end

def modify_target_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticLoadBalancingV2::Client.new(region: region)
  response = client.modify_target_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_target_group(params)
