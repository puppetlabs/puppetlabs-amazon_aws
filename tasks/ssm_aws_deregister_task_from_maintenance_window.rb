...
def operation_with_hash(arg_hash)
  ...
  client.deregister_task_from_maintenance_window(call_hash)
end

def deregister_task_from_maintenance_window(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.deregister_task_from_maintenance_window if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = deregister_task_from_maintenance_window(params)
