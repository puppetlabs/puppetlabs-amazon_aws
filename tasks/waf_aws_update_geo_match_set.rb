...
def operation_with_hash(arg_hash)
  ...
  client.update_geo_match_set(call_hash)
end

def update_geo_match_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.update_geo_match_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_geo_match_set(params)
