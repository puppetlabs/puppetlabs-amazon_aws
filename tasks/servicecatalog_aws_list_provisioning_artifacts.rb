...
def operation_with_hash(arg_hash)
  ...
  client.list_provisioning_artifacts(call_hash)
end

def list_provisioning_artifacts(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.list_provisioning_artifacts if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_provisioning_artifacts(params)
