...
def operation_with_hash(arg_hash)
  ...
  client.list_principal_things(call_hash)
end

def list_principal_things(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.list_principal_things if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_principal_things(params)
