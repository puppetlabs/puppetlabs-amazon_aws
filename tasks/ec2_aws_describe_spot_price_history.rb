...
def operation_with_hash(arg_hash)
  ...
  client.describe_spot_price_history(call_hash)
end

def describe_spot_price_history(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_spot_price_history if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_spot_price_history(params)
