...
def operation_with_hash(arg_hash)
  ...
  client.describe_transit_gateway_attachments(call_hash)
end

def describe_transit_gateway_attachments(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_transit_gateway_attachments if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_transit_gateway_attachments(params)
