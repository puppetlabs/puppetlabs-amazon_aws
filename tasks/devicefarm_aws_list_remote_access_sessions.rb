...
def operation_with_hash(arg_hash)
  ...
  client.list_remote_access_sessions(call_hash)
end

def list_remote_access_sessions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.list_remote_access_sessions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_remote_access_sessions(params)
