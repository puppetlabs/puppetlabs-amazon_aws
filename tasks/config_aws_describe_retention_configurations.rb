...
def operation_with_hash(arg_hash)
  ...
  client.describe_retention_configurations(call_hash)
end

def describe_retention_configurations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.describe_retention_configurations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_retention_configurations(params)
