...
def operation_with_hash(arg_hash)
  ...
  client.get_third_party_job_details(call_hash)
end

def get_third_party_job_details(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodePipeline::Client.new(region: region)
  response = client.get_third_party_job_details if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_third_party_job_details(params)
