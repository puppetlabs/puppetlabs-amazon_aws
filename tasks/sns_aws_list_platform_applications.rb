...
def operation_with_hash(arg_hash)
  ...
  client.list_platform_applications(call_hash)
end

def list_platform_applications(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SNS::Client.new(region: region)
  response = client.list_platform_applications if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_platform_applications(params)
