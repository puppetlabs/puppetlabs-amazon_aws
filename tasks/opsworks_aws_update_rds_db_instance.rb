...
def operation_with_hash(arg_hash)
  ...
  client.update_rds_db_instance(call_hash)
end

def update_rds_db_instance(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::OpsWorks::Client.new(region: region)
  response = client.update_rds_db_instance if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_rds_db_instance(params)
