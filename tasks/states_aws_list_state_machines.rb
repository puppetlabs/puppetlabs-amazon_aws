...
def operation_with_hash(arg_hash)
  ...
  client.list_state_machines(call_hash)
end

def list_state_machines(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::States::Client.new(region: region)
  response = client.list_state_machines if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_state_machines(params)
