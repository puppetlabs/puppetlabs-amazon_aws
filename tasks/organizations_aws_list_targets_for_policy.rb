...
def operation_with_hash(arg_hash)
  ...
  client.list_targets_for_policy(call_hash)
end

def list_targets_for_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Organizations::Client.new(region: region)
  response = client.list_targets_for_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_targets_for_policy(params)
