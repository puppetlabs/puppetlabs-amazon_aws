...
def operation_with_hash(arg_hash)
  ...
  client.create_player_session(call_hash)
end

def create_player_session(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.create_player_session if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_player_session(params)
