...
def operation_with_hash(arg_hash)
  ...
  client.start_match_backfill(call_hash)
end

def start_match_backfill(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.start_match_backfill if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_match_backfill(params)
