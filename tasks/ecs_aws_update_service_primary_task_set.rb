...
def operation_with_hash(arg_hash)
  ...
  client.update_service_primary_task_set(call_hash)
end

def update_service_primary_task_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.update_service_primary_task_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_service_primary_task_set(params)
