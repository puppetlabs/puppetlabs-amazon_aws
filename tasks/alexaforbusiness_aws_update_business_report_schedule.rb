...
def operation_with_hash(arg_hash)
  ...
  client.update_business_report_schedule(call_hash)
end

def update_business_report_schedule(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.update_business_report_schedule if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_business_report_schedule(params)
