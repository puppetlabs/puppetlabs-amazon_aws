...
def operation_with_hash(arg_hash)
  ...
  client.update_integration_response(call_hash)
end

def update_integration_response(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.update_integration_response if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_integration_response(params)
