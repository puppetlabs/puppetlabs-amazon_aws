...
def operation_with_hash(arg_hash)
  ...
  client.get_gateway_responses(call_hash)
end

def get_gateway_responses(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.get_gateway_responses if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_gateway_responses(params)
