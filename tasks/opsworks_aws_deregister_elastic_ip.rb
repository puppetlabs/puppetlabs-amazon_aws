...
def operation_with_hash(arg_hash)
  ...
  client.deregister_elastic_ip(call_hash)
end

def deregister_elastic_ip(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::OpsWorks::Client.new(region: region)
  response = client.deregister_elastic_ip if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = deregister_elastic_ip(params)
