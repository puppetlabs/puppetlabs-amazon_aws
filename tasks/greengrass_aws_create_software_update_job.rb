...
def operation_with_hash(arg_hash)
  ...
  client.create_software_update_job(call_hash)
end

def create_software_update_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.create_software_update_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_software_update_job(params)
