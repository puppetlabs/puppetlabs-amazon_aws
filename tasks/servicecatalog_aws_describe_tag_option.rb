...
def operation_with_hash(arg_hash)
  ...
  client.describe_tag_option(call_hash)
end

def describe_tag_option(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.describe_tag_option if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_tag_option(params)
