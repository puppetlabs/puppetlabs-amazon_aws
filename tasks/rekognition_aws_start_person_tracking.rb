...
def operation_with_hash(arg_hash)
  ...
  client.start_person_tracking(call_hash)
end

def start_person_tracking(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Rekognition::Client.new(region: region)
  response = client.start_person_tracking if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_person_tracking(params)
