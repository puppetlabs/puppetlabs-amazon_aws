...
def operation_with_hash(arg_hash)
  ...
  client.add_source_identifier_to_subscription(call_hash)
end

def add_source_identifier_to_subscription(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Neptune::Client.new(region: region)
  response = client.add_source_identifier_to_subscription if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_source_identifier_to_subscription(params)
