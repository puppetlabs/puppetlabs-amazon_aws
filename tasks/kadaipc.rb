...
def operation_with_hash(arg_hash)
  ...
  client = Aws::KinesisAnalytics::Client.new(region: region)
  client.delete_application_input_processing_configuration(call_hash)
end

def delete_application_input_processing_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KinesisAnalytics::Client.new(region: region)
  response = client.delete_application_input_processing_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_application_input_processing_configuration(params)
