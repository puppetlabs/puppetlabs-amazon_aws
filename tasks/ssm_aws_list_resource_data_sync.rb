...
def operation_with_hash(arg_hash)
  ...
  client.list_resource_data_sync(call_hash)
end

def list_resource_data_sync(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.list_resource_data_sync if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_resource_data_sync(params)
