...
def operation_with_hash(arg_hash)
  ...
  client.create_vpc_endpoint_service_configuration(call_hash)
end

def create_vpc_endpoint_service_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_vpc_endpoint_service_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_vpc_endpoint_service_configuration(params)
