...
def operation_with_hash(arg_hash)
  ...
  client.create_dynamic_thing_group(call_hash)
end

def create_dynamic_thing_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.create_dynamic_thing_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_dynamic_thing_group(params)
