...
def operation_with_hash(arg_hash)
  ...
  client.update_document_version(call_hash)
end

def update_document_version(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.update_document_version if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_document_version(params)
