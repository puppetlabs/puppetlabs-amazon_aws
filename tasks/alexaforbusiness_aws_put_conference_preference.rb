...
def operation_with_hash(arg_hash)
  ...
  client.put_conference_preference(call_hash)
end

def put_conference_preference(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.put_conference_preference if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_conference_preference(params)
