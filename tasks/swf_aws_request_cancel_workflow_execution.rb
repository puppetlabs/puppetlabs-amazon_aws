...
def operation_with_hash(arg_hash)
  ...
  client = Aws::SWF::Client.new(region: region)
  client.request_cancel_workflow_execution(call_hash)
end

def request_cancel_workflow_execution(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SWF::Client.new(region: region)
  response = client.request_cancel_workflow_execution if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = request_cancel_workflow_execution(params)
