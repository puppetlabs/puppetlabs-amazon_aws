...
def operation_with_hash(arg_hash)
  ...
  client.describe_snapshot_schedule(call_hash)
end

def describe_snapshot_schedule(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.describe_snapshot_schedule if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_snapshot_schedule(params)
