...
def operation_with_hash(arg_hash)
  ...
  client.start_image_builder(call_hash)
end

def start_image_builder(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AppStream::Client.new(region: region)
  response = client.start_image_builder if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_image_builder(params)
