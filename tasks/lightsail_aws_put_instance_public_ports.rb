...
def operation_with_hash(arg_hash)
  ...
  client.put_instance_public_ports(call_hash)
end

def put_instance_public_ports(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.put_instance_public_ports if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_instance_public_ports(params)
