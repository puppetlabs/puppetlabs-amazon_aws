...
def operation_with_hash(arg_hash)
  ...
  client.update_directory_config(call_hash)
end

def update_directory_config(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AppStream::Client.new(region: region)
  response = client.update_directory_config if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_directory_config(params)
