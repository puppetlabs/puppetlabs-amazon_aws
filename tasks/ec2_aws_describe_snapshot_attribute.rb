...
def operation_with_hash(arg_hash)
  ...
  client.describe_snapshot_attribute(call_hash)
end

def describe_snapshot_attribute(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_snapshot_attribute if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_snapshot_attribute(params)
