...
def operation_with_hash(arg_hash)
  ...
  client = Aws::Snowball::Client.new(region: region)
  client.list_compatible_images(call_hash)
end

def list_compatible_images(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Snowball::Client.new(region: region)
  response = client.list_compatible_images if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_compatible_images(params)
