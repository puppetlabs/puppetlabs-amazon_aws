...
def operation_with_hash(arg_hash)
  ...
  client.put_scheduled_update_group_action(call_hash)
end

def put_scheduled_update_group_action(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.put_scheduled_update_group_action if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_scheduled_update_group_action(params)
