...
def operation_with_hash(arg_hash)
  ...
  client.delete_service_action(call_hash)
end

def delete_service_action(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.delete_service_action if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_service_action(params)
