...
def operation_with_hash(arg_hash)
  ...
  client.describe_image_attribute(call_hash)
end

def describe_image_attribute(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_image_attribute if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_image_attribute(params)
