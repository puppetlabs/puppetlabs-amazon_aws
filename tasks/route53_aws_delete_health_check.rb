...
def operation_with_hash(arg_hash)
  ...
  client.delete_health_check(call_hash)
end

def delete_health_check(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.delete_health_check if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_health_check(params)
