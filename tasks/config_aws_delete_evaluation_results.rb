...
def operation_with_hash(arg_hash)
  ...
  client.delete_evaluation_results(call_hash)
end

def delete_evaluation_results(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.delete_evaluation_results if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_evaluation_results(params)
