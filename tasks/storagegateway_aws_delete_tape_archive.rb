...
def operation_with_hash(arg_hash)
  ...
  client.delete_tape_archive(call_hash)
end

def delete_tape_archive(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.delete_tape_archive if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_tape_archive(params)
