...
def operation_with_hash(arg_hash)
  ...
  client.describe_player_sessions(call_hash)
end

def describe_player_sessions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.describe_player_sessions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_player_sessions(params)
