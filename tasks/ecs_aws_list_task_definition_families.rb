...
def operation_with_hash(arg_hash)
  ...
  client.list_task_definition_families(call_hash)
end

def list_task_definition_families(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.list_task_definition_families if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_task_definition_families(params)
