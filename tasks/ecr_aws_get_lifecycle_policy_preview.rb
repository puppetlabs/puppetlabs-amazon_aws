...
def operation_with_hash(arg_hash)
  ...
  client.get_lifecycle_policy_preview(call_hash)
end

def get_lifecycle_policy_preview(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECR::Client.new(region: region)
  response = client.get_lifecycle_policy_preview if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_lifecycle_policy_preview(params)
