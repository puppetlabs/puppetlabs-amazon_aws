...
def operation_with_hash(arg_hash)
  ...
  client.describe_index_fields(call_hash)
end

def describe_index_fields(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudSearch::Client.new(region: region)
  response = client.describe_index_fields if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_index_fields(params)
