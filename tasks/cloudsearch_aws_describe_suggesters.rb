...
def operation_with_hash(arg_hash)
  ...
  client.describe_suggesters(call_hash)
end

def describe_suggesters(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudSearch::Client.new(region: region)
  response = client.describe_suggesters if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_suggesters(params)
