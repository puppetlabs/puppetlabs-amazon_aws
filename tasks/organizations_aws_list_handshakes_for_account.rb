...
def operation_with_hash(arg_hash)
  ...
  client.list_handshakes_for_account(call_hash)
end

def list_handshakes_for_account(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Organizations::Client.new(region: region)
  response = client.list_handshakes_for_account if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_handshakes_for_account(params)
