...
def operation_with_hash(arg_hash)
  ...
  client.list_playback_configurations(call_hash)
end

def list_playback_configurations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaTailor::Client.new(region: region)
  response = client.list_playback_configurations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_playback_configurations(params)
