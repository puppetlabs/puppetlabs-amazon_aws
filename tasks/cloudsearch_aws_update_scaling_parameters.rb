...
def operation_with_hash(arg_hash)
  ...
  client.update_scaling_parameters(call_hash)
end

def update_scaling_parameters(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudSearch::Client.new(region: region)
  response = client.update_scaling_parameters if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_scaling_parameters(params)
