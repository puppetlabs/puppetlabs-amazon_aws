...
def operation_with_hash(arg_hash)
  ...
  client.delete_query_logging_config(call_hash)
end

def delete_query_logging_config(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.delete_query_logging_config if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_query_logging_config(params)
