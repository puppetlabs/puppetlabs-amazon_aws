...
def operation_with_hash(arg_hash)
  ...
  client.describe_orderable_cluster_options(call_hash)
end

def describe_orderable_cluster_options(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.describe_orderable_cluster_options if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_orderable_cluster_options(params)
