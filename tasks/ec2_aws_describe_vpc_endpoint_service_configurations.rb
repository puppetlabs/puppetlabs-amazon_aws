...
def operation_with_hash(arg_hash)
  ...
  client.describe_vpc_endpoint_service_configurations(call_hash)
end

def describe_vpc_endpoint_service_configurations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_vpc_endpoint_service_configurations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_vpc_endpoint_service_configurations(params)
