...
def operation_with_hash(arg_hash)
  ...
  client.delete_relational_database_snapshot(call_hash)
end

def delete_relational_database_snapshot(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.delete_relational_database_snapshot if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_relational_database_snapshot(params)
