...
def operation_with_hash(arg_hash)
  ...
  client.list_service_specific_credentials(call_hash)
end

def list_service_specific_credentials(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.list_service_specific_credentials if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_service_specific_credentials(params)
