...
def operation_with_hash(arg_hash)
  ...
  client.list_endpoints_by_platform_application(call_hash)
end

def list_endpoints_by_platform_application(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SNS::Client.new(region: region)
  response = client.list_endpoints_by_platform_application if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_endpoints_by_platform_application(params)
