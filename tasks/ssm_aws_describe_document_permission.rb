...
def operation_with_hash(arg_hash)
  ...
  client.describe_document_permission(call_hash)
end

def describe_document_permission(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.describe_document_permission if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_document_permission(params)
