...
def operation_with_hash(arg_hash)
  ...
  client.describe_capacity_reservations(call_hash)
end

def describe_capacity_reservations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_capacity_reservations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_capacity_reservations(params)
