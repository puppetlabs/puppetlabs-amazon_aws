...
def operation_with_hash(arg_hash)
  ...
  client.describe_change_set(call_hash)
end

def describe_change_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudFormation::Client.new(region: region)
  response = client.describe_change_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_change_set(params)
