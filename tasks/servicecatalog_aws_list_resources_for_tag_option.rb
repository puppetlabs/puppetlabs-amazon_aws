...
def operation_with_hash(arg_hash)
  ...
  client.list_resources_for_tag_option(call_hash)
end

def list_resources_for_tag_option(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.list_resources_for_tag_option if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_resources_for_tag_option(params)
