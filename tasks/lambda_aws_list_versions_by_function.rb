...
def operation_with_hash(arg_hash)
  ...
  client.list_versions_by_function(call_hash)
end

def list_versions_by_function(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lambda::Client.new(region: region)
  response = client.list_versions_by_function if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_versions_by_function(params)
