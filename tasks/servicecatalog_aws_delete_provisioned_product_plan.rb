...
def operation_with_hash(arg_hash)
  ...
  client.delete_provisioned_product_plan(call_hash)
end

def delete_provisioned_product_plan(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.delete_provisioned_product_plan if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_provisioned_product_plan(params)
