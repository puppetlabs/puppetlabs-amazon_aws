...
def operation_with_hash(arg_hash)
  ...
  client.delete_public_access_block(call_hash)
end

def delete_public_access_block(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::S3::Client.new(region: region)
  response = client.delete_public_access_block if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_public_access_block(params)
