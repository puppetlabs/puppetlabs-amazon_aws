...
def operation_with_hash(arg_hash)
  ...
  client.delete_room_skill_parameter(call_hash)
end

def delete_room_skill_parameter(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.delete_room_skill_parameter if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_room_skill_parameter(params)
