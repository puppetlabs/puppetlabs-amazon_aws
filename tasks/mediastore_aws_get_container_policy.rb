...
def operation_with_hash(arg_hash)
  ...
  client.get_container_policy(call_hash)
end

def get_container_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaStore::Client.new(region: region)
  response = client.get_container_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_container_policy(params)
