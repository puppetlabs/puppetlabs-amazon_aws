...
def operation_with_hash(arg_hash)
  ...
  client.list_portfolio_access(call_hash)
end

def list_portfolio_access(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.list_portfolio_access if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_portfolio_access(params)
