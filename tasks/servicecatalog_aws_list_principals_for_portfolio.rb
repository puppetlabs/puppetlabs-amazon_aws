...
def operation_with_hash(arg_hash)
  ...
  client.list_principals_for_portfolio(call_hash)
end

def list_principals_for_portfolio(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.list_principals_for_portfolio if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_principals_for_portfolio(params)
