...
def operation_with_hash(arg_hash)
  ...
  client.delete_chap_credentials(call_hash)
end

def delete_chap_credentials(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.delete_chap_credentials if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_chap_credentials(params)
