...
def operation_with_hash(arg_hash)
  ...
  client.put_retention_configuration(call_hash)
end

def put_retention_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.put_retention_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_retention_configuration(params)
