...
def operation_with_hash(arg_hash)
  ...
  client.update_key_description(call_hash)
end

def update_key_description(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KMS::Client.new(region: region)
  response = client.update_key_description if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_key_description(params)
