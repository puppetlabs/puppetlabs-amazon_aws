...
def operation_with_hash(arg_hash)
  ...
  client.create_disk_from_snapshot(call_hash)
end

def create_disk_from_snapshot(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.create_disk_from_snapshot if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_disk_from_snapshot(params)
