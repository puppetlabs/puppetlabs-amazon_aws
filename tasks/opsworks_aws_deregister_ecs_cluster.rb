...
def operation_with_hash(arg_hash)
  ...
  client.deregister_ecs_cluster(call_hash)
end

def deregister_ecs_cluster(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::OpsWorks::Client.new(region: region)
  response = client.deregister_ecs_cluster if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = deregister_ecs_cluster(params)
