...
def operation_with_hash(arg_hash)
  ...
  client.describe_protection(call_hash)
end

def describe_protection(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Shield::Client.new(region: region)
  response = client.describe_protection if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_protection(params)
