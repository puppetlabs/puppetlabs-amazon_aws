...
def operation_with_hash(arg_hash)
  ...
  client.delete_reusable_delegation_set(call_hash)
end

def delete_reusable_delegation_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.delete_reusable_delegation_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_reusable_delegation_set(params)
