...
def operation_with_hash(arg_hash)
  ...
  client.describe_constraint(call_hash)
end

def describe_constraint(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.describe_constraint if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_constraint(params)
