...
def operation_with_hash(arg_hash)
  ...
  client.create_game_session_queue(call_hash)
end

def create_game_session_queue(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.create_game_session_queue if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_game_session_queue(params)
