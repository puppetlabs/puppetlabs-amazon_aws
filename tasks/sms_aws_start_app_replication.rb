...
def operation_with_hash(arg_hash)
  ...
  client.start_app_replication(call_hash)
end

def start_app_replication(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SMS::Client.new(region: region)
  response = client.start_app_replication if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_app_replication(params)
