...
def operation_with_hash(arg_hash)
  ...
  client.create_custom_action_type(call_hash)
end

def create_custom_action_type(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodePipeline::Client.new(region: region)
  response = client.create_custom_action_type if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_custom_action_type(params)
