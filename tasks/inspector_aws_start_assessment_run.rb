...
def operation_with_hash(arg_hash)
  ...
  client.start_assessment_run(call_hash)
end

def start_assessment_run(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.start_assessment_run if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_assessment_run(params)
