...
def operation_with_hash(arg_hash)
  ...
  client.delete_dynamic_thing_group(call_hash)
end

def delete_dynamic_thing_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.delete_dynamic_thing_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_dynamic_thing_group(params)
