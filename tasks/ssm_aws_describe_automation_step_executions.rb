...
def operation_with_hash(arg_hash)
  ...
  client.describe_automation_step_executions(call_hash)
end

def describe_automation_step_executions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.describe_automation_step_executions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_automation_step_executions(params)
