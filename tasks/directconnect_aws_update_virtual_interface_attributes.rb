...
def operation_with_hash(arg_hash)
  ...
  client.update_virtual_interface_attributes(call_hash)
end

def update_virtual_interface_attributes(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.update_virtual_interface_attributes if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_virtual_interface_attributes(params)
