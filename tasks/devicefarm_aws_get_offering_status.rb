...
def operation_with_hash(arg_hash)
  ...
  client.get_offering_status(call_hash)
end

def get_offering_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.get_offering_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_offering_status(params)
