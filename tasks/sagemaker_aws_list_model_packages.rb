...
def operation_with_hash(arg_hash)
  ...
  client.list_model_packages(call_hash)
end

def list_model_packages(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.list_model_packages if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_model_packages(params)
