...
def operation_with_hash(arg_hash)
  ...
  client.attach_instances_to_load_balancer(call_hash)
end

def attach_instances_to_load_balancer(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.attach_instances_to_load_balancer if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = attach_instances_to_load_balancer(params)
