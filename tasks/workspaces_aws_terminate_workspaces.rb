...
def operation_with_hash(arg_hash)
  ...
  client.terminate_workspaces(call_hash)
end

def terminate_workspaces(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkSpaces::Client.new(region: region)
  response = client.terminate_workspaces if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = terminate_workspaces(params)
