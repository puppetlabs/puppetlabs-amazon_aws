...
def operation_with_hash(arg_hash)
  ...
  client.get_sql_injection_match_set(call_hash)
end

def get_sql_injection_match_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.get_sql_injection_match_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_sql_injection_match_set(params)
