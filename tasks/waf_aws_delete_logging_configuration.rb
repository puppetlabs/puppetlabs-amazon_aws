...
def operation_with_hash(arg_hash)
  ...
  client.delete_logging_configuration(call_hash)
end

def delete_logging_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.delete_logging_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_logging_configuration(params)
