...
def operation_with_hash(arg_hash)
  ...
  client.update_pull_request_status(call_hash)
end

def update_pull_request_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeCommit::Client.new(region: region)
  response = client.update_pull_request_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_pull_request_status(params)
