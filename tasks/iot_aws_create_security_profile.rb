...
def operation_with_hash(arg_hash)
  ...
  client.create_security_profile(call_hash)
end

def create_security_profile(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.create_security_profile if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_security_profile(params)
