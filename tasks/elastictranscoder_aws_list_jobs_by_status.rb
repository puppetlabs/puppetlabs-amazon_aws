...
def operation_with_hash(arg_hash)
  ...
  client.list_jobs_by_status(call_hash)
end

def list_jobs_by_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticTranscoder::Client.new(region: region)
  response = client.list_jobs_by_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_jobs_by_status(params)
