...
def operation_with_hash(arg_hash)
  ...
  client.describe_data_sources(call_hash)
end

def describe_data_sources(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MachineLearning::Client.new(region: region)
  response = client.describe_data_sources if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_data_sources(params)
