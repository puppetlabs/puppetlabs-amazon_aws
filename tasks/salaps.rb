...
def operation_with_hash(arg_hash)
  ...
  client.list_accepted_portfolio_shares(call_hash)
end

def list_accepted_portfolio_shares(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.list_accepted_portfolio_shares if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_accepted_portfolio_shares(params)
