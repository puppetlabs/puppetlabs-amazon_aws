...
def operation_with_hash(arg_hash)
  ...
  client.describe_workspaces(call_hash)
end

def describe_workspaces(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkSpaces::Client.new(region: region)
  response = client.describe_workspaces if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_workspaces(params)
