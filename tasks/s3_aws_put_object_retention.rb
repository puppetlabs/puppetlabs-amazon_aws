...
def operation_with_hash(arg_hash)
  ...
  client.put_object_retention(call_hash)
end

def put_object_retention(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::S3::Client.new(region: region)
  response = client.put_object_retention if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_object_retention(params)
