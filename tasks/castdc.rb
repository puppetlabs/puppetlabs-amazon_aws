...
def operation_with_hash(arg_hash)
  ...
  client.stop_training_document_classifier(call_hash)
end

def stop_training_document_classifier(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Comprehend::Client.new(region: region)
  response = client.stop_training_document_classifier if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = stop_training_document_classifier(params)
