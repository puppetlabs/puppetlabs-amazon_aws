...
def operation_with_hash(arg_hash)
  ...
  client.delete_batch_prediction(call_hash)
end

def delete_batch_prediction(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MachineLearning::Client.new(region: region)
  response = client.delete_batch_prediction if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_batch_prediction(params)
