...
def operation_with_hash(arg_hash)
  ...
  client.create_private_dns_namespace(call_hash)
end

def create_private_dns_namespace(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceDiscovery::Client.new(region: region)
  response = client.create_private_dns_namespace if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_private_dns_namespace(params)
