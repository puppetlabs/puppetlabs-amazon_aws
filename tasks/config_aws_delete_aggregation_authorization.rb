...
def operation_with_hash(arg_hash)
  ...
  client.delete_aggregation_authorization(call_hash)
end

def delete_aggregation_authorization(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.delete_aggregation_authorization if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_aggregation_authorization(params)
