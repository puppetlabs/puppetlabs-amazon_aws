...
def operation_with_hash(arg_hash)
  ...
  client.create_dhcp_options(call_hash)
end

def create_dhcp_options(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_dhcp_options if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_dhcp_options(params)
