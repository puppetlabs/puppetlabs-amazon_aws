...
def operation_with_hash(arg_hash)
  ...
  client.create_notification_subscription(call_hash)
end

def create_notification_subscription(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.create_notification_subscription if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_notification_subscription(params)
