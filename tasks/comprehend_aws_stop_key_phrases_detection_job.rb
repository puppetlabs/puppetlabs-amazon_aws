...
def operation_with_hash(arg_hash)
  ...
  client.stop_key_phrases_detection_job(call_hash)
end

def stop_key_phrases_detection_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Comprehend::Client.new(region: region)
  response = client.stop_key_phrases_detection_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = stop_key_phrases_detection_job(params)
