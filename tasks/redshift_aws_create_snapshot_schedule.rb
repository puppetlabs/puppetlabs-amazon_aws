...
def operation_with_hash(arg_hash)
  ...
  client.create_snapshot_schedule(call_hash)
end

def create_snapshot_schedule(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.create_snapshot_schedule if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_snapshot_schedule(params)
