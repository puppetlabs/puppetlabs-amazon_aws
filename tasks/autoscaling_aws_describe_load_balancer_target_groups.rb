...
def operation_with_hash(arg_hash)
  ...
  client.describe_load_balancer_target_groups(call_hash)
end

def describe_load_balancer_target_groups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.describe_load_balancer_target_groups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_load_balancer_target_groups(params)
