...
def operation_with_hash(arg_hash)
  ...
  client.start_config_rules_evaluation(call_hash)
end

def start_config_rules_evaluation(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.start_config_rules_evaluation if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_config_rules_evaluation(params)
