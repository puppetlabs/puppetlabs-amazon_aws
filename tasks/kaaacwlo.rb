...
def operation_with_hash(arg_hash)
  ...
  client.add_application_cloud_watch_logging_option(call_hash)
end

def add_application_cloud_watch_logging_option(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KinesisAnalytics::Client.new(region: region)
  response = client.add_application_cloud_watch_logging_option if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_application_cloud_watch_logging_option(params)
