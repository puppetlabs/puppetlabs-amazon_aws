...
def operation_with_hash(arg_hash)
  ...
  client.create_origin_endpoint(call_hash)
end

def create_origin_endpoint(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaPackage::Client.new(region: region)
  response = client.create_origin_endpoint if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_origin_endpoint(params)
