...
def operation_with_hash(arg_hash)
  ...
  client.describe_assessment_runs(call_hash)
end

def describe_assessment_runs(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.describe_assessment_runs if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_assessment_runs(params)
