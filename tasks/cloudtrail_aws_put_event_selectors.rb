...
def operation_with_hash(arg_hash)
  ...
  client.put_event_selectors(call_hash)
end

def put_event_selectors(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudTrail::Client.new(region: region)
  response = client.put_event_selectors if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_event_selectors(params)
