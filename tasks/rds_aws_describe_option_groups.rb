...
def operation_with_hash(arg_hash)
  ...
  client.describe_option_groups(call_hash)
end

def describe_option_groups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.describe_option_groups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_option_groups(params)
