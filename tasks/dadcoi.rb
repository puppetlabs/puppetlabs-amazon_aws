...
def operation_with_hash(arg_hash)
  ...
  client.describe_connections_on_interconnect(call_hash)
end

def describe_connections_on_interconnect(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.describe_connections_on_interconnect if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_connections_on_interconnect(params)
