...
def operation_with_hash(arg_hash)
  ...
  client.describe_user_profiles(call_hash)
end

def describe_user_profiles(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::OpsWorks::Client.new(region: region)
  response = client.describe_user_profiles if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_user_profiles(params)
