...
def operation_with_hash(arg_hash)
  ...
  client.delete_event_source_mapping(call_hash)
end

def delete_event_source_mapping(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lambda::Client.new(region: region)
  response = client.delete_event_source_mapping if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_event_source_mapping(params)
