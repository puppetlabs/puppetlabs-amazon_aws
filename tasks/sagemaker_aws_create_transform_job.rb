...
def operation_with_hash(arg_hash)
  ...
  client.create_transform_job(call_hash)
end

def create_transform_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.create_transform_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_transform_job(params)
