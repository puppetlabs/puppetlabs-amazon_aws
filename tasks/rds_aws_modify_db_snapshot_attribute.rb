...
def operation_with_hash(arg_hash)
  ...
  client.modify_db_snapshot_attribute(call_hash)
end

def modify_db_snapshot_attribute(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.modify_db_snapshot_attribute if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_db_snapshot_attribute(params)
