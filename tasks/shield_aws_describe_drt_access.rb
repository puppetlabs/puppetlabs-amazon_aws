...
def operation_with_hash(arg_hash)
  ...
  client.describe_drt_access(call_hash)
end

def describe_drt_access(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Shield::Client.new(region: region)
  response = client.describe_drt_access if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_drt_access(params)
