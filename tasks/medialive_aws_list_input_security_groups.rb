...
def operation_with_hash(arg_hash)
  ...
  client.list_input_security_groups(call_hash)
end

def list_input_security_groups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaLive::Client.new(region: region)
  response = client.list_input_security_groups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_input_security_groups(params)
