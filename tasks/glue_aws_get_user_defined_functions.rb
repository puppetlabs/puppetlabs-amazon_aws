...
def operation_with_hash(arg_hash)
  ...
  client.get_user_defined_functions(call_hash)
end

def get_user_defined_functions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glue::Client.new(region: region)
  response = client.get_user_defined_functions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_user_defined_functions(params)
