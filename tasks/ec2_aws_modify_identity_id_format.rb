...
def operation_with_hash(arg_hash)
  ...
  client.modify_identity_id_format(call_hash)
end

def modify_identity_id_format(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.modify_identity_id_format if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_identity_id_format(params)
