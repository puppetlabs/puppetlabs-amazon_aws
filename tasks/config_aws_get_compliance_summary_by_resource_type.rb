...
def operation_with_hash(arg_hash)
  ...
  client.get_compliance_summary_by_resource_type(call_hash)
end

def get_compliance_summary_by_resource_type(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.get_compliance_summary_by_resource_type if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_compliance_summary_by_resource_type(params)
