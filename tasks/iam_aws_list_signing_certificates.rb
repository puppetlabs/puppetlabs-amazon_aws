...
def operation_with_hash(arg_hash)
  ...
  client.list_signing_certificates(call_hash)
end

def list_signing_certificates(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.list_signing_certificates if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_signing_certificates(params)
