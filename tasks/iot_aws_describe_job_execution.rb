...
def operation_with_hash(arg_hash)
  ...
  client.describe_job_execution(call_hash)
end

def describe_job_execution(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.describe_job_execution if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_job_execution(params)
