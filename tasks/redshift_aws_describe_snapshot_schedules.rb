...
def operation_with_hash(arg_hash)
  ...
  client.describe_snapshot_schedules(call_hash)
end

def describe_snapshot_schedules(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.describe_snapshot_schedules if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_snapshot_schedules(params)
