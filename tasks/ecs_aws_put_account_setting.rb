...
def operation_with_hash(arg_hash)
  ...
  client.put_account_setting(call_hash)
end

def put_account_setting(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.put_account_setting if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_account_setting(params)
