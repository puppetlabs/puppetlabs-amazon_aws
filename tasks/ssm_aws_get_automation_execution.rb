...
def operation_with_hash(arg_hash)
  ...
  client.get_automation_execution(call_hash)
end

def get_automation_execution(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.get_automation_execution if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_automation_execution(params)
