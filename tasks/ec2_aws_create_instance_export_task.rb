...
def operation_with_hash(arg_hash)
  ...
  client.create_instance_export_task(call_hash)
end

def create_instance_export_task(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_instance_export_task if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_instance_export_task(params)
