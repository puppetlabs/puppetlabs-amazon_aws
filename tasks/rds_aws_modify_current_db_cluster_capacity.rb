...
def operation_with_hash(arg_hash)
  ...
  client.modify_current_db_cluster_capacity(call_hash)
end

def modify_current_db_cluster_capacity(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.modify_current_db_cluster_capacity if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_current_db_cluster_capacity(params)
