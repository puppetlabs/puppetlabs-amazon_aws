...
def operation_with_hash(arg_hash)
  ...
  client.list_phone_numbers_opted_out(call_hash)
end

def list_phone_numbers_opted_out(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SNS::Client.new(region: region)
  response = client.list_phone_numbers_opted_out if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_phone_numbers_opted_out(params)
