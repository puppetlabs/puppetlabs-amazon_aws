...
def operation_with_hash(arg_hash)
  ...
  client.get_export_snapshot_records(call_hash)
end

def get_export_snapshot_records(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.get_export_snapshot_records if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_export_snapshot_records(params)
