...
def operation_with_hash(arg_hash)
  ...
  client.deregister_stream_consumer(call_hash)
end

def deregister_stream_consumer(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Kinesis::Client.new(region: region)
  response = client.deregister_stream_consumer if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = deregister_stream_consumer(params)
