...
def operation_with_hash(arg_hash)
  ...
  client.update_game_session_queue(call_hash)
end

def update_game_session_queue(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.update_game_session_queue if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_game_session_queue(params)
