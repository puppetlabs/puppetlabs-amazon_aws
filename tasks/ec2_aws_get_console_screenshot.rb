...
def operation_with_hash(arg_hash)
  ...
  client.get_console_screenshot(call_hash)
end

def get_console_screenshot(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.get_console_screenshot if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_console_screenshot(params)
