...
def operation_with_hash(arg_hash)
  ...
  client.delete_virtual_mfa_device(call_hash)
end

def delete_virtual_mfa_device(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.delete_virtual_mfa_device if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_virtual_mfa_device(params)
