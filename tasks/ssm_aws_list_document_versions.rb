...
def operation_with_hash(arg_hash)
  ...
  client.list_document_versions(call_hash)
end

def list_document_versions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.list_document_versions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_document_versions(params)
