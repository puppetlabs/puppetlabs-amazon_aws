...
def operation_with_hash(arg_hash)
  ...
  client.describe_resource_permissions(call_hash)
end

def describe_resource_permissions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.describe_resource_permissions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_resource_permissions(params)
