...
def operation_with_hash(arg_hash)
  ...
  client.list_service_actions(call_hash)
end

def list_service_actions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.list_service_actions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_service_actions(params)
