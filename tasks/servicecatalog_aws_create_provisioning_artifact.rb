...
def operation_with_hash(arg_hash)
  ...
  client.create_provisioning_artifact(call_hash)
end

def create_provisioning_artifact(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.create_provisioning_artifact if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_provisioning_artifact(params)
