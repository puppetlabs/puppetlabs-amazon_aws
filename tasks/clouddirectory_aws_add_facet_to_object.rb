...
def operation_with_hash(arg_hash)
  ...
  client.add_facet_to_object(call_hash)
end

def add_facet_to_object(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudDirectory::Client.new(region: region)
  response = client.add_facet_to_object if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_facet_to_object(params)
