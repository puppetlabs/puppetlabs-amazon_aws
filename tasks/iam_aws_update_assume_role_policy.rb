...
def operation_with_hash(arg_hash)
  ...
  client.update_assume_role_policy(call_hash)
end

def update_assume_role_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.update_assume_role_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_assume_role_policy(params)
