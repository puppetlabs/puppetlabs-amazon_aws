...
def operation_with_hash(arg_hash)
  ...
  client.delete_assessment_target(call_hash)
end

def delete_assessment_target(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.delete_assessment_target if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_assessment_target(params)
