...
def operation_with_hash(arg_hash)
  ...
  client.add_working_storage(call_hash)
end

def add_working_storage(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.add_working_storage if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_working_storage(params)
