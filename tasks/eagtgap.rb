...
def operation_with_hash(arg_hash)
  ...
  client.get_transit_gateway_attachment_propagations(call_hash)
end

def get_transit_gateway_attachment_propagations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.get_transit_gateway_attachment_propagations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_transit_gateway_attachment_propagations(params)
