...
def operation_with_hash(arg_hash)
  ...
  client.create_address_book(call_hash)
end

def create_address_book(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.create_address_book if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_address_book(params)
