...
def operation_with_hash(arg_hash)
  ...
  client.get_reusable_delegation_set(call_hash)
end

def get_reusable_delegation_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.get_reusable_delegation_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_reusable_delegation_set(params)
