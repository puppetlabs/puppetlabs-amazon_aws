...
def operation_with_hash(arg_hash)
  ...
  client.modify_global_cluster(call_hash)
end

def modify_global_cluster(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.modify_global_cluster if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_global_cluster(params)
