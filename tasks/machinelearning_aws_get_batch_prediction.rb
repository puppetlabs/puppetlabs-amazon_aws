...
def operation_with_hash(arg_hash)
  ...
  client.get_batch_prediction(call_hash)
end

def get_batch_prediction(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MachineLearning::Client.new(region: region)
  response = client.get_batch_prediction if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_batch_prediction(params)
