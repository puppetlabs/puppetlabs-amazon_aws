...
def operation_with_hash(arg_hash)
  ...
  client.describe_config_rule_evaluation_status(call_hash)
end

def describe_config_rule_evaluation_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.describe_config_rule_evaluation_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_config_rule_evaluation_status(params)
