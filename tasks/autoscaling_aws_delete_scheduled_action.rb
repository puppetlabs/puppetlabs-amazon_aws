...
def operation_with_hash(arg_hash)
  ...
  client.delete_scheduled_action(call_hash)
end

def delete_scheduled_action(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.delete_scheduled_action if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_scheduled_action(params)
