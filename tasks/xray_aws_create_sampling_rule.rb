...
def operation_with_hash(arg_hash)
  ...
  client.create_sampling_rule(call_hash)
end

def create_sampling_rule(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::XRay::Client.new(region: region)
  response = client.create_sampling_rule if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_sampling_rule(params)
