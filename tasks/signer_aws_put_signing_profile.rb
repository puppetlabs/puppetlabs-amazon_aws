...
def operation_with_hash(arg_hash)
  ...
  client.put_signing_profile(call_hash)
end

def put_signing_profile(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Signer::Client.new(region: region)
  response = client.put_signing_profile if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_signing_profile(params)
