...
def operation_with_hash(arg_hash)
  ...
  client.get_federation_token(call_hash)
end

def get_federation_token(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::STS::Client.new(region: region)
  response = client.get_federation_token if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_federation_token(params)
