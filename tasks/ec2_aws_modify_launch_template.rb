...
def operation_with_hash(arg_hash)
  ...
  client.modify_launch_template(call_hash)
end

def modify_launch_template(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.modify_launch_template if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_launch_template(params)
