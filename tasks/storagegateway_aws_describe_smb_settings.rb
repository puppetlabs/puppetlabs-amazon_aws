...
def operation_with_hash(arg_hash)
  ...
  client.describe_smb_settings(call_hash)
end

def describe_smb_settings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.describe_smb_settings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_smb_settings(params)
