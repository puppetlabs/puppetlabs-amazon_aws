...
def operation_with_hash(arg_hash)
  ...
  client.get_speech_synthesis_task(call_hash)
end

def get_speech_synthesis_task(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Polly::Client.new(region: region)
  response = client.get_speech_synthesis_task if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_speech_synthesis_task(params)
