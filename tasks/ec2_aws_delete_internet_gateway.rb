...
def operation_with_hash(arg_hash)
  ...
  client.delete_internet_gateway(call_hash)
end

def delete_internet_gateway(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.delete_internet_gateway if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_internet_gateway(params)
