...
def operation_with_hash(arg_hash)
  ...
  client.modify_network_interface_attribute(call_hash)
end
...
def modify_network_interface_attribute(*args)
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.modify_network_interface_attribute if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_network_interface_attribute(params)
