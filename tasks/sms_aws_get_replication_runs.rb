...
def operation_with_hash(arg_hash)
  ...
  client.get_replication_runs(call_hash)
end

def get_replication_runs(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SMS::Client.new(region: region)
  response = client.get_replication_runs if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_replication_runs(params)
