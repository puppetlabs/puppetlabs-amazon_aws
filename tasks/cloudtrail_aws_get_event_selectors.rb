...
def operation_with_hash(arg_hash)
  ...
  client.get_event_selectors(call_hash)
end

def get_event_selectors(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudTrail::Client.new(region: region)
  response = client.get_event_selectors if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_event_selectors(params)
