...
def operation_with_hash(arg_hash)
  ...
  client.run_scheduled_instances(call_hash)
end

def run_scheduled_instances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.run_scheduled_instances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = run_scheduled_instances(params)
