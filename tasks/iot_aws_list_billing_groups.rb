...
def operation_with_hash(arg_hash)
  ...
  client.list_billing_groups(call_hash)
end

def list_billing_groups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.list_billing_groups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_billing_groups(params)
