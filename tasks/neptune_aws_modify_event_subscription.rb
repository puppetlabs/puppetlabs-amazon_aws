...
def operation_with_hash(arg_hash)
  ...
  client.modify_event_subscription(call_hash)
end

def modify_event_subscription(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Neptune::Client.new(region: region)
  response = client.modify_event_subscription if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_event_subscription(params)
