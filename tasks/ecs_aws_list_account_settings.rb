...
def operation_with_hash(arg_hash)
  ...
  client.list_account_settings(call_hash)
end

def list_account_settings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.list_account_settings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_account_settings(params)
