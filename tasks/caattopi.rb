...
def operation_with_hash(arg_hash)
  ...
  client.add_tags_to_on_premises_instances(call_hash)
end

def add_tags_to_on_premises_instances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeDeploy::Client.new(region: region)
  response = client.add_tags_to_on_premises_instances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_tags_to_on_premises_instances(params)
