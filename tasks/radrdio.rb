...
def operation_with_hash(arg_hash)
  ...
  client.describe_reserved_db_instances_offerings(call_hash)
end

def describe_reserved_db_instances_offerings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.describe_reserved_db_instances_offerings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_reserved_db_instances_offerings(params)
