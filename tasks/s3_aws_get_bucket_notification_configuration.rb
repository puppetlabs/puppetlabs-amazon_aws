...
def operation_with_hash(arg_hash)
  ...
  client.get_bucket_notification_configuration(call_hash)
end

def get_bucket_notification_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::S3::Client.new(region: region)
  response = client.get_bucket_notification_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_bucket_notification_configuration(params)
