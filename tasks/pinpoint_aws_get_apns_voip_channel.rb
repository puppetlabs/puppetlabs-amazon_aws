...
def operation_with_hash(arg_hash)
  ...
  client.get_apns_voip_channel(call_hash)
end

def get_apns_voip_channel(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Pinpoint::Client.new(region: region)
  response = client.get_apns_voip_channel if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_apns_voip_channel(params)
