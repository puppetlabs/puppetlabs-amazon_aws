...
def operation_with_hash(arg_hash)
  ...
  client.delete_client_vpn_route(call_hash)
end

def delete_client_vpn_route(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.delete_client_vpn_route if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_client_vpn_route(params)
