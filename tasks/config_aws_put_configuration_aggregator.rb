...
def operation_with_hash(arg_hash)
  ...
  client.put_configuration_aggregator(call_hash)
end

def put_configuration_aggregator(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.put_configuration_aggregator if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_configuration_aggregator(params)
