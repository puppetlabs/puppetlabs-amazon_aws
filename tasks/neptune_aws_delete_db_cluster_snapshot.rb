...
def operation_with_hash(arg_hash)
  ...
  client.delete_db_cluster_snapshot(call_hash)
end

def delete_db_cluster_snapshot(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Neptune::Client.new(region: region)
  response = client.delete_db_cluster_snapshot if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_db_cluster_snapshot(params)
