...
def operation_with_hash(arg_hash)
  ...
  client = Aws::GameLift::Client.new(region: region)
  client.describe_matchmaking_configurations(call_hash)
end

def describe_matchmaking_configurations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.describe_matchmaking_configurations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_matchmaking_configurations(params)
