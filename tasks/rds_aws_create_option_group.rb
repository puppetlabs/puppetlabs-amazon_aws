...
def operation_with_hash(arg_hash)
  ...
  client.create_option_group(call_hash)
end

def create_option_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.create_option_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_option_group(params)
