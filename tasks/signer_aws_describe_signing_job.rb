...
def operation_with_hash(arg_hash)
  ...
  client = Aws::Signer::Client.new(region: region)
  client.describe_signing_job(call_hash)
end

def describe_signing_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Signer::Client.new(region: region)
  response = client.describe_signing_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_signing_job(params)
