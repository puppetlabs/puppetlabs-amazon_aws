...
def operation_with_hash(arg_hash)
  ...
  client.delete_state_machine(call_hash)
end

def delete_state_machine(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::States::Client.new(region: region)
  response = client.delete_state_machine if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_state_machine(params)
