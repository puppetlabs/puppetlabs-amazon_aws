...
def operation_with_hash(arg_hash)
  ...
  client.get_crawler_metrics(call_hash)
end

def get_crawler_metrics(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glue::Client.new(region: region)
  response = client.get_crawler_metrics if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_crawler_metrics(params)
