...
def operation_with_hash(arg_hash)
  ...
  client.create_platform_endpoint(call_hash)
end

def create_platform_endpoint(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SNS::Client.new(region: region)
  response = client.create_platform_endpoint if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_platform_endpoint(params)
