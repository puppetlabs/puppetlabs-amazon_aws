...
def operation_with_hash(arg_hash)
  ...
  client.list_tags_for_project(call_hash)
end

def list_tags_for_project(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeStar::Client.new(region: region)
  response = client.list_tags_for_project if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_tags_for_project(params)
