...
def operation_with_hash(arg_hash)
  ...
  client.create_association_batch(call_hash)
end

def create_association_batch(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.create_association_batch if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_association_batch(params)
