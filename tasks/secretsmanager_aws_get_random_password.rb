...
def operation_with_hash(arg_hash)
  ...
  client.get_random_password(call_hash)
end

def get_random_password(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SecretsManager::Client.new(region: region)
  response = client.get_random_password if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_random_password(params)
