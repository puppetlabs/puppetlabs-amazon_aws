...
def operation_with_hash(arg_hash)
  ...
  client = Aws::Rekognition::Client.new(region: region)
  client.start_celebrity_recognition(call_hash)
end

def start_celebrity_recognition(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Rekognition::Client.new(region: region)
  response = client.start_celebrity_recognition if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_celebrity_recognition(params)
