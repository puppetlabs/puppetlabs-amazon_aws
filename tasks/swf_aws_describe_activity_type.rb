...
def operation_with_hash(arg_hash)
  ...
  client.describe_activity_type(call_hash)
end

def describe_activity_type(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SWF::Client.new(region: region)
  response = client.describe_activity_type if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_activity_type(params)
