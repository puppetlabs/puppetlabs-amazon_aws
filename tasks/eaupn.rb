...
def operation_with_hash(arg_hash)
  ...
  client.update_pipeline_notifications(call_hash)
end

def update_pipeline_notifications(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticTranscoder::Client.new(region: region)
  response = client.update_pipeline_notifications if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_pipeline_notifications(params)
