...
def operation_with_hash(arg_hash)
  ...
  client.get_aggregate_resource_config(call_hash)
end

def get_aggregate_resource_config(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.get_aggregate_resource_config if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_aggregate_resource_config(params)
