...
def operation_with_hash(arg_hash)
  ...
  client.delete_workspace_image(call_hash)
end

def delete_workspace_image(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkSpaces::Client.new(region: region)
  response = client.delete_workspace_image if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_workspace_image(params)
