...
def operation_with_hash(arg_hash)
  ...
  client.get_campaign_version(call_hash)
end

def get_campaign_version(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Pinpoint::Client.new(region: region)
  response = client.get_campaign_version if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_campaign_version(params)
