...
def operation_with_hash(arg_hash)
  ...
  client.describe_permissions(call_hash)
end

def describe_permissions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::OpsWorks::Client.new(region: region)
  response = client.describe_permissions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_permissions(params)
