...
def operation_with_hash(arg_hash)
  ...
  client.get_bulk_deployment_status(call_hash)
end

def get_bulk_deployment_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.get_bulk_deployment_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_bulk_deployment_status(params)
