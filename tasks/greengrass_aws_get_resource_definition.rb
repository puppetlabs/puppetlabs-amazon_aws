...
def operation_with_hash(arg_hash)
  ...
  client.get_resource_definition(call_hash)
end

def get_resource_definition(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.get_resource_definition if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_resource_definition(params)
