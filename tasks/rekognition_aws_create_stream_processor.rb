...
def operation_with_hash(arg_hash)
  ...
  client.create_stream_processor(call_hash)
end

def create_stream_processor(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Rekognition::Client.new(region: region)
  response = client.create_stream_processor if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_stream_processor(params)
