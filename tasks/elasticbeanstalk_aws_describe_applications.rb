...
def operation_with_hash(arg_hash)
  ...
  client.describe_applications(call_hash)
end

def describe_applications(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.describe_applications if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_applications(params)
