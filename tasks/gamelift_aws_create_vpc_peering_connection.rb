...
def operation_with_hash(arg_hash)
  ...
  client.create_vpc_peering_connection(call_hash)
end

def create_vpc_peering_connection(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.create_vpc_peering_connection if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_vpc_peering_connection(params)
