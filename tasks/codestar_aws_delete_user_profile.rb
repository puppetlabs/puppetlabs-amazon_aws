...
def operation_with_hash(arg_hash)
  ...
  client.delete_user_profile(call_hash)
end

def delete_user_profile(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeStar::Client.new(region: region)
  response = client.delete_user_profile if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_user_profile(params)
