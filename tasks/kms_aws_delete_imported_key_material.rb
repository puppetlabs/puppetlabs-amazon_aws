...
def operation_with_hash(arg_hash)
  ...
  client.delete_imported_key_material(call_hash)
end

def delete_imported_key_material(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KMS::Client.new(region: region)
  response = client.delete_imported_key_material if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_imported_key_material(params)
