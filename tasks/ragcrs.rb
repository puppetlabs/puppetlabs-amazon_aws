...
def operation_with_hash(arg_hash)
  ...
  client = Aws::Route53Domains::Client.new(region: region)
  client.get_contact_reachability_status(call_hash)
end

def get_contact_reachability_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53Domains::Client.new(region: region)
  response = client.get_contact_reachability_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_contact_reachability_status(params)
