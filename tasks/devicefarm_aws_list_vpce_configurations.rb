...
def operation_with_hash(arg_hash)
  ...
  client.list_vpce_configurations(call_hash)
end

def list_vpce_configurations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.list_vpce_configurations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_vpce_configurations(params)
