...
def operation_with_hash(arg_hash)
  ...
  client.create_conference_provider(call_hash)
end

def create_conference_provider(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.create_conference_provider if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_conference_provider(params)
