...
def operation_with_hash(arg_hash)
  ...
  client.start_lifecycle_policy_preview(call_hash)
end

def start_lifecycle_policy_preview(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECR::Client.new(region: region)
  response = client.start_lifecycle_policy_preview if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_lifecycle_policy_preview(params)
