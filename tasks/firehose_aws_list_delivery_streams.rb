...
def operation_with_hash(arg_hash)
  ...
  client.list_delivery_streams(call_hash)
end

def list_delivery_streams(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Firehose::Client.new(region: region)
  response = client.list_delivery_streams if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_delivery_streams(params)
