...
def operation_with_hash(arg_hash)
  ...
  client = Aws::CodePipeline::Client.new(region: region)
  client.deregister_webhook_with_third_party(call_hash)
end

def deregister_webhook_with_third_party(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodePipeline::Client.new(region: region)
  response = client.deregister_webhook_with_third_party if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = deregister_webhook_with_third_party(params)
