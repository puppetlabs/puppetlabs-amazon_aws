...
def operation_with_hash(arg_hash)
  ...
  client.list_assessment_run_agents(call_hash)
end

def list_assessment_run_agents(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.list_assessment_run_agents if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_assessment_run_agents(params)
