...
def operation_with_hash(arg_hash)
  ...
  client.put_mailbox_permissions(call_hash)
end

def put_mailbox_permissions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkMail::Client.new(region: region)
  response = client.put_mailbox_permissions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_mailbox_permissions(params)
