...
def operation_with_hash(arg_hash)
  ...
  client = Aws::WorkMail::Client.new(region: region)
  client.update_primary_email_address(call_hash)
end

def update_primary_email_address(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkMail::Client.new(region: region)
  response = client.update_primary_email_address if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_primary_email_address(params)
