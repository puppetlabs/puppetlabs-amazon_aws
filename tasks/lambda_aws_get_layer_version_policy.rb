...
def operation_with_hash(arg_hash)
  ...
  client.get_layer_version_policy(call_hash)
end

def get_layer_version_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lambda::Client.new(region: region)
  response = client.get_layer_version_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_layer_version_policy(params)
