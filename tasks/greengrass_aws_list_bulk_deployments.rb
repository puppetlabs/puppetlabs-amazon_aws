...
def operation_with_hash(arg_hash)
  ...
  client.list_bulk_deployments(call_hash)
end

def list_bulk_deployments(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.list_bulk_deployments if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_bulk_deployments(params)
