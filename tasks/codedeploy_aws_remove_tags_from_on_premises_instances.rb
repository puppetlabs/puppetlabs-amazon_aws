...
def operation_with_hash(arg_hash)
  ...
  client.remove_tags_from_on_premises_instances(call_hash)
end

def remove_tags_from_on_premises_instances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeDeploy::Client.new(region: region)
  response = client.remove_tags_from_on_premises_instances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = remove_tags_from_on_premises_instances(params)
