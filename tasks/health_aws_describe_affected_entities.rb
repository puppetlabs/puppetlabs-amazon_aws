...
def operation_with_hash(arg_hash)
  ...
  client.describe_affected_entities(call_hash)
end

def describe_affected_entities(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Health::Client.new(region: region)
  response = client.describe_affected_entities if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_affected_entities(params)
