...
def operation_with_hash(arg_hash)
  ...
  client.delete_matchmaking_rule_set(call_hash)
end

def delete_matchmaking_rule_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.delete_matchmaking_rule_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_matchmaking_rule_set(params)
