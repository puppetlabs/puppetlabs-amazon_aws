...
def operation_with_hash(arg_hash)
  ...
  client.put_role_permissions_boundary(call_hash)
end

def put_role_permissions_boundary(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.put_role_permissions_boundary if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_role_permissions_boundary(params)
