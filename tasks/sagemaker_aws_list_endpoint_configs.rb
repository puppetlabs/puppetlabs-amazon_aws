...
def operation_with_hash(arg_hash)
  ...
  client.list_endpoint_configs(call_hash)
end

def list_endpoint_configs(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.list_endpoint_configs if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_endpoint_configs(params)
