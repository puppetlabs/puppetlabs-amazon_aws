...
def operation_with_hash(arg_hash)
  ...
  client.list_source_credentials(call_hash)
end

def list_source_credentials(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeBuild::Client.new(region: region)
  response = client.list_source_credentials if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_source_credentials(params)
