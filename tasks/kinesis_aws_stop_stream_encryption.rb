...
def operation_with_hash(arg_hash)
  ...
  client.stop_stream_encryption(call_hash)
end

def stop_stream_encryption(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Kinesis::Client.new(region: region)
  response = client.stop_stream_encryption if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = stop_stream_encryption(params)
