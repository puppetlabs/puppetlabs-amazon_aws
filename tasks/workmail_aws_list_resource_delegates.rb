...
def operation_with_hash(arg_hash)
  ...
  client.list_resource_delegates(call_hash)
end

def list_resource_delegates(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkMail::Client.new(region: region)
  response = client.list_resource_delegates if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_resource_delegates(params)
