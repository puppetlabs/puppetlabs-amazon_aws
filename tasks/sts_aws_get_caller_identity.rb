...
def operation_with_hash(arg_hash)
  ...
  client.get_caller_identity(call_hash)
end

def get_caller_identity(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::STS::Client.new(region: region)
  response = client.get_caller_identity if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_caller_identity(params)
