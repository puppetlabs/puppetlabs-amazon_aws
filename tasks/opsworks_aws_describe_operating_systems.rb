...
def operation_with_hash(arg_hash)
  ...
  client.describe_operating_systems(call_hash)
end

def describe_operating_systems(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::OpsWorks::Client.new(region: region)
  response = client.describe_operating_systems if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_operating_systems(params)
