...
def operation_with_hash(arg_hash)
  ...
  client = Aws::Inspector::Client.new(region: region)
  client.describe_cross_account_access_role(call_hash)
end

def describe_cross_account_access_role(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.describe_cross_account_access_role if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_cross_account_access_role(params)
