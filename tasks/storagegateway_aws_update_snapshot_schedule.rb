...
def operation_with_hash(arg_hash)
  ...
  client.update_snapshot_schedule(call_hash)
end

def update_snapshot_schedule(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.update_snapshot_schedule if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_snapshot_schedule(params)
