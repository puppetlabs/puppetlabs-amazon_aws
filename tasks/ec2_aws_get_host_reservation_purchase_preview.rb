...
def operation_with_hash(arg_hash)
  ...
  client.get_host_reservation_purchase_preview(call_hash)
end

def get_host_reservation_purchase_preview(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.get_host_reservation_purchase_preview if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_host_reservation_purchase_preview(params)
