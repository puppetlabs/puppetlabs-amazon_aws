...
def operation_with_hash(arg_hash)
  ...
  client.get_label_detection(call_hash)
end

def get_label_detection(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Rekognition::Client.new(region: region)
  response = client.get_label_detection if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_label_detection(params)
