...
def operation_with_hash(arg_hash)
  ...
  client.update_container_instances_state(call_hash)
end

def update_container_instances_state(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.update_container_instances_state if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_container_instances_state(params)
