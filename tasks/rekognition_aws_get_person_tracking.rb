...
def operation_with_hash(arg_hash)
  ...
  client.get_person_tracking(call_hash)
end

def get_person_tracking(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Rekognition::Client.new(region: region)
  response = client.get_person_tracking if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_person_tracking(params)
