...
def operation_with_hash(arg_hash)
  ...
  client.delete_launch_configuration(call_hash)
end

def delete_launch_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.delete_launch_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_launch_configuration(params)
