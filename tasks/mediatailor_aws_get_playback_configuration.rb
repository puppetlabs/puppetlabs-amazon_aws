...
def operation_with_hash(arg_hash)
  ...
  client.get_playback_configuration(call_hash)
end

def get_playback_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaTailor::Client.new(region: region)
  response = client.get_playback_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_playback_configuration(params)
