...
def operation_with_hash(arg_hash)
  ...
  client.add_attributes_to_findings(call_hash)
end

def add_attributes_to_findings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.add_attributes_to_findings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_attributes_to_findings(params)
