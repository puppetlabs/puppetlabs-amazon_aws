...
def operation_with_hash(arg_hash)
  ...
  client.describe_aggregate_compliance_by_config_rules(call_hash)
end

def describe_aggregate_compliance_by_config_rules(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.describe_aggregate_compliance_by_config_rules if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_aggregate_compliance_by_config_rules(params)
