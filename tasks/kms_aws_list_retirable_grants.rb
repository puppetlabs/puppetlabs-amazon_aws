...
def operation_with_hash(arg_hash)
  ...
  client.list_retirable_grants(call_hash)
end

def list_retirable_grants(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KMS::Client.new(region: region)
  response = client.list_retirable_grants if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_retirable_grants(params)
