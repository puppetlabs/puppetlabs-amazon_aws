...
def operation_with_hash(arg_hash)
  ...
  client.create_capacity_reservation(call_hash)
end

def create_capacity_reservation(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_capacity_reservation if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_capacity_reservation(params)
