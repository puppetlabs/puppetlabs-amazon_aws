...
def operation_with_hash(arg_hash)
  ...
  client.get_credential_report(call_hash)
end

def get_credential_report(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.get_credential_report if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_credential_report(params)
