...
def operation_with_hash(arg_hash)
  ...
  client.describe_endpoint_config(call_hash)
end

def describe_endpoint_config(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.describe_endpoint_config if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_endpoint_config(params)
