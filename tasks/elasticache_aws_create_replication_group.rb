...
def operation_with_hash(arg_hash)
  ...
  client.create_replication_group(call_hash)
end

def create_replication_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElastiCache::Client.new(region: region)
  response = client.create_replication_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_replication_group(params)
