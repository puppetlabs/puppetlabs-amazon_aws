...
def operation_with_hash(arg_hash)
  ...
  client.describe_hosted_connections(call_hash)
end

def describe_hosted_connections(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.describe_hosted_connections if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_hosted_connections(params)
