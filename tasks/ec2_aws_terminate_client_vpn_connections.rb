...
def operation_with_hash(arg_hash)
  ...
  client.terminate_client_vpn_connections(call_hash)
end

def terminate_client_vpn_connections(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.terminate_client_vpn_connections if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = terminate_client_vpn_connections(params)
