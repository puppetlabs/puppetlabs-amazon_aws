...
def operation_with_hash(arg_hash)
  ...
  client.delete_table_version(call_hash)
end

def delete_table_version(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glue::Client.new(region: region)
  response = client.delete_table_version if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_table_version(params)
