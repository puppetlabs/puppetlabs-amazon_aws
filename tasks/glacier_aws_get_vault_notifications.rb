...
def operation_with_hash(arg_hash)
  ...
  client.get_vault_notifications(call_hash)
end

def get_vault_notifications(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glacier::Client.new(region: region)
  response = client.get_vault_notifications if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_vault_notifications(params)
