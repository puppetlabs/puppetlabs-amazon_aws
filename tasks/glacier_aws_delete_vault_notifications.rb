...
def operation_with_hash(arg_hash)
  ...
  client.delete_vault_notifications(call_hash)
end

def delete_vault_notifications(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glacier::Client.new(region: region)
  response = client.delete_vault_notifications if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_vault_notifications(params)
