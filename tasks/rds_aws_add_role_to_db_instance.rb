...
def operation_with_hash(arg_hash)
  ...
  client.add_role_to_db_instance(call_hash)
end

def add_role_to_db_instance(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.add_role_to_db_instance if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_role_to_db_instance(params)
