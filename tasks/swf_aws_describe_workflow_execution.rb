...
def operation_with_hash(arg_hash)
  ...
  client.describe_workflow_execution(call_hash)
end

def describe_workflow_execution(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SWF::Client.new(region: region)
  response = client.describe_workflow_execution if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_workflow_execution(params)
