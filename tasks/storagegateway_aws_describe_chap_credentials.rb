...
def operation_with_hash(arg_hash)
  ...
  client.describe_chap_credentials(call_hash)
end

def describe_chap_credentials(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.describe_chap_credentials if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_chap_credentials(params)
