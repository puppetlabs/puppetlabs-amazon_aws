...
def operation_with_hash(arg_hash)
  ...
  client.put_permission_policy(call_hash)
end

def put_permission_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.put_permission_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_permission_policy(params)
