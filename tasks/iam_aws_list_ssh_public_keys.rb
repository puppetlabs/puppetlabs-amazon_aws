...
def operation_with_hash(arg_hash)
  ...
  client.list_ssh_public_keys(call_hash)
end

def list_ssh_public_keys(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.list_ssh_public_keys if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_ssh_public_keys(params)
