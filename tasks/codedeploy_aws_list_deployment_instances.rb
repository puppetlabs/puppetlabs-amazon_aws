...
def operation_with_hash(arg_hash)
  ...
  client.list_deployment_instances(call_hash)
end

def list_deployment_instances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeDeploy::Client.new(region: region)
  response = client.list_deployment_instances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_deployment_instances(params)
