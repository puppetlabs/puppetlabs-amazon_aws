...
def operation_with_hash(arg_hash)
  ...
  client.update_relational_database_parameters(call_hash)
end

def update_relational_database_parameters(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.update_relational_database_parameters if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_relational_database_parameters(params)
