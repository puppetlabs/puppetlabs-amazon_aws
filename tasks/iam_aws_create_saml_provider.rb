...
def operation_with_hash(arg_hash)
  ...
  client.create_saml_provider(call_hash)
end

def create_saml_provider(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.create_saml_provider if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_saml_provider(params)
