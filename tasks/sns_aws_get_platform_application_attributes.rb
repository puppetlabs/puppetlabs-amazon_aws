...
def operation_with_hash(arg_hash)
  ...
  client.get_platform_application_attributes(call_hash)
end

def get_platform_application_attributes(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SNS::Client.new(region: region)
  response = client.get_platform_application_attributes if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_platform_application_attributes(params)
