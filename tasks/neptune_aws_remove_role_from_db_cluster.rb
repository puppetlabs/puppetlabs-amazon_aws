...
def operation_with_hash(arg_hash)
  ...
  client.remove_role_from_db_cluster(call_hash)
end

def remove_role_from_db_cluster(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Neptune::Client.new(region: region)
  response = client.remove_role_from_db_cluster if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = remove_role_from_db_cluster(params)
