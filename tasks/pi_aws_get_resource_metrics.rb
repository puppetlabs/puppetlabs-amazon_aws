...
def operation_with_hash(arg_hash)
  ...
  client.get_resource_metrics(call_hash)
end

def get_resource_metrics(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::PI::Client.new(region: region)
  response = client.get_resource_metrics if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_resource_metrics(params)
