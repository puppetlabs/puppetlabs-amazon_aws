...
def operation_with_hash(arg_hash)
  ...
  client.delete_analysis_scheme(call_hash)
end

def delete_analysis_scheme(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudSearch::Client.new(region: region)
  response = client.delete_analysis_scheme if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_analysis_scheme(params)
