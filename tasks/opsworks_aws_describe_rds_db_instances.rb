...
def operation_with_hash(arg_hash)
  ...
  client.describe_rds_db_instances(call_hash)
end

def describe_rds_db_instances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::OpsWorks::Client.new(region: region)
  response = client.describe_rds_db_instances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_rds_db_instances(params)
