...
def operation_with_hash(arg_hash)
  ...
  client.create_custom_metadata(call_hash)
end

def create_custom_metadata(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.create_custom_metadata if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_custom_metadata(params)
