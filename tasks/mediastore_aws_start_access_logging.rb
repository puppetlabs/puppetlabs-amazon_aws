...
def operation_with_hash(arg_hash)
  ...
  client.start_access_logging(call_hash)
end

def start_access_logging(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaStore::Client.new(region: region)
  response = client.start_access_logging if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_access_logging(params)
