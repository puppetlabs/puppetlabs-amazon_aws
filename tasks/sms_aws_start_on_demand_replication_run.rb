...
def operation_with_hash(arg_hash)
  ...
  client.start_on_demand_replication_run(call_hash)
end

def start_on_demand_replication_run(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SMS::Client.new(region: region)
  response = client.start_on_demand_replication_run if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_on_demand_replication_run(params)
