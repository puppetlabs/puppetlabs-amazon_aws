...
def operation_with_hash(arg_hash)
  ...
  client.get_game_session_log_url(call_hash)
end

def get_game_session_log_url(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.get_game_session_log_url if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_game_session_log_url(params)
