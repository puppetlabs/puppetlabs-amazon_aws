...
def operation_with_hash(arg_hash)
  ...
  client.create_service_specific_credential(call_hash)
end

def create_service_specific_credential(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.create_service_specific_credential if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_service_specific_credential(params)
