...
def operation_with_hash(arg_hash)
  ...
  client.list_query_executions(call_hash)
end

def list_query_executions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Athena::Client.new(region: region)
  response = client.list_query_executions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_query_executions(params)
