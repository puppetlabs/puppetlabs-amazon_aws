...
def operation_with_hash(arg_hash)
  ...
  client.describe_direct_connect_gateway_associations(call_hash)
end

def describe_direct_connect_gateway_associations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.describe_direct_connect_gateway_associations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_direct_connect_gateway_associations(params)
