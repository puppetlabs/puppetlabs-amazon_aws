...
def operation_with_hash(arg_hash)
  ...
  client.describe_patch_groups(call_hash)
end

def describe_patch_groups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.describe_patch_groups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_patch_groups(params)
