...
def operation_with_hash(arg_hash)
  ...
  client.delete_bucket_inventory_configuration(call_hash)
end

def delete_bucket_inventory_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::S3::Client.new(region: region)
  response = client.delete_bucket_inventory_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_bucket_inventory_configuration(params)
