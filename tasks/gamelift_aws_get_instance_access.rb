...
def operation_with_hash(arg_hash)
  ...
  client.get_instance_access(call_hash)
end

def get_instance_access(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.get_instance_access if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_instance_access(params)
