...
def operation_with_hash(arg_hash)
  ...
  client.describe_scheduled_audit(call_hash)
end

def describe_scheduled_audit(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.describe_scheduled_audit if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_scheduled_audit(params)
