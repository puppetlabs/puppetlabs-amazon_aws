...
def operation_with_hash(arg_hash)
  ...
  client = Aws::IoTAnalytics::Client.new(region: region)
  client.start_pipeline_reprocessing(call_hash)
end

def start_pipeline_reprocessing(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoTAnalytics::Client.new(region: region)
  response = client.start_pipeline_reprocessing if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_pipeline_reprocessing(params)
