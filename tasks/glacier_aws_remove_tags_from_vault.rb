...
def operation_with_hash(arg_hash)
  ...
  client.remove_tags_from_vault(call_hash)
end

def remove_tags_from_vault(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glacier::Client.new(region: region)
  response = client.remove_tags_from_vault if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = remove_tags_from_vault(params)
