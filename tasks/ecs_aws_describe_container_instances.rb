...
def operation_with_hash(arg_hash)
  ...
  client.describe_container_instances(call_hash)
end

def describe_container_instances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.describe_container_instances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_container_instances(params)
