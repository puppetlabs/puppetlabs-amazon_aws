...
def operation_with_hash(arg_hash)
  ...
  client.describe_instance_information(call_hash)
end

def describe_instance_information(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.describe_instance_information if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_instance_information(params)
