...
def operation_with_hash(arg_hash)
  ...
  client.delete_logger_definition(call_hash)
end

def delete_logger_definition(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.delete_logger_definition if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_logger_definition(params)
