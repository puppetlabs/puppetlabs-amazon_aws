...
def operation_with_hash(arg_hash)
  ...
  client.update_termination_protection(call_hash)
end

def update_termination_protection(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudFormation::Client.new(region: region)
  response = client.update_termination_protection if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_termination_protection(params)
