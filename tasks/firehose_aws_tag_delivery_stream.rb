...
def operation_with_hash(arg_hash)
  ...
  client.tag_delivery_stream(call_hash)
end

def tag_delivery_stream(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Firehose::Client.new(region: region)
  response = client.tag_delivery_stream if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = tag_delivery_stream(params)
