...
def operation_with_hash(arg_hash)
  ...
  client.stop_monitoring_members(call_hash)
end

def stop_monitoring_members(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GuardDuty::Client.new(region: region)
  response = client.stop_monitoring_members if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = stop_monitoring_members(params)
