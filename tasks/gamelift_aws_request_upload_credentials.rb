...
def operation_with_hash(arg_hash)
  ...
  client.request_upload_credentials(call_hash)
end

def request_upload_credentials(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.request_upload_credentials if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = request_upload_credentials(params)
