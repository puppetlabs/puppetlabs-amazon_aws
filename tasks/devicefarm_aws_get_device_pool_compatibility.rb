...
def operation_with_hash(arg_hash)
  ...
  client.get_device_pool_compatibility(call_hash)
end

def get_device_pool_compatibility(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.get_device_pool_compatibility if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_device_pool_compatibility(params)
