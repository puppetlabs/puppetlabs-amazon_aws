...
def operation_with_hash(arg_hash)
  ...
  client.list_entity_recognizers(call_hash)
end

def list_entity_recognizers(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Comprehend::Client.new(region: region)
  response = client.list_entity_recognizers if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_entity_recognizers(params)
