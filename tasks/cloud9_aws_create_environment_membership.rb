...
def operation_with_hash(arg_hash)
  ...
  client.create_environment_membership(call_hash)
end

def create_environment_membership(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Cloud9::Client.new(region: region)
  response = client.create_environment_membership if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_environment_membership(params)
