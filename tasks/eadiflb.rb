...
def operation_with_hash(arg_hash)
  ...
  client.deregister_instances_from_load_balancer(call_hash)
end

def deregister_instances_from_load_balancer(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticLoadBalancingV2::Client.new(region: region)
  response = client.deregister_instances_from_load_balancer if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = deregister_instances_from_load_balancer(params)
