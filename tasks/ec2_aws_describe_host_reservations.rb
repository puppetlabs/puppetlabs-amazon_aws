...
def operation_with_hash(arg_hash)
  ...
  client.describe_host_reservations(call_hash)
end

def describe_host_reservations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_host_reservations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_host_reservations(params)
