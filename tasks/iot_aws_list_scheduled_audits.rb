...
def operation_with_hash(arg_hash)
  ...
  client.list_scheduled_audits(call_hash)
end

def list_scheduled_audits(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.list_scheduled_audits if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_scheduled_audits(params)
