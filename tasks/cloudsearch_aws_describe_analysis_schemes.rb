...
def operation_with_hash(arg_hash)
  ...
  client.describe_analysis_schemes(call_hash)
end

def describe_analysis_schemes(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudSearch::Client.new(region: region)
  response = client.describe_analysis_schemes if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_analysis_schemes(params)
