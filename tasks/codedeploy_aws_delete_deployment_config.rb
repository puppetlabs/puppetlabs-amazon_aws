...
def operation_with_hash(arg_hash)
  ...
  client.delete_deployment_config(call_hash)
end

def delete_deployment_config(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeDeploy::Client.new(region: region)
  response = client.delete_deployment_config if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_deployment_config(params)
