...
def operation_with_hash(arg_hash)
  ...
  client.put_invitation_configuration(call_hash)
end

def put_invitation_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.put_invitation_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_invitation_configuration(params)
