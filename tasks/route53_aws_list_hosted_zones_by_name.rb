...
def operation_with_hash(arg_hash)
  ...
  client.list_hosted_zones_by_name(call_hash)
end

def list_hosted_zones_by_name(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.list_hosted_zones_by_name if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_hosted_zones_by_name(params)
