...
def operation_with_hash(arg_hash)
  ...
  client.create_byte_match_set(call_hash)
end

def create_byte_match_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.create_byte_match_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_byte_match_set(params)
