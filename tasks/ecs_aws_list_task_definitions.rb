...
def operation_with_hash(arg_hash)
  ...
  client.list_task_definitions(call_hash)
end

def list_task_definitions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.list_task_definitions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_task_definitions(params)
