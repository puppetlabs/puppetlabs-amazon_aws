...
def operation_with_hash(arg_hash)
  ...
  client.delete_security_configuration(call_hash)
end

def delete_security_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Glue::Client.new(region: region)
  response = client.delete_security_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_security_configuration(params)
