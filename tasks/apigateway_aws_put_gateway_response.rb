...
def operation_with_hash(arg_hash)
  ...
  client.put_gateway_response(call_hash)
end

def put_gateway_response(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.put_gateway_response if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_gateway_response(params)
