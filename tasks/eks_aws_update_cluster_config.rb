...
def operation_with_hash(arg_hash)
  ...
  client = Aws::EKS::Client.new(region: region)
  client.update_cluster_config(call_hash)
end

def update_cluster_config(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EKS::Client.new(region: region)
  response = client.update_cluster_config if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_cluster_config(params)
