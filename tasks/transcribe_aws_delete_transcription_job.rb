...
def operation_with_hash(arg_hash)
  ...
  client.delete_transcription_job(call_hash)
end

def delete_transcription_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::TranscribeService::Client.new(region: region)
  response = client.delete_transcription_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_transcription_job(params)
