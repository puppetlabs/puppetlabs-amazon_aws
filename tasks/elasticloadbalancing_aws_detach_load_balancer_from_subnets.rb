...
def operation_with_hash(arg_hash)
  ...
  client.detach_load_balancer_from_subnets(call_hash)
end

def detach_load_balancer_from_subnets(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticLoadBalancingV2::Client.new(region: region)
  response = client.detach_load_balancer_from_subnets if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = detach_load_balancer_from_subnets(params)
