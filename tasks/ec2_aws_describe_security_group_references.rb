...
def operation_with_hash(arg_hash)
  ...
  client.describe_security_group_references(call_hash)
end

def describe_security_group_references(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_security_group_references if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_security_group_references(params)
