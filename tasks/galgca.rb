...
def operation_with_hash(arg_hash)
  ...
  client.list_group_certificate_authorities(call_hash)
end

def list_group_certificate_authorities(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.list_group_certificate_authorities if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_group_certificate_authorities(params)
