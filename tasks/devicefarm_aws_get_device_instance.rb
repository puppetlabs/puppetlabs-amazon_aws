...
def operation_with_hash(arg_hash)
  ...
  client.get_device_instance(call_hash)
end

def get_device_instance(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.get_device_instance if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_device_instance(params)
