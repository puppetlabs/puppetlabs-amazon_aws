...
def operation_with_hash(arg_hash)
  ...
  client.update_stack_instances(call_hash)
end

def update_stack_instances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudFormation::Client.new(region: region)
  response = client.update_stack_instances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_stack_instances(params)
