...
def operation_with_hash(arg_hash)
  ...
  client.create_hsm_client_certificate(call_hash)
end

def create_hsm_client_certificate(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.create_hsm_client_certificate if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_hsm_client_certificate(params)
