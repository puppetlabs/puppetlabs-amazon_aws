...
def operation_with_hash(arg_hash)
  ...
  client.get_hosted_zone_count(call_hash)
end

def get_hosted_zone_count(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.get_hosted_zone_count if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_hosted_zone_count(params)
