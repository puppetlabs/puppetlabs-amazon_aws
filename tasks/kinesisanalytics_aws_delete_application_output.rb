...
def operation_with_hash(arg_hash)
  ...
  client.delete_application_output(call_hash)
end

def delete_application_output(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KinesisAnalytics::Client.new(region: region)
  response = client.delete_application_output if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_application_output(params)
