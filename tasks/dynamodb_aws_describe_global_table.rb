...
def operation_with_hash(arg_hash)
  ...
  client.describe_global_table(call_hash)
end

def describe_global_table(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DynamoDB::Client.new(region: region)
  response = client.describe_global_table if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_global_table(params)
