...
def operation_with_hash(arg_hash)
  ...
  client.update_organizational_unit(call_hash)
end

def update_organizational_unit(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Organizations::Client.new(region: region)
  response = client.update_organizational_unit if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_organizational_unit(params)
