...
def operation_with_hash(arg_hash)
  ...
  client.delete_snapshot_schedule(call_hash)
end

def delete_snapshot_schedule(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.delete_snapshot_schedule if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_snapshot_schedule(params)
