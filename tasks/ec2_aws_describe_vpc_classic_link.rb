...
def operation_with_hash(arg_hash)
  ...
  client.describe_vpc_classic_link(call_hash)
end

def describe_vpc_classic_link(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_vpc_classic_link if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_vpc_classic_link(params)
