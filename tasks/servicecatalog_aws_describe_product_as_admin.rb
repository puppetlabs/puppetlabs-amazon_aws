...
def operation_with_hash(arg_hash)
  ...
  client.describe_product_as_admin(call_hash)
end

def describe_product_as_admin(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.describe_product_as_admin if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_product_as_admin(params)
