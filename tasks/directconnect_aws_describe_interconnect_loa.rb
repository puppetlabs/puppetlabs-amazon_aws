...
def operation_with_hash(arg_hash)
  ...
  client.describe_interconnect_loa(call_hash)
end

def describe_interconnect_loa(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.describe_interconnect_loa if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_interconnect_loa(params)
