...
def operation_with_hash(arg_hash)
  ...
  client.get_notification_channel(call_hash)
end

def get_notification_channel(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::FMS::Client.new(region: region)
  response = client.get_notification_channel if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_notification_channel(params)
