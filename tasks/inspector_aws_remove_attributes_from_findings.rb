...
def operation_with_hash(arg_hash)
  ...
  client.remove_attributes_from_findings(call_hash)
end

def remove_attributes_from_findings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.remove_attributes_from_findings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = remove_attributes_from_findings(params)
