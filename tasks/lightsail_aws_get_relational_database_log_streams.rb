...
def operation_with_hash(arg_hash)
  ...
  client.get_relational_database_log_streams(call_hash)
end

def get_relational_database_log_streams(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.get_relational_database_log_streams if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_relational_database_log_streams(params)
