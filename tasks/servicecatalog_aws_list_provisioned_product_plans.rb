...
def operation_with_hash(arg_hash)
  ...
  client.list_provisioned_product_plans(call_hash)
end

def list_provisioned_product_plans(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.list_provisioned_product_plans if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_provisioned_product_plans(params)
