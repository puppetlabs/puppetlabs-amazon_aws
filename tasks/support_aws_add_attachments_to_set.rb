...
def operation_with_hash(arg_hash)
  ...
  client.add_attachments_to_set(call_hash)
end

def add_attachments_to_set(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Support::Client.new(region: region)
  response = client.add_attachments_to_set if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_attachments_to_set(params)
