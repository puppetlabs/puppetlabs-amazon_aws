...
def operation_with_hash(arg_hash)
  ...
  client.list_logging_configurations(call_hash)
end

def list_logging_configurations(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.list_logging_configurations if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_logging_configurations(params)
