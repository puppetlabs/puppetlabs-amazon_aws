...
def operation_with_hash(arg_hash)
  ...
  client.describe_continuous_backups(call_hash)
end

def describe_continuous_backups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DynamoDB::Client.new(region: region)
  response = client.describe_continuous_backups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_continuous_backups(params)
