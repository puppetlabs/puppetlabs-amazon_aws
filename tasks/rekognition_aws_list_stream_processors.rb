...
def operation_with_hash(arg_hash)
  ...
  client.list_stream_processors(call_hash)
end

def list_stream_processors(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Rekognition::Client.new(region: region)
  response = client.list_stream_processors if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_stream_processors(params)
