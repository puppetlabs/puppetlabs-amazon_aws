...
def operation_with_hash(arg_hash)
  ...
  client.delete_documentation_version(call_hash)
end

def delete_documentation_version(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.delete_documentation_version if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_documentation_version(params)
