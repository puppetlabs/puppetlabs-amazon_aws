...
def operation_with_hash(arg_hash)
  ...
  client.delete_vpc_endpoints(call_hash)
end

def delete_vpc_endpoints(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.delete_vpc_endpoints if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_vpc_endpoints(params)
