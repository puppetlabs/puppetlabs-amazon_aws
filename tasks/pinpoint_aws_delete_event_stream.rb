...
def operation_with_hash(arg_hash)
  ...
  client.delete_event_stream(call_hash)
end

def delete_event_stream(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Pinpoint::Client.new(region: region)
  response = client.delete_event_stream if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_event_stream(params)
