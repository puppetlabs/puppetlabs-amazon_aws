...
def operation_with_hash(arg_hash)
  ...
  client.update_tags_for_resource(call_hash)
end

def update_tags_for_resource(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.update_tags_for_resource if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_tags_for_resource(params)
