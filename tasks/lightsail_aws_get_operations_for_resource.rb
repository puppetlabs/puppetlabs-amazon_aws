...
def operation_with_hash(arg_hash)
  ...
  client.get_operations_for_resource(call_hash)
end

def get_operations_for_resource(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.get_operations_for_resource if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_operations_for_resource(params)
