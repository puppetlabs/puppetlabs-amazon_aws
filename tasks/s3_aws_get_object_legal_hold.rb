...
def operation_with_hash(arg_hash)
  ...
  client.get_object_legal_hold(call_hash)
end

def get_object_legal_hold(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::S3::Client.new(region: region)
  response = client.get_object_legal_hold if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_object_legal_hold(params)
