...
def operation_with_hash(arg_hash)
  ...
  client.get_associated_role(call_hash)
end

def get_associated_role(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.get_associated_role if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_associated_role(params)
