...
def operation_with_hash(arg_hash)
  ...
  client.describe_inventory_deletions(call_hash)
end

def describe_inventory_deletions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.describe_inventory_deletions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_inventory_deletions(params)
