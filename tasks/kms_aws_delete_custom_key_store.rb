...
def operation_with_hash(arg_hash)
  ...
  client.delete_custom_key_store(call_hash)
end

def delete_custom_key_store(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KMS::Client.new(region: region)
  response = client.delete_custom_key_store if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_custom_key_store(params)
