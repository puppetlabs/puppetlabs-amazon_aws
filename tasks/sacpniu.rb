...
def operation_with_hash(arg_hash)
  ...
  client.create_presigned_notebook_instance_url(call_hash)
end

def create_presigned_notebook_instance_url(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.create_presigned_notebook_instance_url if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_presigned_notebook_instance_url(params)
