...
def operation_with_hash(arg_hash)
  ...
  client.describe_tape_recovery_points(call_hash)
end

def describe_tape_recovery_points(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.describe_tape_recovery_points if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_tape_recovery_points(params)
