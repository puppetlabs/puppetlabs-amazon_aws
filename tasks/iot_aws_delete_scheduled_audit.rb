...
def operation_with_hash(arg_hash)
  ...
  client.delete_scheduled_audit(call_hash)
end

def delete_scheduled_audit(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.delete_scheduled_audit if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_scheduled_audit(params)
