...
def operation_with_hash(arg_hash)
  ...
  client.describe_image_permissions(call_hash)
end

def describe_image_permissions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AppStream::Client.new(region: region)
  response = client.describe_image_permissions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_image_permissions(params)
