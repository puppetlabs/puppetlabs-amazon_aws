...
def operation_with_hash(arg_hash)
  ...
  client.remove_account_from_organization(call_hash)
end

def remove_account_from_organization(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Organizations::Client.new(region: region)
  response = client.remove_account_from_organization if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = remove_account_from_organization(params)
