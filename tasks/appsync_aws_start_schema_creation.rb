...
def operation_with_hash(arg_hash)
  ...
  client.start_schema_creation(call_hash)
end

def start_schema_creation(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AppSync::Client.new(region: region)
  response = client.start_schema_creation if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_schema_creation(params)
