...
def operation_with_hash(arg_hash)
  ...
  client.get_transcription_job(call_hash)
end

def get_transcription_job(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::TranscribeService::Client.new(region: region)
  response = client.get_transcription_job if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_transcription_job(params)
