...
def operation_with_hash(arg_hash)
  ...
  client.update_assessment_target(call_hash)
end

def update_assessment_target(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.update_assessment_target if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_assessment_target(params)
