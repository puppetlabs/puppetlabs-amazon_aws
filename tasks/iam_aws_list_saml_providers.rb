...
def operation_with_hash(arg_hash)
  ...
  client.list_saml_providers(call_hash)
end

def list_saml_providers(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.list_saml_providers if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_saml_providers(params)
