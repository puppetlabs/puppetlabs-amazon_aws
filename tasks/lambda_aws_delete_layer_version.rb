...
def operation_with_hash(arg_hash)
  ...
  client.delete_layer_version(call_hash)
end

def delete_layer_version(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lambda::Client.new(region: region)
  response = client.delete_layer_version if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_layer_version(params)
