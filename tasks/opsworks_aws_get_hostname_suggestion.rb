...
def operation_with_hash(arg_hash)
  ...
  client.get_hostname_suggestion(call_hash)
end

def get_hostname_suggestion(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::OpsWorks::Client.new(region: region)
  response = client.get_hostname_suggestion if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_hostname_suggestion(params)
