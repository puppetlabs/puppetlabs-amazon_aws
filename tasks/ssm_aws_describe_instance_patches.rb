...
def operation_with_hash(arg_hash)
  ...
  client.describe_instance_patches(call_hash)
end

def describe_instance_patches(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.describe_instance_patches if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_instance_patches(params)
