...
def operation_with_hash(arg_hash)
  ...
  client = Aws::Health::Client.new(region: region)
  client.describe_entity_aggregates(call_hash)
end

def describe_entity_aggregates(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Health::Client.new(region: region)
  response = client.describe_entity_aggregates if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_entity_aggregates(params)
