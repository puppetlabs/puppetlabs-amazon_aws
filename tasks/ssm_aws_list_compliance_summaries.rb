...
def operation_with_hash(arg_hash)
  ...
  client.list_compliance_summaries(call_hash)
end

def list_compliance_summaries(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.list_compliance_summaries if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_compliance_summaries(params)
