...
def operation_with_hash(arg_hash)
  ...
  client.get_key_rotation_status(call_hash)
end

def get_key_rotation_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::KMS::Client.new(region: region)
  response = client.get_key_rotation_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_key_rotation_status(params)
