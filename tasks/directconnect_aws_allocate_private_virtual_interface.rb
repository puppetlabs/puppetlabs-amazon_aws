...
def operation_with_hash(arg_hash)
  ...
  client.allocate_private_virtual_interface(call_hash)
end

def allocate_private_virtual_interface(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.allocate_private_virtual_interface if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = allocate_private_virtual_interface(params)
