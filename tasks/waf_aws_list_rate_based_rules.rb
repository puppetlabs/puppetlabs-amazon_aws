...
def operation_with_hash(arg_hash)
  ...
  client.list_rate_based_rules(call_hash)
end

def list_rate_based_rules(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.list_rate_based_rules if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_rate_based_rules(params)
