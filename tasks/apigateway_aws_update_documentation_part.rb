...
def operation_with_hash(arg_hash)
  ...
  client.update_documentation_part(call_hash)
end

def update_documentation_part(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.update_documentation_part if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_documentation_part(params)
