...
def operation_with_hash(arg_hash)
  ...
  client.remove_tags_from_stream(call_hash)
end

def remove_tags_from_stream(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Kinesis::Client.new(region: region)
  response = client.remove_tags_from_stream if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = remove_tags_from_stream(params)
