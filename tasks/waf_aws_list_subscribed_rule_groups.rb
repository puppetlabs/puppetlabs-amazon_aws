...
def operation_with_hash(arg_hash)
  ...
  client.list_subscribed_rule_groups(call_hash)
end

def list_subscribed_rule_groups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.list_subscribed_rule_groups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_subscribed_rule_groups(params)
