...
def operation_with_hash(arg_hash)
  ...
  client.create_vpn_connection_route(call_hash)
end

def create_vpn_connection_route(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_vpn_connection_route if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_vpn_connection_route(params)
