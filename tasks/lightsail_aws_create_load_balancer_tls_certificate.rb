...
def operation_with_hash(arg_hash)
  ...
  client.create_load_balancer_tls_certificate(call_hash)
end

def create_load_balancer_tls_certificate(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.create_load_balancer_tls_certificate if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_load_balancer_tls_certificate(params)
