...
def operation_with_hash(arg_hash)
  ...
  client = Aws::ResourceGroupsTaggingAPI::Client.new(region: region)
  client.get_tag_values(call_hash)
end

def get_tag_values(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ResourceGroupsTaggingAPI::Client.new(region: region)
  response = client.get_tag_values if arg_hash.empty?
