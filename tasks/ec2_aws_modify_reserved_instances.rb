...
def operation_with_hash(arg_hash)
  ...
  client.modify_reserved_instances(call_hash)
end

def modify_reserved_instances(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.modify_reserved_instances if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_reserved_instances(params)
