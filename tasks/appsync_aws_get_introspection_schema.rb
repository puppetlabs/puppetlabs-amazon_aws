...
def operation_with_hash(arg_hash)
  ...
  client.get_introspection_schema(call_hash)
end

def get_introspection_schema(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AppSync::Client.new(region: region)
  response = client.get_introspection_schema if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_introspection_schema(params)
