...
def operation_with_hash(arg_hash)
  ...
  client.get_authorization_token(call_hash)
end

def get_authorization_token(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECR::Client.new(region: region)
  response = client.get_authorization_token if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_authorization_token(params)
