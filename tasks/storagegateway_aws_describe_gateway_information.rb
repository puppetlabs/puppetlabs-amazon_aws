...
def operation_with_hash(arg_hash)
  ...
  client.describe_gateway_information(call_hash)
end

def describe_gateway_information(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.describe_gateway_information if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_gateway_information(params)
