...
def operation_with_hash(arg_hash)
  ...
  client.delete_patch_baseline(call_hash)
end

def delete_patch_baseline(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.delete_patch_baseline if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_patch_baseline(params)
