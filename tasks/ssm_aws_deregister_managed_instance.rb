...
def operation_with_hash(arg_hash)
  ...
  client.deregister_managed_instance(call_hash)
end

def deregister_managed_instance(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.deregister_managed_instance if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = deregister_managed_instance(params)
