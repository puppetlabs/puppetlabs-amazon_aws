...
def operation_with_hash(arg_hash)
  ...
  client.list_attached_indices(call_hash)
end

def list_attached_indices(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudDirectory::Client.new(region: region)
  response = client.list_attached_indices if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_attached_indices(params)
