...
def operation_with_hash(arg_hash)
  ...
  client.create_documentation_part(call_hash)
end

def create_documentation_part(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.create_documentation_part if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_documentation_part(params)
