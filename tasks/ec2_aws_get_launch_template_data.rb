...
def operation_with_hash(arg_hash)
  ...
  client.get_launch_template_data(call_hash)
end

def get_launch_template_data(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.get_launch_template_data if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_launch_template_data(params)
