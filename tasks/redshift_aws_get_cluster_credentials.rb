...
def operation_with_hash(arg_hash)
  ...
  client.get_cluster_credentials(call_hash)
end

def get_cluster_credentials(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.get_cluster_credentials if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_cluster_credentials(params)
