...
def operation_with_hash(arg_hash)
  ...
  client.describe_ssl_policies(call_hash)
end

def describe_ssl_policies(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticLoadBalancingV2::Client.new(region: region)
  response = client.describe_ssl_policies if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_ssl_policies(params)
