...
def operation_with_hash(arg_hash)
  ...
  client.delete_method_response(call_hash)
end

def delete_method_response(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.delete_method_response if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_method_response(params)
