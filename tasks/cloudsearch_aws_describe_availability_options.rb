...
def operation_with_hash(arg_hash)
  ...
  client.describe_availability_options(call_hash)
end

def describe_availability_options(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudSearch::Client.new(region: region)
  response = client.describe_availability_options if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_availability_options(params)
