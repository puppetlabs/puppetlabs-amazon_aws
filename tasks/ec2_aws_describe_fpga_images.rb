...
def operation_with_hash(arg_hash)
  ...
  client.describe_fpga_images(call_hash)
end

def describe_fpga_images(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_fpga_images if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_fpga_images(params)
