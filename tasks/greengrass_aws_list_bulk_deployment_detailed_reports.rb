...
def operation_with_hash(arg_hash)
  ...
  client.list_bulk_deployment_detailed_reports(call_hash)
end

def list_bulk_deployment_detailed_reports(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.list_bulk_deployment_detailed_reports if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_bulk_deployment_detailed_reports(params)
