...
def operation_with_hash(arg_hash)
  ...
  client.put_delivery_channel(call_hash)
end

def put_delivery_channel(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.put_delivery_channel if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_delivery_channel(params)
