...
def operation_with_hash(arg_hash)
  ...
  client.update_health_check(call_hash)
end

def update_health_check(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.update_health_check if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_health_check(params)
