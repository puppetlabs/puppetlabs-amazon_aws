...
def operation_with_hash(arg_hash)
  ...
  client.list_transcription_jobs(call_hash)
end

def list_transcription_jobs(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::TranscribeService::Client.new(region: region)
  response = client.list_transcription_jobs if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_transcription_jobs(params)
