...
def operation_with_hash(arg_hash)
  ...
  client.delete_delivery_channel(call_hash)
end

def delete_delivery_channel(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.delete_delivery_channel if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_delivery_channel(params)
