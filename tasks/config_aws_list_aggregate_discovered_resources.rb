...
def operation_with_hash(arg_hash)
  ...
  client.list_aggregate_discovered_resources(call_hash)
end

def list_aggregate_discovered_resources(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.list_aggregate_discovered_resources if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_aggregate_discovered_resources(params)
