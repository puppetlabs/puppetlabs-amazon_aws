...
def operation_with_hash(arg_hash)
  ...
  client.get_function_configuration(call_hash)
end

def get_function_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lambda::Client.new(region: region)
  response = client.get_function_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_function_configuration(params)
