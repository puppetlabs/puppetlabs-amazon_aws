...
def operation_with_hash(arg_hash)
  ...
  client.remove_role_from_db_instance(call_hash)
end

def remove_role_from_db_instance(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.remove_role_from_db_instance if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = remove_role_from_db_instance(params)
