...
def operation_with_hash(arg_hash)
  ...
  client.describe_runtime_configuration(call_hash)
end

def describe_runtime_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.describe_runtime_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_runtime_configuration(params)
