...
def operation_with_hash(arg_hash)
  ...
  client.get_group_certificate_authority(call_hash)
end

def get_group_certificate_authority(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.get_group_certificate_authority if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_group_certificate_authority(params)
