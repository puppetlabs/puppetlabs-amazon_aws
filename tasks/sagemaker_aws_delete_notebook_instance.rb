...
def operation_with_hash(arg_hash)
  ...
  client.delete_notebook_instance(call_hash)
end

def delete_notebook_instance(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.delete_notebook_instance if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_notebook_instance(params)
