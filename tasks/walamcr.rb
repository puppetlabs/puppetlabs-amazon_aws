...
def operation_with_hash(arg_hash)
  ...
  client.list_available_management_cidr_ranges(call_hash)
end

def list_available_management_cidr_ranges(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkSpaces::Client.new(region: region)
  response = client.list_available_management_cidr_ranges if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_available_management_cidr_ranges(params)
