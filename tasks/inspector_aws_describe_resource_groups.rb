...
def operation_with_hash(arg_hash)
  ...
  client.describe_resource_groups(call_hash)
end

def describe_resource_groups(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.describe_resource_groups if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_resource_groups(params)
