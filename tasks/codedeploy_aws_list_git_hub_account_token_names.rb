...
def operation_with_hash(arg_hash)
  ...
  client.list_git_hub_account_token_names(call_hash)
end

def list_git_hub_account_token_names(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeDeploy::Client.new(region: region)
  response = client.list_git_hub_account_token_names if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_git_hub_account_token_names(params)
