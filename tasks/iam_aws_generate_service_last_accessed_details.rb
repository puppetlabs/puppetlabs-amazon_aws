...
def operation_with_hash(arg_hash)
  ...
  client.generate_service_last_accessed_details(call_hash)
end

def generate_service_last_accessed_details(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.generate_service_last_accessed_details if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = generate_service_last_accessed_details(params)
