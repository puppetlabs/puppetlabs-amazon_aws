...
def operation_with_hash(arg_hash)
  ...
  client.describe_luna_client(call_hash)
end

def describe_luna_client(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudHSM::Client.new(region: region)
  response = client.describe_luna_client if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_luna_client(params)
