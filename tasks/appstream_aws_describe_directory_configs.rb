...
def operation_with_hash(arg_hash)
  ...
  client.describe_directory_configs(call_hash)
end

def describe_directory_configs(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AppStream::Client.new(region: region)
  response = client.describe_directory_configs if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_directory_configs(params)
