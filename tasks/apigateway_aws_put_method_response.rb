...
def operation_with_hash(arg_hash)
  ...
  client.put_method_response(call_hash)
end

def put_method_response(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.put_method_response if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_method_response(params)
