...
def operation_with_hash(arg_hash)
  ...
  client.delete_folder_contents(call_hash)
end

def delete_folder_contents(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.delete_folder_contents if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_folder_contents(params)
