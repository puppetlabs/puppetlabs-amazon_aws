...
def operation_with_hash(arg_hash)
  ...
  client.modify_cluster_snapshot_schedule(call_hash)
end

def modify_cluster_snapshot_schedule(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.modify_cluster_snapshot_schedule if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_cluster_snapshot_schedule(params)
