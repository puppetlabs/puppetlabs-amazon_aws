...
def operation_with_hash(arg_hash)
  ...
  client.remove_layer_version_permission(call_hash)
end

def remove_layer_version_permission(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lambda::Client.new(region: region)
  response = client.remove_layer_version_permission if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = remove_layer_version_permission(params)
