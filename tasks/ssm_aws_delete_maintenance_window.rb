...
def operation_with_hash(arg_hash)
  ...
  client.delete_maintenance_window(call_hash)
end

def delete_maintenance_window(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.delete_maintenance_window if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_maintenance_window(params)
