...
def operation_with_hash(arg_hash)
  ...
  client.list_portfolios_for_product(call_hash)
end

def list_portfolios_for_product(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.list_portfolios_for_product if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_portfolios_for_product(params)
