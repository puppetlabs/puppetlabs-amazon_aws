...
def operation_with_hash(arg_hash)
  ...
  client.create_group_certificate_authority(call_hash)
end

def create_group_certificate_authority(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.create_group_certificate_authority if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_group_certificate_authority(params)
