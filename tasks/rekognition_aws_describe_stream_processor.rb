...
def operation_with_hash(arg_hash)
  ...
  client.describe_stream_processor(call_hash)
end

def describe_stream_processor(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Rekognition::Client.new(region: region)
  response = client.describe_stream_processor if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_stream_processor(params)
