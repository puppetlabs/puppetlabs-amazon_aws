...
def operation_with_hash(arg_hash)
  ...
  client.delete_matchmaking_configuration(call_hash)
end

def delete_matchmaking_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::GameLift::Client.new(region: region)
  response = client.delete_matchmaking_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_matchmaking_configuration(params)
