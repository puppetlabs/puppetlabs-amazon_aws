...
def operation_with_hash(arg_hash)
  ...
  client.create_billing_group(call_hash)
end

def create_billing_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.create_billing_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_billing_group(params)
