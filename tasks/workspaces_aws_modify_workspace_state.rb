...
def operation_with_hash(arg_hash)
  ...
  client.modify_workspace_state(call_hash)
end

def modify_workspace_state(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkSpaces::Client.new(region: region)
  response = client.modify_workspace_state if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_workspace_state(params)
