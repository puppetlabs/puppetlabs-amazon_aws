...
def operation_with_hash(arg_hash)
  ...
  client.get_instance_snapshot(call_hash)
end

def get_instance_snapshot(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.get_instance_snapshot if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_instance_snapshot(params)
