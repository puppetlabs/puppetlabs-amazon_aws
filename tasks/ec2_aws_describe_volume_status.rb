...
def operation_with_hash(arg_hash)
  ...
  client.describe_volume_status(call_hash)
end

def describe_volume_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_volume_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_volume_status(params)
