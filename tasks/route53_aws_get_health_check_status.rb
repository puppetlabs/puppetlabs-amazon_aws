...
def operation_with_hash(arg_hash)
  ...
  client.get_health_check_status(call_hash)
end

def get_health_check_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.get_health_check_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_health_check_status(params)
