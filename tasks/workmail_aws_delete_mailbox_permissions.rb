...
def operation_with_hash(arg_hash)
  ...
  client.delete_mailbox_permissions(call_hash)
end

def delete_mailbox_permissions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkMail::Client.new(region: region)
  response = client.delete_mailbox_permissions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_mailbox_permissions(params)
