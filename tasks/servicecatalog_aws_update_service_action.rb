...
def operation_with_hash(arg_hash)
  ...
  client.update_service_action(call_hash)
end

def update_service_action(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ServiceCatalog::Client.new(region: region)
  response = client.update_service_action if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_service_action(params)
