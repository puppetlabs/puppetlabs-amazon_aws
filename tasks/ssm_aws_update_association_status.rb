...
def operation_with_hash(arg_hash)
  ...
  client.update_association_status(call_hash)
end

def update_association_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.update_association_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_association_status(params)
