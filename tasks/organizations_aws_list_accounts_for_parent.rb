...
def operation_with_hash(arg_hash)
  ...
  client.list_accounts_for_parent(call_hash)
end

def list_accounts_for_parent(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Organizations::Client.new(region: region)
  response = client.list_accounts_for_parent if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_accounts_for_parent(params)
