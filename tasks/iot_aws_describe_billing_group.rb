...
def operation_with_hash(arg_hash)
  ...
  client.describe_billing_group(call_hash)
end

def describe_billing_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.describe_billing_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_billing_group(params)
