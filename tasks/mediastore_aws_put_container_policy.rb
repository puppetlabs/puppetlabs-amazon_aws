...
def operation_with_hash(arg_hash)
  ...
  client.put_container_policy(call_hash)
end

def put_container_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaStore::Client.new(region: region)
  response = client.put_container_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_container_policy(params)
