...
def operation_with_hash(arg_hash)
  ...
  client.get_aggregate_config_rule_compliance_summary(call_hash)
end

def get_aggregate_config_rule_compliance_summary(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.get_aggregate_config_rule_compliance_summary if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_aggregate_config_rule_compliance_summary(params)
