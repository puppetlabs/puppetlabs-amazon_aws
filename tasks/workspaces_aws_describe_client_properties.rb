...
def operation_with_hash(arg_hash)
  ...
  client.describe_client_properties(call_hash)
end

def describe_client_properties(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkSpaces::Client.new(region: region)
  response = client.describe_client_properties if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_client_properties(params)
