...
def operation_with_hash(arg_hash)
  ...
  client.update_role_description(call_hash)
end

def update_role_description(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.update_role_description if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_role_description(params)
