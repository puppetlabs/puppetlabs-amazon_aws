...
def operation_with_hash(arg_hash)
  ...
  client.update_application_resource_lifecycle(call_hash)
end

def update_application_resource_lifecycle(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.update_application_resource_lifecycle if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_application_resource_lifecycle(params)
