...
def operation_with_hash(arg_hash)
  ...
  client.describe_bundle_tasks(call_hash)
end

def describe_bundle_tasks(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_bundle_tasks if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_bundle_tasks(params)
