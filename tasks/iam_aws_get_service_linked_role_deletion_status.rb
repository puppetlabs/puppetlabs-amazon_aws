...
def operation_with_hash(arg_hash)
  ...
  client.get_service_linked_role_deletion_status(call_hash)
end

def get_service_linked_role_deletion_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.get_service_linked_role_deletion_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_service_linked_role_deletion_status(params)
