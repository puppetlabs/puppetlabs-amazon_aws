...
def operation_with_hash(arg_hash)
  ...
  client.describe_assessment_targets(call_hash)
end

def describe_assessment_targets(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.describe_assessment_targets if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_assessment_targets(params)
