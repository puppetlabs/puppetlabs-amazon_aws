...
def operation_with_hash(arg_hash)
  ...
  client.describe_severity_levels(call_hash)
end

def describe_severity_levels(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Support::Client.new(region: region)
  response = client.describe_severity_levels if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_severity_levels(params)
