...
def operation_with_hash(arg_hash)
  ...
  client.delete_server_catalog(call_hash)
end

def delete_server_catalog(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SMS::Client.new(region: region)
  response = client.delete_server_catalog if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_server_catalog(params)
