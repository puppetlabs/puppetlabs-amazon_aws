...
def operation_with_hash(arg_hash)
  ...
  client.create_instance_snapshot(call_hash)
end

def create_instance_snapshot(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.create_instance_snapshot if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_instance_snapshot(params)
