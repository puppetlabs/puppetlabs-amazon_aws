...
def operation_with_hash(arg_hash)
  ...
  client.get_workflow_execution_history(call_hash)
end

def get_workflow_execution_history(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SWF::Client.new(region: region)
  response = client.get_workflow_execution_history if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_workflow_execution_history(params)
