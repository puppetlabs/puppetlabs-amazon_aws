...
def operation_with_hash(arg_hash)
  ...
  client.list_platform_versions(call_hash)
end

def list_platform_versions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.list_platform_versions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_platform_versions(params)
