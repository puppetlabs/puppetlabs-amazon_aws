...
def operation_with_hash(arg_hash)
  ...
  client.list_query_logging_configs(call_hash)
end

def list_query_logging_configs(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53::Client.new(region: region)
  response = client.list_query_logging_configs if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_query_logging_configs(params)
