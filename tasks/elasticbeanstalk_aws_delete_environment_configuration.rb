...
def operation_with_hash(arg_hash)
  ...
  client.delete_environment_configuration(call_hash)
end

def delete_environment_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.delete_environment_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_environment_configuration(params)
