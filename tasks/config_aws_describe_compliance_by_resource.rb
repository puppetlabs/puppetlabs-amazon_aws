...
def operation_with_hash(arg_hash)
  ...
  client.describe_compliance_by_resource(call_hash)
end

def describe_compliance_by_resource(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.describe_compliance_by_resource if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_compliance_by_resource(params)
