...
def operation_with_hash(arg_hash)
  ...
  client.describe_organization(call_hash)
end

def describe_organization(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkMail::Client.new(region: region)
  response = client.describe_organization if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_organization(params)
