...
def operation_with_hash(arg_hash)
  ...
  client.start_relational_database(call_hash)
end

def start_relational_database(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.start_relational_database if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_relational_database(params)
