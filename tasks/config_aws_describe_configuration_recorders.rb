...
def operation_with_hash(arg_hash)
  ...
  client.describe_configuration_recorders(call_hash)
end

def describe_configuration_recorders(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ConfigService::Client.new(region: region)
  response = client.describe_configuration_recorders if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_configuration_recorders(params)
