...
def operation_with_hash(arg_hash)
  ...
  client.get_subscription_state(call_hash)
end

def get_subscription_state(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Shield::Client.new(region: region)
  response = client.get_subscription_state if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_subscription_state(params)
