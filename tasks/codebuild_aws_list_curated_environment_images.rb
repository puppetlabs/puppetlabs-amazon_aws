...
def operation_with_hash(arg_hash)
  ...
  client.list_curated_environment_images(call_hash)
end

def list_curated_environment_images(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeBuild::Client.new(region: region)
  response = client.list_curated_environment_images if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_curated_environment_images(params)
