...
def operation_with_hash(arg_hash)
  ...
  client.delete_ssh_public_key(call_hash)
end

def delete_ssh_public_key(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.delete_ssh_public_key if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_ssh_public_key(params)
