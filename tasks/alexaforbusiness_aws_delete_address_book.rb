...
def operation_with_hash(arg_hash)
  ...
  client.delete_address_book(call_hash)
end

def delete_address_book(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.delete_address_book if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_address_book(params)
