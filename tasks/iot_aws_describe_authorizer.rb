...
def operation_with_hash(arg_hash)
  ...
  client.describe_authorizer(call_hash)
end

def describe_authorizer(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.describe_authorizer if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_authorizer(params)
