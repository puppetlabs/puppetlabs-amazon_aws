...
def operation_with_hash(arg_hash)
  ...
  client.run_pipeline_activity(call_hash)
end

def run_pipeline_activity(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoTAnalytics::Client.new(region: region)
  response = client.run_pipeline_activity if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = run_pipeline_activity(params)
