...
def operation_with_hash(arg_hash)
  ...
  client.get_integration_response(call_hash)
end

def get_integration_response(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.get_integration_response if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_integration_response(params)
