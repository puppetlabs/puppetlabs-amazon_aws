...
def operation_with_hash(arg_hash)
  ...
  client.delete_input_security_group(call_hash)
end

def delete_input_security_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaLive::Client.new(region: region)
  response = client.delete_input_security_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_input_security_group(params)
