...
def operation_with_hash(arg_hash)
  ...
  client.get_query_execution(call_hash)
end

def get_query_execution(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Athena::Client.new(region: region)
  response = client.get_query_execution if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_query_execution(params)
