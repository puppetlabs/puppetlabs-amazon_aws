...
def operation_with_hash(arg_hash)
  ...
  client.create_interconnect(call_hash)
end

def create_interconnect(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.create_interconnect if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_interconnect(params)
