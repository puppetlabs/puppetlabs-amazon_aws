...
def operation_with_hash(arg_hash)
  ...
  client.stop_assessment_run(call_hash)
end

def stop_assessment_run(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Inspector::Client.new(region: region)
  response = client.stop_assessment_run if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = stop_assessment_run(params)
