...
def operation_with_hash(arg_hash)
  ...
  client.start_notebook_instance(call_hash)
end

def start_notebook_instance(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.start_notebook_instance if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = start_notebook_instance(params)
