...
def operation_with_hash(arg_hash)
  ...
  client.create_platform_version(call_hash)
end

def create_platform_version(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticBeanstalk::Client.new(region: region)
  response = client.create_platform_version if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_platform_version(params)
