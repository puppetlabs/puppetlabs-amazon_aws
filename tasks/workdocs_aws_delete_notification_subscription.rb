...
def operation_with_hash(arg_hash)
  ...
  client.delete_notification_subscription(call_hash)
end

def delete_notification_subscription(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.delete_notification_subscription if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_notification_subscription(params)
