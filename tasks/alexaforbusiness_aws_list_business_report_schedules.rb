...
def operation_with_hash(arg_hash)
  ...
  client.list_business_report_schedules(call_hash)
end

def list_business_report_schedules(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.list_business_report_schedules if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_business_report_schedules(params)
