...
def operation_with_hash(arg_hash)
  ...
  client.get_conference_provider(call_hash)
end

def get_conference_provider(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AlexaForBusiness::Client.new(region: region)
  response = client.get_conference_provider if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_conference_provider(params)
