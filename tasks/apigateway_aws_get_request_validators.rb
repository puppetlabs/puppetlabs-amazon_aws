...
def operation_with_hash(arg_hash)
  ...
  client.get_request_validators(call_hash)
end

def get_request_validators(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.get_request_validators if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_request_validators(params)
