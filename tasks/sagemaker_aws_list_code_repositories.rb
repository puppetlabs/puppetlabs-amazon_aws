...
def operation_with_hash(arg_hash)
  ...
  client.list_code_repositories(call_hash)
end

def list_code_repositories(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.list_code_repositories if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_code_repositories(params)
