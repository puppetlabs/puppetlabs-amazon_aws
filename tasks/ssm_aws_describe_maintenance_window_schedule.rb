...
def operation_with_hash(arg_hash)
  ...
  client.describe_maintenance_window_schedule(call_hash)
end

def describe_maintenance_window_schedule(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SSM::Client.new(region: region)
  response = client.describe_maintenance_window_schedule if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_maintenance_window_schedule(params)
