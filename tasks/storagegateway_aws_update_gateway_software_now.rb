...
def operation_with_hash(arg_hash)
  ...
  client.update_gateway_software_now(call_hash)
end

def update_gateway_software_now(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.update_gateway_software_now if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_gateway_software_now(params)
