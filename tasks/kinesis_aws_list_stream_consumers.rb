...
def operation_with_hash(arg_hash)
  ...
  client.list_stream_consumers(call_hash)
end

def list_stream_consumers(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Kinesis::Client.new(region: region)
  response = client.list_stream_consumers if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_stream_consumers(params)
