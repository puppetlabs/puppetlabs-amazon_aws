...
def operation_with_hash(arg_hash)
  ...
  client.update_service_access_policies(call_hash)
end

def update_service_access_policies(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudSearch::Client.new(region: region)
  response = client.update_service_access_policies if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_service_access_policies(params)
