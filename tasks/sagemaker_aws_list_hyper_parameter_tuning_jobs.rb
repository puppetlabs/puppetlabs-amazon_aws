...
def operation_with_hash(arg_hash)
  ...
  client.list_hyper_parameter_tuning_jobs(call_hash)
end

def list_hyper_parameter_tuning_jobs(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.list_hyper_parameter_tuning_jobs if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_hyper_parameter_tuning_jobs(params)
