...
def operation_with_hash(arg_hash)
  ...
  client.get_logging_configuration(call_hash)
end

def get_logging_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.get_logging_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_logging_configuration(params)
