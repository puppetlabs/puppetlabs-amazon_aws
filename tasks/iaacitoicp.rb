...
def operation_with_hash(arg_hash)
  ...
  client.add_client_id_to_open_id_connect_provider(call_hash)
end

def add_client_id_to_open_id_connect_provider(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.add_client_id_to_open_id_connect_provider if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = add_client_id_to_open_id_connect_provider(params)
