...
def operation_with_hash(arg_hash)
  ...
  client.create_launch_template_version(call_hash)
end

def create_launch_template_version(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_launch_template_version if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_launch_template_version(params)
