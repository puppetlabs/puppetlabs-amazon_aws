...
def operation_with_hash(arg_hash)
  ...
  client.update_emergency_contact_settings(call_hash)
end

def update_emergency_contact_settings(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Shield::Client.new(region: region)
  response = client.update_emergency_contact_settings if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_emergency_contact_settings(params)
