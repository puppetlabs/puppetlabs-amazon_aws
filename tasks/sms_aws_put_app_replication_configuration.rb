...
def operation_with_hash(arg_hash)
  ...
  client.put_app_replication_configuration(call_hash)
end

def put_app_replication_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SMS::Client.new(region: region)
  response = client.put_app_replication_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_app_replication_configuration(params)
