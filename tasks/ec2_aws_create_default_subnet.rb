...
def operation_with_hash(arg_hash)
  ...
  client.create_default_subnet(call_hash)
end

def create_default_subnet(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.create_default_subnet if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_default_subnet(params)
