...
def operation_with_hash(arg_hash)
  ...
  client = Aws::MediaTailor::Client.new(region: region)
  client.delete_playback_configuration(call_hash)
end

def delete_playback_configuration(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MediaTailor::Client.new(region: region)
  response = client.delete_playback_configuration if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_playback_configuration(params)
