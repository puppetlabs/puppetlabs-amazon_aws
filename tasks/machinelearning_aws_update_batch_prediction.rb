...
def operation_with_hash(arg_hash)
  ...
  client.update_batch_prediction(call_hash)
end

def update_batch_prediction(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::MachineLearning::Client.new(region: region)
  response = client.update_batch_prediction if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_batch_prediction(params)
