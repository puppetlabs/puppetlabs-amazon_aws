...
def operation_with_hash(arg_hash)
  ...
  client.describe_identity_id_format(call_hash)
end

def describe_identity_id_format(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_identity_id_format if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_identity_id_format(params)
