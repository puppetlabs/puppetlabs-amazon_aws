...
def operation_with_hash(arg_hash)
  ...
  client.delete_source_credentials(call_hash)
end

def delete_source_credentials(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeBuild::Client.new(region: region)
  response = client.delete_source_credentials if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_source_credentials(params)
