...
def operation_with_hash(arg_hash)
  ...
  client.describe_workflow_type(call_hash)
end

def describe_workflow_type(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SWF::Client.new(region: region)
  response = client.describe_workflow_type if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_workflow_type(params)
