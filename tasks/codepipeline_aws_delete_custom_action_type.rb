...
def operation_with_hash(arg_hash)
  ...
  client.delete_custom_action_type(call_hash)
end

def delete_custom_action_type(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodePipeline::Client.new(region: region)
  response = client.delete_custom_action_type if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_custom_action_type(params)
