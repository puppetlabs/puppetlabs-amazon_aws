...
def operation_with_hash(arg_hash)
  ...
  client.list_deployment_targets(call_hash)
end

def list_deployment_targets(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeDeploy::Client.new(region: region)
  response = client.list_deployment_targets if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_deployment_targets(params)
