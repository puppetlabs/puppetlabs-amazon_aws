...
def operation_with_hash(arg_hash)
  ...
  client.put_action_revision(call_hash)
end

def put_action_revision(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodePipeline::Client.new(region: region)
  response = client.put_action_revision if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = put_action_revision(params)
