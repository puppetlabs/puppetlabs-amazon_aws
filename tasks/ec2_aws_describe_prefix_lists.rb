...
def operation_with_hash(arg_hash)
  ...
  client.describe_prefix_lists(call_hash)
end

def describe_prefix_lists(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_prefix_lists if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_prefix_lists(params)
