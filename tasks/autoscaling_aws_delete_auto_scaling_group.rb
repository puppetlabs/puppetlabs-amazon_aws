...
def operation_with_hash(arg_hash)
  ...
  client.delete_auto_scaling_group(call_hash)
end

def delete_auto_scaling_group(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AutoScaling::Client.new(region: region)
  response = client.delete_auto_scaling_group if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_auto_scaling_group(params)
