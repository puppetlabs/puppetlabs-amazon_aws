...
def operation_with_hash(arg_hash)
  ...
  client.list_mailbox_permissions(call_hash)
end

def list_mailbox_permissions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkMail::Client.new(region: region)
  response = client.list_mailbox_permissions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_mailbox_permissions(params)
