...
def operation_with_hash(arg_hash)
  ...
  client.attach_load_balancer_tls_certificate(call_hash)
end

def attach_load_balancer_tls_certificate(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.attach_load_balancer_tls_certificate if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = attach_load_balancer_tls_certificate(params)
