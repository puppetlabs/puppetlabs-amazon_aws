...
def operation_with_hash(arg_hash)
  ...
  client.delete_direct_connect_gateway(call_hash)
end

def delete_direct_connect_gateway(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.delete_direct_connect_gateway if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_direct_connect_gateway(params)
