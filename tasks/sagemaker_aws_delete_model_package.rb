...
def operation_with_hash(arg_hash)
  ...
  client.delete_model_package(call_hash)
end

def delete_model_package(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::SageMaker::Client.new(region: region)
  response = client.delete_model_package if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_model_package(params)
