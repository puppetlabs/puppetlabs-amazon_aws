...
def operation_with_hash(arg_hash)
  ...
  client.list_virtual_mfa_devices(call_hash)
end

def list_virtual_mfa_devices(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.list_virtual_mfa_devices if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_virtual_mfa_devices(params)
