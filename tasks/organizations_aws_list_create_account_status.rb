...
def operation_with_hash(arg_hash)
  ...
  client.list_create_account_status(call_hash)
end

def list_create_account_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Organizations::Client.new(region: region)
  response = client.list_create_account_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_create_account_status(params)
