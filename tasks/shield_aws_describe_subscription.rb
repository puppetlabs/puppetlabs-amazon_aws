...
def operation_with_hash(arg_hash)
  ...
  client.describe_subscription(call_hash)
end

def describe_subscription(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Shield::Client.new(region: region)
  response = client.describe_subscription if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_subscription(params)
