...
def operation_with_hash(arg_hash)
  ...
  client.describe_stack_events(call_hash)
end

def describe_stack_events(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CloudFormation::Client.new(region: region)
  response = client.describe_stack_events if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_stack_events(params)
