...
def operation_with_hash(arg_hash)
  ...
  client.create_smb_file_share(call_hash)
end

def create_smb_file_share(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::StorageGateway::Client.new(region: region)
  response = client.create_smb_file_share if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_smb_file_share(params)
