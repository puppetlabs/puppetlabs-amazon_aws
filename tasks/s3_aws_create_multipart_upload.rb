...
def operation_with_hash(arg_hash)
  ...
  client.create_multipart_upload(call_hash)
end

def create_multipart_upload(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::S3::Client.new(region: region)
  response = client.create_multipart_upload if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_multipart_upload(params)
