...
def operation_with_hash(arg_hash)
  ...
  client.describe_event_details(call_hash)
end

def describe_event_details(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Health::Client.new(region: region)
  response = client.describe_event_details if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_event_details(params)
