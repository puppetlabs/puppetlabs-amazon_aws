...
def operation_with_hash(arg_hash)
  ...
  client.list_offering_promotions(call_hash)
end

def list_offering_promotions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DeviceFarm::Client.new(region: region)
  response = client.list_offering_promotions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_offering_promotions(params)
