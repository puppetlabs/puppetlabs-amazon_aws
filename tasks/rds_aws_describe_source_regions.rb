...
def operation_with_hash(arg_hash)
  ...
  client.describe_source_regions(call_hash)
end

def describe_source_regions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::RDS::Client.new(region: region)
  response = client.describe_source_regions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_source_regions(params)
