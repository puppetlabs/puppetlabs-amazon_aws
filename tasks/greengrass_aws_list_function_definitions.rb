...
def operation_with_hash(arg_hash)
  ...
  client.list_function_definitions(call_hash)
end

def list_function_definitions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Greengrass::Client.new(region: region)
  response = client.list_function_definitions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_function_definitions(params)
