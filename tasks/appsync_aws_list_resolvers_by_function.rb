...
def operation_with_hash(arg_hash)
  ...
  client = Aws::AppSync::Client.new(region: region)
  client.list_resolvers_by_function(call_hash)
end

def list_resolvers_by_function(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::AppSync::Client.new(region: region)
  response = client.list_resolvers_by_function if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = list_resolvers_by_function(params)
