...
def operation_with_hash(arg_hash)
  ...
  client.describe_nat_gateways(call_hash)
end

def describe_nat_gateways(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_nat_gateways if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_nat_gateways(params)
