...
def operation_with_hash(arg_hash)
  ...
  client.update_domain_entry(call_hash)
end

def update_domain_entry(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.update_domain_entry if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_domain_entry(params)
