...
def operation_with_hash(arg_hash)
  ...
  client.get_change_token_status(call_hash)
end

def get_change_token_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.get_change_token_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_change_token_status(params)
