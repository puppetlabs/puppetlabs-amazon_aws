...
def operation_with_hash(arg_hash)
  ...
  client.delete_load_balancer_policy(call_hash)
end

def delete_load_balancer_policy(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ElasticLoadBalancingV2::Client.new(region: region)
  response = client.delete_load_balancer_policy if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_load_balancer_policy(params)
