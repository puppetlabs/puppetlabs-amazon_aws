...
def operation_with_hash(arg_hash)
  ...
  client.describe_egress_only_internet_gateways(call_hash)
end

def describe_egress_only_internet_gateways(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_egress_only_internet_gateways if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_egress_only_internet_gateways(params)
