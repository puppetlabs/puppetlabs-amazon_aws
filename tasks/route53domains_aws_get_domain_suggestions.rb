...
def operation_with_hash(arg_hash)
  ...
  client.get_domain_suggestions(call_hash)
end

def get_domain_suggestions(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Route53Domains::Client.new(region: region)
  response = client.get_domain_suggestions if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_domain_suggestions(params)
