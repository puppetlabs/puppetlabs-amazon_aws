...
def operation_with_hash(arg_hash)
  ...
  client.describe_folder_contents(call_hash)
end

def describe_folder_contents(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WorkDocs::Client.new(region: region)
  response = client.describe_folder_contents if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_folder_contents(params)
