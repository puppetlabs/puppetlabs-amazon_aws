...
def operation_with_hash(arg_hash)
  ...
  client.describe_logging_options(call_hash)
end

def describe_logging_options(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoTAnalytics::Client.new(region: region)
  response = client.describe_logging_options if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_logging_options(params)
