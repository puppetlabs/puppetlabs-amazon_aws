...
def operation_with_hash(arg_hash)
  ...
  client.update_client_certificate(call_hash)
end

def update_client_certificate(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.update_client_certificate if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_client_certificate(params)
