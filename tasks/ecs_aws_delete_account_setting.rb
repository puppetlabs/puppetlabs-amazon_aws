...
def operation_with_hash(arg_hash)
  ...
  client.delete_account_setting(call_hash)
end

def delete_account_setting(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.delete_account_setting if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_account_setting(params)
