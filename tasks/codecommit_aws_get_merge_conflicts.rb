...
def operation_with_hash(arg_hash)
  ...
  client.get_merge_conflicts(call_hash)
end

def get_merge_conflicts(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::CodeCommit::Client.new(region: region)
  response = client.get_merge_conflicts if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_merge_conflicts(params)
