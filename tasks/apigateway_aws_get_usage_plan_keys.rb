...
def operation_with_hash(arg_hash)
  ...
  client.get_usage_plan_keys(call_hash)
end

def get_usage_plan_keys(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.get_usage_plan_keys if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_usage_plan_keys(params)
