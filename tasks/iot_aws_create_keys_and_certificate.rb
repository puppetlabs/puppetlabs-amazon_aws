...
def operation_with_hash(arg_hash)
  ...
  client.create_keys_and_certificate(call_hash)
end

def create_keys_and_certificate(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.create_keys_and_certificate if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = create_keys_and_certificate(params)
