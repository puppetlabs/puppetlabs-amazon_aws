...
def operation_with_hash(arg_hash)
  ...
  client.modify_cluster_iam_roles(call_hash)
end

def modify_cluster_iam_roles(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.modify_cluster_iam_roles if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = modify_cluster_iam_roles(params)
