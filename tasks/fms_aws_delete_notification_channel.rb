...
def operation_with_hash(arg_hash)
  ...
  client = Aws::FMS::Client.new(region: region)
  client.delete_notification_channel(call_hash)
end

def delete_notification_channel(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::FMS::Client.new(region: region)
  response = client.delete_notification_channel if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_notification_channel(params)
