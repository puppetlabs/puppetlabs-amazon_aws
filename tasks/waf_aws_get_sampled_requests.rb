...
def operation_with_hash(arg_hash)
  ...
  client.get_sampled_requests(call_hash)
end

def get_sampled_requests(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::WAF::Client.new(region: region)
  response = client.get_sampled_requests if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_sampled_requests(params)
