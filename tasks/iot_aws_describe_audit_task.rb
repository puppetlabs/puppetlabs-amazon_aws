...
def operation_with_hash(arg_hash)
  ...
  client.describe_audit_task(call_hash)
end

def describe_audit_task(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IoT1ClickDevicesService::Client.new(region: region)
  response = client.describe_audit_task if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_audit_task(params)
