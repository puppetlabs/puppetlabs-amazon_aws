...
def operation_with_hash(arg_hash)
  ...
  client.delete_service_linked_role(call_hash)
end

def delete_service_linked_role(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::IAM::Client.new(region: region)
  response = client.delete_service_linked_role if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_service_linked_role(params)
