...
def operation_with_hash(arg_hash)
  ...
  client.update_request_validator(call_hash)
end

def update_request_validator(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::APIGateway::Client.new(region: region)
  response = client.update_request_validator if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = update_request_validator(params)
