...
def operation_with_hash(arg_hash)
  ...
  client.get_relational_databases(call_hash)
end

def get_relational_databases(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Lightsail::Client.new(region: region)
  response = client.get_relational_databases if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = get_relational_databases(params)
