...
def operation_with_hash(arg_hash)
  ...
  client.delete_interconnect(call_hash)
end

def delete_interconnect(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::DirectConnect::Client.new(region: region)
  response = client.delete_interconnect if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_interconnect(params)
