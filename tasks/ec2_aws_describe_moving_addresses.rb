...
def operation_with_hash(arg_hash)
  ...
  client.describe_moving_addresses(call_hash)
end

def describe_moving_addresses(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.describe_moving_addresses if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_moving_addresses(params)
