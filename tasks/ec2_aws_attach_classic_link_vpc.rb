...
def operation_with_hash(arg_hash)
  ...
  client.attach_classic_link_vpc(call_hash)
end

def attach_classic_link_vpc(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::EC2::Client.new(region: region)
  response = client.attach_classic_link_vpc if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = attach_classic_link_vpc(params)
