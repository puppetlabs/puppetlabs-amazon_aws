...
def operation_with_hash(arg_hash)
  ...
  client.deregister_container_instance(call_hash)
end

def deregister_container_instance(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::ECS::Client.new(region: region)
  response = client.deregister_container_instance if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = deregister_container_instance(params)
