...
def operation_with_hash(arg_hash)
  ...
  client.delete_baidu_channel(call_hash)
end

def delete_baidu_channel(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Pinpoint::Client.new(region: region)
  response = client.delete_baidu_channel if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = delete_baidu_channel(params)
