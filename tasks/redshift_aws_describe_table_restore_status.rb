...
def operation_with_hash(arg_hash)
  ...
  client.describe_table_restore_status(call_hash)
end

def describe_table_restore_status(*args)
  argstring = args[0]
  argstring.delete('\\')
  ...
  client = Aws::Redshift::Client.new(region: region)
  response = client.describe_table_restore_status if arg_hash.empty?
...
def task
  ...
  params = STDIN.read
  result = describe_table_restore_status(params)
